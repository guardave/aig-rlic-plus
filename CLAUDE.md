...
- **Streamlit Cloud URL (preview / dawodev):** `https://aig-rlic-plus-dawodev.streamlit.app/` — the active preview app. As of the lead-horizon wave (2026-06) it tracks branch **`fix260620_lead_horizon`**; use this URL for cloud-render verification while that branch is in flight. Repoint it to whatever branch is in flight when the current one merges. **Caution:** a similarly-named app `aig-rlic-plus-fix260620.streamlit.app` tracks a DIFFERENT/older branch — do NOT verify against it (it served stale old-winners during the lead-horizon wave). Always confirm which branch/commit a preview tracks before diagnosing "stale cloud."
- **Branch tracked:** main URL (`aig-rlic-plus.streamlit.app`) tracks `main`; the dawodev preview tracks the in-flight feature branch. Both auto-redeploy on push, with occasional uneven file-sync lag (a known `META-FRD` class — see `docs/agent-sops/team-coordination.md`); a reboot, not a plain push, forces a clean full re-sync.
