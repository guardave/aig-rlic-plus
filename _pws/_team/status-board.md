...
## 2026-06-02 — Lead — EOD: prospective-pairs SSoT shipped + LEAD-NPB1/GATE-CMP1 infrastructure + crude_oil_xle Mode-2 verification

**Status:** HANDOVER. Mode 2 exit criteria NOT YET MET; three of four checker subagents returned FAIL/FAIL_WITH_WARNINGS on `crude_oil_xle`. Resume next session by getting user's Category B disposition decision and fixing Categories A + C.

**Work shipped to main:**
- `2510ba0` (merge) fix260602_prospective_pairs — `docs/Indicator x Target.csv` (115 Done-Y pairs) + `scripts/build_prospective_pairs.py` generator + `data/prospective_pairs.csv` (now the SSoT for the priority universe) + landing page Reports/Status tabs (st.table pivot, alphabetical sort, inline alias format, dynamic 115 denominator everywhere) + LEAD-DV1 SOP rule (Pre-master row-2 verification before merging indicators) + retire `docs/priority-combinations-catalog.md`. Production verified clean.

**Work on `fix260602_pair4_prep` branch (6 commits ahead of main, pushed, NOT merged):**
- `bf78a05` GATE-CMP1 wrapper (`scripts/gate_pair_completeness.py`)
- `1338900` LEAD-NPB1 SOP rule + first brief draft (later replaced)
- `1256922` Retire priority-combinations-catalog.md (consolidated into earlier merge planning, kept here)
- `0380d51` Brief refresh to `crude_oil_xle` + drop "Pair #N" framing per user direction
- `433c937` BL items converted from brief-bullets to mechanical `_check_backlog_hygiene` checks; brief becomes thin neutral contract (user feedback)
- `c59f0c2` `crude_oil_xle` full Mode-2 build (Dana/Evan/Vera/Ray/Ace hats); winner `wti_high_vol_long` OOS Sharpe 0.47 vs B&H 0.04; GATE-CMP1 PASS (94 PASS / 0 FAIL)

**Discoveries / insights:**

1. **LEAD-NPB1 thin-brief discipline works.** Original draft injected prejudice (domain notes, expected directions, BL-item list). User flagged it; rewritten as identity-only + acceptance gate. Validated on crude_oil_xle: data picked vol-regime (not momentum, which my old draft implicitly favoured by listing it first).
2. **The cleanest SOP guardrails are mechanical, not judgmental.** Per user direction Q2 "cut in the middle, schema/presence + mandatory items exist", BL items 002/003/004/801/APP-PT1/DUP-4/DUP-11/DUP-15/COMMISSION-BASIS now run as on-disk checks in `_check_backlog_hygiene` rather than Lead-side brief mentions. Agents inherit the discipline mechanically.
3. **GATE-CMP1 floor + human checker ceiling** — gate caught real structural issues (typos, missing fields, missing PNGs, wrong key naming) that would have shipped broken otherwise. Completeness human checker still found 5 gate items the script lets through (analysis_brief, master parquet, exploratory subdir shape, core_models dir, acceptance.md). The mechanical gate is the floor.
4. **Prose-authoring hallucination is the new failure mode.** Pearson 0.55-vs-0.26 across 4 prose locations vs the JSON I had just emitted — wrote from memory instead of reading the source. Recorded in `memories.md` as a discipline rule. Forcing-function candidate: GATE-CMP1 grep-cross-check of numeric tokens in pair_config / narrative md against source artifacts.
5. **Streamlit Cloud cold-start triage** — first request after branch repoint can spin 30-90s. User reported "infinite loop" on dawodev; Playwright probe showed steady state at t+3s. Saved as auto-memory `reference_cloud_cold_start.md`.
6. **"Pair #N" numbering is ambiguous** (per-target vs global vs build-order) — dropped from LEAD-NPB1 and the brief. Refer by `pair_id` (`crude_oil_xle`) or indicator-target description (`Crude Oil Price → XLE`).
7. **`docs/priority-combinations-catalog.md` was orphaned.** No code consumed it. `data/prospective_pairs.csv` is now the SSoT for the priority universe; the catalog is retired with 5 referencing docs updated.

**Outstanding work for next session:**

1. Get user decision on Category B disposition (fill missing artifacts / extend gate mechanically / document as exception)
2. Fix Category A real bugs (9 items: Pearson 0.55→0.26, trade-log P&L attribution, gross-vs-net charts, R² claim, 4-vs-3 momentum count, broker pnl_pct, log-vs-simple drawdown, transition-counter rename, hardcoded interpretation_metadata fields)
3. Re-dispatch Correctness + Consistency checkers
4. Fix Category C ELI5 polish (Sharpe undefined first mention, etc.; find-and-replace scale)
5. Re-dispatch ELI5 checker
6. Re-run GATE-CMP1 → cloud re-verify on dawodev → merge `fix260602_pair4_prep` → main

**Branch state at session end:**

| Branch | Status | Tip |
|---|---|---|
| `main` | clean, deployed | `2510ba0` |
| `fix260602_pair4_prep` | held, pushed | `c59f0c2` (6 ahead) |

dawodev currently pointed at `fix260602_pair4_prep`.
