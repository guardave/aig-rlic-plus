...
completed_pair_ids = {p["pair_id"] for p in pairs}
n_universe_total = prospective_n_universe()
n_completed_count = prospective_n_completed(completed_pair_ids)

# --- Helper: performance coloring (defined once, used inside Reports tab) ---
...
with reports_tab:
    ...
    display_pairs = filtered

    st.markdown(
        f"**Showing {len(display_pairs)} of {total_pairs} pair{'s' if total_pairs != 1 else ''}**"
    )
    st.markdown("")

    # --- Render cards in 3-column grid ---
    cols_per_row = 3
    for i in range(0, len(display_pairs), cols_per_row):
        cols = st.columns(cols_per_row)
        for j, col in enumerate(cols):
            idx = i + j
            if idx >= len(display_pairs):
                break
            p = display_pairs[idx]
...
with status_tab:
    st.markdown(
        f"### Prospective-pair universe — {n_completed_count} of {n_universe_total} completed"
    )
    st.caption(
        "Rows = indicators in the source `docs/Indicator x Target.csv` matrix. "
        "Columns = 9 sector-ETF targets. ✅ = completed analysis, "
        "🚫 = archived/deprecated, · = in universe but not started, blank = not in universe. "
        "When an indicator appears under multiple CSV names, the additional names are shown "
        "inline as \"(also: …)\"."
    )
    pivot = build_status_pivot(completed_pair_ids)
    if pivot.empty:
...
st.markdown(
    ...
    f"Generated with AIG-RLIC+ | {n_completed_count} of {n_universe_total} priority pairs completed"
    "</div>",
    unsafe_allow_html=True,
)
