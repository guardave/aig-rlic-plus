...
def load_evidence_status(pair_id: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if errors or data is None:
        # APP-SEV1 L2: schema-invalid but file-exists case. Attempt a raw JSON
        # read so a still-honest disposition can be surfaced while the
        # producer fixes the schema mismatch (e.g. v5 sequential pairs with
        # qa_status=pending_quincy fail the qa_passed conditional but the
        # underlying failed_final_exam status is the dispositive fact for
        # the reader). DPS-FE2 routing depends on the status being honest.
        try:
            import json as _json
            with open(path) as _f:
                raw = _json.load(_f)
            status = raw.get("status", "found_in_search")
            copy = _STATUS_COPY.get(status, _STATUS_COPY["found_in_search"])
            merged = {
                **raw,
                "label": raw.get("label") or copy["label"],
                "short_explanation": raw.get("short_explanation") or copy["short"],
                "plain_english": raw.get("plain_english") or copy["plain"],
                "source": "evidence_status_json_raw_schema_warn",
            }
