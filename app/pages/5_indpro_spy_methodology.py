"""Finding 2 — Methodology: INDPRO → SPY Technical Appendix."""
...
st.set_page_config(
    page_title="IP Methodology | AIG-RLIC+",
    page_icon="📐",
    layout="wide",
...
with col1:
    st.metric("Full Sample", "1990-01 to 2025-12", delta="432 monthly observations")
with col2:
...
| Predictive OLS | Baseline regression with HC3 robust SEs | 3 signals x 4 horizons = 12 regressions |
| Local projections (Jorda) | Impulse response at multiple horizons | HAC (Newey-West) standard errors |
| Regime-dependent LP | Interaction with contraction dummy | Tests H3 (asymmetric effect) |
...
| Random Forest | Walk-forward feature importance | 200 trees, max_depth=5, 20 windows |
""")
...
| **Threshold methods (6)** | Fixed percentile (IS), rolling percentile, rolling z-score, zero-crossing, HMM prob, Markov-Switching prob |
| **Strategies (3)** | Long/Cash, Signal-Strength, Long/Short |
| **Lead times (5)** | 0, 1, 2, 3, 6 months |
| **Lookback windows (3)** | 3Y, 5Y, 10Y |
| **Total grid** | ~8,100 raw; ~1,666 after pruning |
""")
