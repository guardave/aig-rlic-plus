...
with st.expander("Deeper dive (background only -- out of scope for this pair)"):
    st.markdown(
        "*Is there a deeper signal within the credit market itself? "
        "(The CCC-BB quality spread -- educational background.)*\n\n"
        ...
        "The spread between CCC and BB yields -- commonly called the **quality "
        "spread** -- is a more granular credit-market stress signal. When this "
        "quality spread widens, it suggests investors are specifically fleeing "
        "the weakest, most default-prone companies. Historically this has often "
        "moved before the broader HY-IG spread fully reflects stress, because "
        "the weakest links break first.\n\n"
        "As a general market observation, during the GFC the CCC-BB quality "
        "spread began widening months before the overall HY-IG spread reached "
        "crisis levels; during COVID the quality-spread spike was even more "
        "dramatic, with CCC-rated bonds briefly yielding over 20% while BB "
        "bonds remained relatively contained. In the credit-markets literature "
        "the quality spread is sometimes described as a \"canary in the coal "
        "mine\" within credit itself.\n\n"
        "**Scope note.** The CCC-BB quality spread is **not** part of this "
        "pair's analytical universe. The authoritative list of in-scope "
        "signals for HY-IG v2 x SPY is the Signal Universe table on the "
        "Methodology page (rendered from `signal_scope.json`). CCC-BB was "
        "noted during exploration but is logged as informational only -- "
        "see *Analyst Suggestions for Future Work* on the Methodology page "
        "for the formal record and for candidate follow-up pairs."
    )
