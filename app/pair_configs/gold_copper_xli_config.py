...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "dashed line is XLI buy-and-hold (Sharpe 0.66). Per the Lead-Grid "
        ...
        "a different clock, misleadingly peaked at L10). The winner reads the "
        "gold/copper z-score same-day (L0), real-time floor L0 (close-of-day)."
    ),
    question=(
    ...
    chart_caption=(
        ...
        "(same-day) lead (OOS Sharpe 1.2730)** and falls away as the signal is "
        "lagged (0.43 at 1d, ~0.58 at 5–21d, ~0.04–0.44 from 63d out) — a "
        "coincident edge, not a lead. The best-of-any-signal envelope also peaks "
        "at 0d (1.27) and the extension to 126/252 trading days finds no "
        "long-lead edge (best ~0.79/0.38, both **failing a t>3 hurdle**). "
        "Sharpe clears buy-and-hold XLI (0.66) at the front of the grid."
    ),
    observation=(
        "On the native daily grid the published winner "
        "(`gold_copper_zscore_126d / T2_p50 / P1_long_cash`, OOS Sharpe "
        "**1.2730**) is a **same-day (0-day lead) coincident edge**: its own "
        "curve is highest at 0d (1.27) and drops sharply as the signal is lagged "
        "(1d 0.43, 5d 0.58, 21d 0.58, 63d 0.11), with 252d near zero (0.04). "
        "This is the **fast daily risk-off reaction** of industrial stocks to a "
        "rising gold/copper ratio — a same-day dynamic, not a forecast.\n\n"
        "The free full-grid daily selection (no cap) extends to 126/252 trading "
        "days to test for a slow leading edge and finds none: the best combo at "
        "126d/252d scores ~0.79/0.38 and **both fail the raised t>3 hurdle** "
        "(HAC t≈2.1/1.0). The old monthly-resampled view of this block appeared "
        "to peak at L10 — that was a slower-clock, monthly-rebalanced pattern on "
        "resampled data (a different instrument), NOT a better version of the "
        "daily rule; the correct daily axis shows the tradable edge is "
        "same-day. The winner survives its own hurdle (HAC t=3.50>3)."
    ),
    interpretation=(
        "The honest read: **this is a coincident, same-day edge, not a leading "
        "indicator.** The earlier narrative reported a monthly L10 'peak' off a "
        "resampled axis and had to explain it away as a different timescale; on "
        "the correct daily axis the risk-adjusted profile peaks at 0-day lead "
        "and decays with lag, and the full daily grid out to a year surfaces no "
        "durable long-lead edge (126/252-day candidates fail t>3). A reader "
        "should read L0 as what it is — a fast, regime-conditional risk-off "
        "reaction the daily rule trades contemporaneously — not a forecast of "
        "industrials weeks or months ahead."
    ),
    key_message=(
        "On its native daily axis the winner is a **same-day (0-day lead) "
        "coincident edge** (OOS Sharpe 1.2730, HAC t=3.50>3): the curve peaks at "
        "0d and decays as the signal is lagged, and the full daily grid out to "
        "252 trading days finds no leading edge (126/252d candidates fail t>3). "
        "A rising gold/copper ratio and weaker industrials move together the "
        "same day — the daily rule de-risks contemporaneously, it does not "
        "forecast months ahead. (The old monthly L10 'peak' was a resampling "
        "artifact on a slower clock.)"
    ),
)
