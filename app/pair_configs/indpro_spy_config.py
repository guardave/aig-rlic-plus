...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "on the SAME grid; the dashed line is SPY buy-and-hold (Sharpe 0.90). "
        ...
        "earlier version of this block used a separate exploratory *comparator* "
        "sweep on a different grid, which suggested a broad L3–L8 ridge; that "
        "was a grid artefact. The winner-selecting native grid, shown here, "
        "tells a sharper — and more honest — story.)"
    ),
    question=(
        "On the tournament's own grid, is the winner's L4 a durable ridge or a "
        "single-lead spike — and do the lead-correlation and the tournament "
        "agree on it?"
    ),
    how_to_read=(
        "Orange line + green star: the deployed winner's Sharpe by lead, peaking "
        "at its traded L4. A broad plateau around the traded lead is robust; a "
    ...
    chart_caption=(
        "The winner's own curve is a **sharp isolated spike at its traded L4 "
        "(OOS Sharpe 1.2301)** that collapses to ~0.12 at L3 and ~0.27 at L5 — "
        "NOT the broad ridge the earlier comparator sweep suggested. The one "
        "redeeming feature: that spike lands exactly where the lead-correlation "
        "for `indpro_mom` peaks (L4, r=+0.150), so two diagnostics agree on L4. "
        "But the best-of-any-signal envelope is **nearly flat** (leads span "
        "~0.95–1.23) and L4 only ties L11 (a contraction-signal combo at 1.23, "
        "margin 0.0001), so at the cross-signal level no lead is decisively best."
    ),
    observation=(
        "On the native tournament grid the published winner (`indpro_mom / "
        "T2_roll_p75 / P1_long_cash / L4`, OOS Sharpe **1.2301**) sits at the "
        "top of the lead profile at its traded L4. But its own curve is a "
        "**single isolated spike**: 1.23 at L4, collapsing to ~0.12 at L3 and "
        "~0.27 at L5, and staying below ~1.0 at every other lead. The one "
        "genuine positive is that the spike coincides with the lead-correlation "
        "peak for the same signal (`indpro_mom` at L4, r=+0.150) — two "
        "independent diagnostics agreeing on L4, which petrol/busloans lacked.\n\n"
        "The cross-signal envelope, however, is **nearly flat** (~0.95–1.23 "
        "across leads) and L4 (1.2301) only edges L11 (`indpro_contraction`, "
        "1.2300) by 0.0001 — a near-tie. So while the momentum rule's L4 is "
        "correlation-corroborated, the pair is still weakly identified at the "
        "cross-signal level, and the winner's own curve is a spike, not a ridge."
    ),
    interpretation=(
        "The honest summary: **on the native grid the winner's L4 is a sharp "
        "single-lead spike, not the broad ridge an earlier comparator sweep "
        "implied** — but, unusually for this fleet, the spike coincides with the "
        "lead-correlation peak for the same momentum signal (L4, r=+0.150), so "
        "the lead itself is corroborated by two diagnostics. The offsetting "
        "caution is the near-flat cross-signal envelope (L4 ties an L11 "
        "contraction combo at ~1.23), which keeps the pair low-confidence. Read "
        "L4 as the momentum rule's correlation-endorsed lead, but weight the "
        "single-lead spike and flat envelope as fragility flags."
    ),
    key_message=(
        "On the native grid the winner's L4 (OOS Sharpe 1.2301) is a **sharp "
        "single-lead spike** (collapsing to ~0.12/0.27 at L3/L5), not the broad "
        "ridge the old comparator suggested — but it lands exactly on the "
        "lead-correlation peak for `indpro_mom` (L4, r=+0.150), so two "
        "diagnostics agree on the lead. The near-flat cross-signal envelope "
        "(L4 ties L11 at ~1.23) keeps it low-confidence."
    ),
)
