...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "on the SAME grid; the dashed line is SPY buy-and-hold (Sharpe 0.88). "
    ...
    question=(
        "On the tournament's own grid, does the traded 3-month lead sit at a "
        "robust peak or on a flat, weakly-identified profile — on a pair with "
        "no forward edge?"
    ),
    how_to_read=(
        "Orange line + green star: the deployed winner's Sharpe by lead, peaking "
        "at its traded L3. Grey bars: the best any signal achieves at each lead "
        "(envelope ≥ the winner curve by construction). A tall isolated peak is "
        "a fragile point; a flat profile means the lead is weakly identified."
    ),
    chart_name="lead_sharpe_distribution",
    chart_caption=(
        "The winner's own curve peaks at its traded **L3 (OOS Sharpe 1.5377)**, "
        "which is also the top of the best-of-any-signal envelope — but the "
        "envelope is **nearly flat** across leads (top-two within ~0.08 Sharpe), "
        "so no lead is decisively best. Combined with the null lead-correlation "
        "result, the flat profile indicates a searched in-window result, not a "
        "robust predictive ridge."
    ),
    observation=(
        "On the native tournament grid the published winner "
        "(`ism_services_gap_50 / T3_zscore_neg_1.0 / P1_long_cash`, OOS Sharpe "
        "**1.5377**) sits at **the top of the lead profile** at its traded L3 — "
        "its own curve and the cross-signal envelope both peak there. But the "
        "envelope is **nearly flat**: leads differ by under ~0.08 Sharpe, and "
        "every lead clears buy-and-hold (0.88), so **no lead is decisively "
        "best** — the lead is weakly identified.\n\n"
        "(An earlier version of this chart used a separate exploratory sweep on "
        "a different grid, on which L3 appeared to sit in a dip; that was a "
        "grid artefact. On the tournament's own grid — the one that selected the "
        "winner — L3 is the top of a flat profile.) Given the lead-correlation "
        "null and the backward-causality finding, the flat Sharpe profile is "
        "best read as **a searched in-window result**, not a stable predictive "
        "relationship — exactly the low-confidence framing the rest of this "
        "page adopts."
    ),
    interpretation=(
        "The honest summary: **on the tournament's own grid the winner's lead "
        "is at the top of a flat Sharpe profile — weakly identified, not a "
        "sharp ridge — and the lead-correlation diagnostic finds nothing "
        "significant at any lead**, on a pair where causality runs backward. "
        "That is a caution flag, not a robustness badge. A reader should treat "
        "the strong OOS Sharpe as a searched result that scores well in-window "
        "and weight it accordingly, rather than as evidence the survey predicts "
        "the market."
    ),
    key_message=(
        "On the native grid the winner's lead L3 (OOS Sharpe 1.5377) is at the "
        "top of a **nearly flat** lead profile — not a robust ridge. With the "
        "lead-correlation null and backward causality, that Sharpe is best read "
        "as a searched, low-confidence result, not a discovered predictive lead."
    ),
)
