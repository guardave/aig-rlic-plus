...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "Solid markers are leads the tournament scored directly (L1,2,3,6,12); "
        "open markers are leads patched by the same validated rule. Read it "
        "alongside the backward-causality caveat: any Sharpe here is a searched "
        "result, not evidence of forward predictive content."
    ),
    question=(
        ...
        "assumed lead moved by a month?"
    ),
    how_to_read=(
        "Orange line + green star: the deployed winner's Sharpe by lead. A broad "
        "plateau around the traded lead is robust; a tall isolated point that "
        "collapses one lead either side is fragile. Grey bars: the best any "
        "signal achieves at each lead (envelope ≥ the winner curve)."
    ),
    chart_name="lead_sharpe_distribution",
    chart_caption=(
        "The winner's own curve is a **sharp isolated spike at its traded L2 "
        "(OOS Sharpe 1.6882)** that collapses to ~1.0 one month either side "
        "(L1 1.02, L3 1.03). L2 is also the top of the cross-signal envelope, "
        "but that peak is this same combo; every other lead sits in a flat "
        "~1.0–1.26 band. A single-lead spike on the deployed lead is the "
        "hallmark of a fragile, searched result, not a robust ridge."
    ),
    observation=(
        "On the native tournament grid the published winner "
        "(`m2sl_yoy_accel_pct / T1_fixed_p50 / P1_long_cash`, OOS Sharpe "
        "**1.6882**) is the top of the lead profile at its traded L2 — but its "
        "own curve is a **single isolated spike**: it collapses to **~1.0 one "
        "month either side** (L1 1.02, L3 1.03), and the best-of-any-signal "
        "envelope away from L2 is a flat ~1.0–1.26 band. So the edge lives "
        "**entirely at the deployed lead** and would be missed by a one-month "
        "error.\n\n"
        "(An earlier version of this chart used a separate exploratory sweep on "
        "a different grid, which placed the spike at L1 and the traded L2 on a "
        "flat plain; that was a grid artefact. On the tournament's own grid the "
        "spike IS the traded L2.) A single-lead spike on the deployed lead is a "
        "textbook fragility warning. Combined with the weak lead-correlation and "
        "the backward-causality finding, the durable read is that M2's apparent "
        "edge is a searched, in-window artefact, not a robust predictive ridge."
    ),
    interpretation=(
        "The honest summary: **on the tournament's own grid the winner's edge is "
        "a single isolated spike at its deployed lead (L2) — collapsing to ~1.0 "
        "one month either side — not a durable ridge.** With a weak, "
        "insignificant lead-correlation and backward causality, this pair shows "
        "none of the hallmarks of a robust lead relationship. A reader should "
        "treat the strong OOS Sharpe as a searched result that scores well in "
        "one window and weight it as low-confidence, exactly as the rest of the "
        "Evidence page advises."
    ),
    key_message=(
        "On the native grid the winner's edge is a **single isolated spike at "
        "its traded L2 (1.6882)** that collapses to ~1.0 one month either side — "
        "a fragility warning, not a ridge. With weak lead-correlation and "
        "backward causality, that OOS Sharpe is a searched, low-confidence "
        "result."
    ),
)
