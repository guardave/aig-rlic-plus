...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "the pair's NATIVE DAILY axis — trading days {1, 5, 21, 63, 126, 252} — "
        "against the best-of-any-signal envelope on the same grid; the dashed "
        "line is SPY buy-and-hold (Sharpe 0.82). Per the Lead-Grid Frequency "
        "Standard (2026-07-15) the axis unit equals the signal's native daily "
        "release, replacing the old monthly resample. This pair FLOORS at L1 "
        "(NO L0): a same-day SOX/SPY reading is co-movement (corr 0.71), not a "
        "forecast. Unlike the other three daily pairs (0-day coincident), this "
        "winner sits at a GENUINE ~1-quarter (63-trading-day) LEAD."
    ),
    question=(
        "Is the 63-day lead a real quarter-ahead ridge, or an isolated peak — "
        "do shorter leads decay/reverse, and does 63d clear the t>3 hurdle "
        "while the pair's 'thin edge' caveats still bite?"
    ),
    how_to_read=(
        "Orange line + star: the deployed winner's OOS Sharpe at each daily "
        "lead, with the star at its traded 63-day lead. Grey bars: the best any "
        "signal reaches at each lead (envelope ≥ the winner curve). A curve that "
        "PEAKS at 63d with weaker neighbours is a genuine but relatively "
        "isolated lead; a broad plateau would be a robust ridge."
    ),
    chart_name="lead_sharpe_distribution",
    chart_caption=(
        "The winner's own curve on the DAILY axis **peaks sharply at its traded "
        "63-day (~1-quarter) lead (OOS Sharpe 1.57, HAC t=4.10 — clears t>3)**, "
        "with much weaker neighbours (1d 0.52, 5d 0.81, 21d 0.30) and decay past "
        "it (126d 1.10, 252d ~0). So it is a GENUINE quarter-ahead lead, NOT "
        "coincident — but a relatively ISOLATED peak, not the broad ridge the "
        "old monthly resample suggested. The best-of-any-signal envelope is high "
        "at every lead (~1.4–1.5), a favourable-window artefact consistent with "
        "this pair's heavy overfitting caveats."
    ),
    observation=(
        "On the native daily grid the published winner "
        "(`sox_spy_ratio_mom_6m_pct / T2_roll_p75 / P1_long_cash`, OOS Sharpe "
        "**1.57**) is a **genuine ~1-quarter (63-trading-day) LEAD** — the only "
        "one of the four daily pairs that is not a same-day co-move. Its own "
        "curve peaks at 63d (1.57) and is much weaker at every other lead (1d "
        "0.52, 5d 0.81, 21d 0.30, 126d 1.10, 252d ≈ 0). Only the 63-day lead "
        "clears the raised **t>3 hurdle (HAC t=4.10)**; the neighbours do not "
        "(5d t=2.01, 21d t=0.84, 126d t=2.48). So the quarter-ahead structure is "
        "real, but it is a relatively **isolated peak**, not the 'broad L0–L4 "
        "ridge' the old monthly-resampled view reported (that was a resampling "
        "artefact on a slower clock).\n\n"
        "**The lead is real; the alpha is thin.** Durability of the LEAD is not "
        "durability of the EDGE: the winner's IS Sharpe is 0.10 vs OOS 1.57 (a "
        "favourable 2021–26 semis-bull draw), the MEDIAN valid combo (0.67) sits "
        "BELOW buy-and-hold (0.82), the strategy LOST in every pre-OOS stress "
        "episode, and Granger is bidirectional (feedback, not a clean one-way "
        "lead). Both statements are true and both are stated here for honesty."
    ),
    interpretation=(
        "The honest summary has two parts. First, on the correct daily axis the "
        "**63-day lead is genuine and statistically distinct** — it peaks at 63d "
        "and clears t>3 there while its neighbours do not, so this is a real "
        "quarter-ahead relationship, not co-movement and not (as the monthly "
        "view implied) a broad flat ridge. Second, **a genuine lead does not "
        "rescue a thin, window-dependent edge**: the OOS Sharpe leans heavily on "
        "a chip-friendly test window, the median searched combo loses to "
        "buy-and-hold, and the signal failed every historical stress episode. "
        "Take the 63-day lead as real structure and the strategy's alpha as "
        "LOW-CONFIDENCE — the framing the rest of this Evidence page carries."
    ),
    key_message=(
        "On its native daily axis the winner is a **genuine ~1-quarter "
        "(63-trading-day) LEAD** (OOS Sharpe 1.57, HAC t=4.10 clears t>3) — the "
        "only non-coincident pair of the four. Its curve peaks sharply at 63d "
        "with much weaker neighbours (a real but relatively isolated lead, not "
        "the monthly view's 'broad ridge'). But the edge is thin and "
        "window-dependent (IS 0.10 vs OOS 1.57; median valid combo below B&H; "
        "lost every pre-OOS stress episode): the lead is durable, the alpha is not."
    ),
)
