...
LEAD_TOURNAMENT_BLOCK = dict(
    ...
    method_theory=(
        ...
        "dashed line is SPY buy-and-hold (Sharpe 0.77). Per the Lead-Grid "
        ...
        "lead axis — NOT the monthly resample this block used to show (which, on "
        "a slower clock, misleadingly peaked at L3). The winner reads the "
        "VIX/VIX3M term-structure z-score same-day (L0), real-time floor L0."
    ),
    question=(
    ...
    how_to_read=(
        ...
        "lead, peaking at its traded 0-day lead. Grey bars: the best any signal "
        "achieves at each lead (envelope ≥ the winner curve by construction). A "
        "profile that decays as the signal is lagged means the edge is "
    ...
    chart_caption=(
        ...
        "(same-day) lead (OOS Sharpe 1.1295)** and falls away as the signal is "
        "lagged (1.09 at 1d, 0.79 at 5d, ~0.53–0.67 from 63d out) — a coincident "
        "edge, not a lead. **Low-confidence flag:** the winner's Newey-West HAC "
        "t=2.98 sits just BELOW the strict t>3 multiple-testing hurdle — a "
        "modest, borderline edge (weaker than the other daily pairs). The grid "
        "extension to 126/252 days finds no long-lead edge (long-lead bests "
        "~0.84–0.98 all fail t>3). Sharpe clears buy-and-hold (0.77) only at the "
        "front of the grid."
    ),
    observation=(
        "On the native daily grid the published winner "
        "(`vix_ratio_zscore_126d / T2_rp75 / P1_long_cash`, OOS Sharpe "
        "**1.1295**) is a **same-day (0-day lead) coincident edge**: its own "
        "curve is highest at 0d (1.13) and falls as the signal is lagged (1d "
        "1.09, 5d 0.79, 21d 0.76, 63d 0.53). This is the **fast same-day "
        "risk-off reaction** of equities to a VIX term-structure stress spike — "
        "a same-day dynamic, not a forecast.\n\n"
        "**Honest confidence caveat:** the winner's Newey-West HAC t-stat is "
        "**2.98 — marginally UNDER the raised t>3 hurdle** (Harvey-Liu-Zhu). It "
        "reproduces the committed headline exactly and remains the global-max "
        "valid combo, but it is a lower-confidence edge than hy_ig (t=3.8) or "
        "gold_copper (t=3.5); read the 1.1295 as a real-but-modest coincident "
        "signal, not a strong one. The free full-grid daily selection extends to "
        "126/252 days and finds no leading edge either — the best long-lead "
        "combos (~0.84–0.98) all fail t>3 (HAC t≈2.3–2.8). The old monthly-"
        "resampled view of this block appeared to peak at L3 (1.87); that was a "
        "slower-clock, monthly-rebalanced pattern on resampled data, a different "
        "instrument — NOT a better version of the daily rule."
    ),
    interpretation=(
        "The honest read: **this is a coincident, same-day edge — real but "
        "modest, and lower-confidence than the other daily pairs.** The earlier "
        "narrative reported a monthly L3 'peak' off a resampled axis and had to "
        "explain it away as a different timescale; on the correct daily axis the "
        "risk-adjusted profile peaks at 0-day lead and decays with lag, so there "
        "is no leading edge to switch to (the 126/252-day candidates fail t>3). "
        "But the winner itself only reaches HAC t=2.98, just short of the strict "
        "multiple-testing hurdle — so treat it as a low-confidence same-day "
        "risk-off signal, not a robust one. A reader should NOT read L0 as a "
        "forecast, and should weight this pair's edge accordingly."
    ),
    key_message=(
        "On its native daily axis the winner is a **same-day (0-day lead) "
        "coincident edge** (OOS Sharpe 1.1295) that decays as the signal is "
        "lagged, with no leading edge out to 252 days (long leads fail t>3). "
        "**Confidence is modest:** the winner's HAC t=2.98 is just below the "
        "strict t>3 hurdle — a real-but-borderline same-day risk-off signal, "
        "weaker than hy_ig/gold_copper. The old monthly L3 'peak' was a "
        "resampling artifact on a slower clock, not a better rule."
    ),
)
