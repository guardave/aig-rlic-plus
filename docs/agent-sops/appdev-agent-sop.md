...
- [ ] **GATE-CL6 — Cross-Period Consistency section renders without `st.error` for all active pairs (Wave 10J, 2026-04-24; relocated 2026-06-10).** Per stakeholder direction (fix260610_xpair_general) the Cross-Period Consistency section now renders inside `render_strategy_page()`'s **Confidence tab** (after Walk-Forward Rolling Sharpe, before Tournament Scatter), NOT on the Evidence page. Verify that every active pair's Strategy page Confidence tab either (a) shows the cross-period charts when the JSON files are present, or (b) shows `st.info("Cross-period analysis pending — ... chart not yet available for this pair.")` when they are absent. A bare `st.error` in this section is a gate failure. Verification: load each Strategy page with headless Playwright, activate the Confidence tab, and grep the rendered text for "Error" in the Cross-Period Consistency region.
...
> **Terminology note (Wave 10J taxonomy, 2026-04-24):** Ace's APP-ST1 check is now called **"portal lint"** — AST syntax parsing + `load_plotly_chart` non-None assertion. The term "smoke test" is reserved exclusively for Quincy's end-to-end cloud verification via `scripts/cloud_verify.py`. Do not use "smoke test" to refer to Ace's loader checks.
...
- **Before Ace finishes a page**, run the portal lint that:
  ...
  3. Asserts the return value is not None AND `len(fig.data) > 0` AND the figure is self-titled: `fig.layout.title.text` is a non-empty string, **OR** — for multi-panel figures (amended 2026-06-11, fix260611_meta_cmp) — at least one non-empty subplot-title annotation is present (`make_subplots(subplot_titles=...)` emits these as `layout.annotations`). Rationale: the VIZ-QR1 dual-panel regime charts are self-describing via two subplot titles and carry no overall title by user-approved design (canonical umcsent_xlv reference layout); the criterion's intent is "no anonymous charts", not "exactly one title field". An overall title remains acceptable where present (e.g. hy_ig_spy's regime chart carries both).
