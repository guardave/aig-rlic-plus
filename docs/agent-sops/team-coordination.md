...
## Cross-Agent Impact Log Protocol (D3a — Wave 10J)

### Purpose

When any agent adds a new SOP rule that creates an obligation on another agent, the authoring agent must post an entry to `_pws/_team/status-board.md` under a `## Cross-Agent Impact Log` header. This makes cross-agent obligations visible at SOD without requiring a separate registry file — the status board is already read at session start.
...
|---------|-------------|-----------------|-----------------|------|
| ECON-CP1 | Evan | Vera, Ray | Vera: generate CP1 charts; Ray: provide CP1 narrative | 10J |
```

| Field | Definition |
|-------|------------|
| `rule_id` | The SOP rule identifier (e.g., `ECON-DIR1`, `RES-OD1`) |
| `authored_by` | Which agent added the rule |
| `affected_agents` | Which other agents must respond (comma-separated) |
| `action_required` | One sentence per affected agent describing what they must do |
| `wave` | Wave number when the rule was added |

### SOD Obligation

All agents read the Cross-Agent Impact Log at SOD and process any entry where they appear in `affected_agents`. Processing means:
1. Identifying whether the required action is already complete
2. If not, adding the action to the current wave's work queue
3. Logging completion in their handoff note referencing the `rule_id`

### Authoring Obligation

When you add a SOP rule that affects another agent's behavior, you must post the Cross-Agent Impact Log entry **in the same commit** as the SOP change — not as a follow-up. An SOP rule that creates a cross-agent obligation without a corresponding status-board entry is a META-SRV violation.

---

## Lesson Recording Protocol (D3b — Wave 10J)

### Entry Gate (Ace's model — primary gate)

A lesson must produce a SOP rule change before it enters any record. The pipeline is:

1. **Observation** — something goes wrong or a new pattern is identified
2. **Rule** — the lesson is formalized as a specific, actionable SOP rule with a rule ID
3. **Record** — the rule enters the relevant agent SOP AND the lesson enters `experience.md` with a forward-pointer to the rule

**No rule = not ready to record in experience.md.** A bare observation without a formalized rule may not enter experience.md as a lesson. It may only enter as a "pending-rule" entry (see below).

### Pending-Rule Status (Quincy's carve-out)

When a lesson cannot yet be formalized as a rule (e.g., insufficient data, ongoing debate, requires Lead arbitration), it may enter as `[PENDING-RULE]` status with a one-wave deadline:

```
[PENDING-RULE] <lesson summary> — deadline: Wave NN+1. Escalates to Lead if no rule by then.
```

After one wave without a corresponding rule, the lesson is **escalated to Lead** for arbitration. Lead either (a) formulates the rule, (b) accepts the lesson as-is with rationale, or (c) dismisses it with rationale.

### Forward-Pointer Requirement

Every `experience.md` entry that was generated by a lesson-to-rule pipeline must include a forward-pointer to the specific rule it generated:

```
→ Rule: [RULE-ID] in [agent]-agent-sop.md
```

Example:
```
Learned: producer-side schema creation-time check and consumer-side integration-time check catch different failure modes.
→ Rule: VIZ-V2 (blocking pre-handoff gate) + APP-CL1 (integration-time JSON shape check) in viz-agent-sop.md + appdev-agent-sop.md
```

An experience.md entry without a forward-pointer (for lessons that generated rules) is a META-AM violation and is flagged by QA-CL3.

### Post-Wave Ratification (Quincy's layer)

After each wave, Quincy runs a ratification round: each agent reads the Cross-Agent Impact entries added since the last wave and explicitly adopts or dismisses each entry. Results are logged in `_pws/_team/wave_NNx_lessons_ratified.md`.

---

## Canonical Handoff Format (T2 — Wave 10J)

### Authority

`docs/agent-sops/team-coordination.md` owns the canonical handoff definition. All agent SOPs reference this section — they do not define their own handoff variants. The format below supersedes any agent-SOP-specific handoff template. Per-agent SOPs may add agent-specific fields *after* the mandatory fields, but must not omit any mandatory field.

### Mandatory Fields (all six required in every handoff note)

Every handoff note — regardless of which agent pair is involved — must include all six fields:

```markdown
## Handoff Note — [sender_role] → [receiver_role]

**pair_id:** <e.g., indpro_spy>

**artifact_paths:**
- `/workspaces/aig-rlic-plus/<path/to/file1>`
- `/workspaces/aig-rlic-plus/<path/to/file2>`
- (list every delivered file with absolute repo path)

**schema_compliance:**
- Tool: <validation tool used, e.g., `python3 scripts/validate_schema.py --schema winner_summary --instance results/indpro_spy/winner_summary.json`>
- Verdict: PASS | FAIL
- Evidence: <exit code, log excerpt, or line count>

**known_gaps:**
- <explicit list of what is missing or incomplete, with rationale>
- (or "none")

**items_received_from_upstream:**
- <list what you received from the previous agent in the pipeline, confirming it was complete>
- (or "none — this is the first stage")

**questions_for_downstream:**
- <explicit questions the receiving agent needs to answer or act on>
- (or "none")
```
...
| Field | Purpose |
|-------|---------|
| `pair_id` | Ties the handoff to a specific analysis pair; enables cross-reference to results/ and output/ |
| `artifact_paths` | Absolute paths eliminate path ambiguity; receiver can verify existence immediately |
| `schema_compliance` | Producer-side self-validation per META-SRV — tool + verdict + evidence, not just a claim |
| `known_gaps` | Explicit declaration prevents receiver from discovering gaps silently; enables informed downstream planning |
| `items_received_from_upstream` | Bidirectional receipt — closes the loop on what was handed to you; flags if upstream delivery was incomplete |
| `questions_for_downstream` | Explicit questions prevent silent assumptions; receiver must address or escalate each one |

### Verification Terminology (Unified Taxonomy — Wave 10J Convergence)

The following terms are reserved and must not be used interchangeably:

| Term | Owner | Scope | Tool |
|------|-------|-------|------|
| **Schema lint** | Evan | `validate_schema.py` against `winner_summary.json` per pair | `scripts/validate_schema.py` |
| **Portal lint** | Ace | AST-parse + `load_plotly_chart` non-None assertion | `scripts/portal_lint.py` or equivalent |
| **Chart rendering validation (VIZ-CV1)** | Vera | VIZ-V5 JSON integrity + kaleido render to PNG | Vera's VIZ-V5 script |
| **Smoke test** | Quincy | `cloud_verify.py` + mandatory DOM-file read | `app/_smoke_tests/cloud_verify.py` |

**The term "smoke test" is reserved exclusively for Quincy's end-to-end cloud verification run (`cloud_verify.py`).** No other agent uses this term for their own validation steps. Other agents use the terms in the table above for their respective checks. Using "smoke test" in a handoff note to describe a schema lint, portal lint, or chart rendering validation is a terminology violation and may confuse cross-agent audit trails.
