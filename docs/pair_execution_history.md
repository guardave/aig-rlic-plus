...
## Lead-Horizon Wave Phase 1 — Native-tournament RE-RUN confirmation (fix260620_lead_horizon)

**Dispatch (Evan, 2026-06-20):** Execute the 3 monthly RE-RUN candidates the Phase 0 gating sweep flagged (indpro_spy, indpro_xlp, umcsent_xlv) by re-running each pair's **NATIVE** tournament on the extended monthly lead grid L0..12 (the only change vs the legacy `{0,1,2,3,6}` / `{0..6}` grids; all other dimensions — signals, thresholds, strategies, lookbacks, validity rule — exactly as the pair's own `stage_tournament` defines them). Inputs = the frozen monthly parquets through 2025-12-31 (no live re-fetch; reproducible, seed=42). Winner re-selected per ECON-T3 cascade + ECON-T4 (BENCHMARK `valid=False`). This is the native-machinery confirmation the 2026-06-13 Caveat mandated before any cascade.

**Harness validation:** restricting each native re-run to the legacy lead grid reproduces every published winner exactly (indpro_spy S6_mom3m/T1_fixed_p75/P1/L6 → 1.1036; indpro_xlp S8_accel/T2_roll_p75/P3_long_short_counter/L3 → 1.1147; umcsent_xlv S3_mom/T3_zscore_1.0/P1/L6 → 1.1586). The harness is faithful.

| pair | gating-sweep L* (Sharpe) | NATIVE L* (Sharpe) | native gate | decision | rationale |
|------|:--:|:--:|:--:|:--|:--|
| indpro_spy | 12 (1.374) | **4** (1.2301) | L*∉{7..12} | **NO RE-RUN** | Native best is L4 ∈ {0..6}; whole L7..12 region peaks at only 1.2300 (L11), below L4. The gating sweep's L12=1.374 is the **polarity-mirror** of native `S7_mom6m/T1_fixed_p75/P3` which scores −1.374 (valid=False) — a combo the native spy tournament cannot express (no polarity inversion on fixed-pct P3). Gate trigger FALSE → keep published L6 winner. (Native L4 beats published L6, but promoting it is outside ECON-LT1's L*∈7..12 scope — flagged to Lead as a separate decision.) |
| indpro_xlp | 8 (1.423) | **11** (1.3282) | L*∈{7..12} ✓ & 1.3282>1.1147 | **RE-RUN (cascaded)** | New winner S3_mom/T1_fixed_p50/P1_long_cash_pro/L11 supersedes S8_accel/T2_roll_p75/P3_long_short_counter/L3. xlp's native grid has pro/counter polarity, so L11 is natively expressible & valid. Guardrail recompute PASS (1.3282==1.3282). beats_benchmark TRUE (vs BH 0.7437). Cascaded: winner_summary.json, strategy_returns_20260620.csv (+meta, ECON-SR1 reconciled), winner_trade_log.csv (span), winner_trades_broker_style.csv, subperiod_sharpe.csv, tournament_results_20260620.csv. |
| umcsent_xlv | 11 (1.188) | **6** (1.1586) | L*∉{7..12} | **NO RE-RUN** | Native best is L6 ∈ {0..6} — and is **identical to the published winner** (S3_mom/T3_zscore_1.0/P1/L6, 1.1586). L7..12 region maxes at 1.0330 (L10), well below L6. The gating sweep's L11=1.188 was its simplified P1/P2 grid finding a different combo not native to this pipeline. Gate trigger FALSE → no change. |

**Net Phase 1 outcome: only 1 of the 3 sweep-flagged candidates (indpro_xlp) survives the native-tournament confirmation and is cascaded.** indpro_spy and umcsent_xlv do NOT gate to RE-RUN under the source-of-truth native tournament — the gating sweep's standardized P1/P2 two-polarity comparator manufactured high-Sharpe long leads (via polarity mirrors / combos absent from the native grids) that the native pipelines cannot reproduce. This is precisely the instrument-divergence the 2026-06-13 Caveat warned of. ECON-LT1 keys the gate on the native Sharpe winner's lead → no cascade for spy/umcsent. Escalated to Lead: (a) the spy native L4 winner (1.2301) outperforms the published L6 (1.1036) — a candidate winner upgrade outside this gate's L*∈7..12 scope; (b) the gating-sweep vs native divergence for all 3 should inform whether gold_copper_xli (the 4th sweep candidate) likewise needs native confirmation before any cascade.

## Lead-Horizon Wave Phase B — ECON-T5 cascades (fix260620_lead_horizon)

**Decision (stakeholder/Lead, 2026-06-20):** "ECON-T5 first, then re-run pairs under it." Phase A landed ECON-T5 (winner-selection provenance + tournament-CSV immutability guard) + winner_summary.schema v1.2.0. Phase B finalizes the two REAL lead-horizon re-runs UNDER ECON-T5, each winner now carrying a `selection` provenance block. Independently confirmed by the Codex/Ivy audit (xlp L11=1.3282 recomputes exactly; spy L4=1.2301 is a real extended-grid winner outside the coarse committed grid). Both winner_summaries validate against schema v1.2.0; the published coarse tournament CSVs (`indpro_spy/20260314`, `indpro_xlp/20260420`) remain immutable/untouched.

| pair | old winner (Sharpe) | new winner (Sharpe) | L* | guardrail recompute | provenance | benchmark |
|------|--------------------|--------------------|----|--------------------|------------|-----------|
| indpro_xlp | S8_accel/T2_roll_p75/P3_long_short_counter/L3 (1.1147) | **S3_mom/T1_fixed_p50/P1_long_cash_pro/L11** (1.3282) | 11 | PASS (1.3282 == 1.3282, ±0.03) | `selection` added to existing Phase-1 cascade: objective=max_oos_sharpe; grid L0..12; 6,966 valid; runner_up S3_mom/T1_fixed_p50/P1_long_cash_pro/L8 (1.2379); raw_winner_row src row 2814 of tournament_results_20260620.csv; divergence=null | beats BH 0.7437 ✓ |
| indpro_spy | S6_mom3m/T1_fixed_p75/P1_long_cash/L6 (1.1036) | **S3_mom/T2_roll_p75/P1_long_cash/L4** (1.2301) | 4 | PASS (1.2301 == 1.2301, ±0.03) | full fresh cascade under ECON-T5: objective=max_oos_sharpe; grid L0..12; 2,938 valid; runner_up S9_contraction/T3_zscore_1.0/P2_signal_strength/L11 (1.2300, a 0.0001 shadow — no Sharpe tie); raw_winner_row src row 1146 of tournament_results_20260620.csv; divergence=null (winner IS raw max); displaced L6 reproduces at 1.1036 in-grid | beats BH 0.8998 ✓ |
