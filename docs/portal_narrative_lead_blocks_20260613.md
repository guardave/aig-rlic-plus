...
| gold_copper_xli | 0 | **10** | 1.370 | 1.273 | ~~RE-RUN~~ → CHARTS-ONLY (reverted) | L0 / L5 |
...
### A.6 gold_copper_xli — Gold/Copper Ratio → XLI (Industrials)  **[CHARTS-ONLY — FINAL (re-run reverted, old winner retained)]**

> **Daily real-time signal — "no-lag" framing (ELI5).** The Gold/Copper Ratio is computed from commodity prices that trade every business day with zero publication lag — today's ratio is on your screen right now. This is fundamentally different from monthly macro indicators like Industrial Production or Building Permits, which are published 2-6 weeks after the reference month. For those indicators, the Lead Analysis reveals something non-obvious: the "natural lead" can be much longer than the publication lag (permit's optimal lead is L8-9, not L6). For a daily real-time signal like gold/copper, the question "how stale should the signal be?" has an obvious answer: not stale at all. You would not check last year's gold price to decide what to do today — today's price is right there on your screen. So the Lead Analysis confirming L0 as the correlation-optimal lead is the **expected result**, not a discovery. These blocks serve cross-pair comparability: a reader comparing gold_copper to permit can see that permits have a genuine lead-selection puzzle and commodity ratios do not.

#### CORRELATION_LEAD_VIEW_BLOCK ("Lead Analysis")

- **method_theory:** "The Correlation block fixes signal lag at zero and varies the forward horizon. The monthly-strategy *lead* question (daily ratio resampled to a monthly L=0..12 grid per ECON-LL1): how stale may the Gold/Copper Ratio signal get before trading XLI? For each transform we compute Pearson r between the signal lagged L months and XLI's 1-month forward return. Gold/copper is a risk-appetite proxy — a rising ratio (fear-metal over industrial-metal) is counter-cyclical for industrials. Because commodity prices are available every trading day with zero publication lag, the economically expected answer is L0: unlike factory-output numbers that arrive weeks late, today's gold and copper prices are live, so there is no structural reason to use a stale reading."
- **question:** "How many months should we lag the Gold/Copper Ratio signal to time XLI — and does the published same-day (L=0) lead match the data? For a zero-lag daily signal, L0 dominance is the expected result; the analysis confirms rather than discovers it."
...
- **chart_name:** `correlations_lead_view`
- **chart_caption:** "Pearson r between Gold/Copper Ratio signal lagged L months and XLI 1-month forward return. The significant cells are **short-lead and counter-cyclical**: `gold_copper_roc_63d` at **L0 (−0.162**)** and L1 (−0.126*), `gold_copper_mom_63d` at L0 (−0.155**), `gold_copper_roc_21d` at L0 (−0.127*) and L1/L12. The level/z-score transforms turn weakly *positive* around L5 (`gold_copper_pctrank_504d` +0.107, `zscore_126d` +0.106) but none reach significance."
- **observation:** "Reading directly: the momentum/RoC transforms carry significant **negative** cells at **L0-L1** — a recent rise in the gold/copper ratio precedes XLI weakness, the expected counter-cyclical sign. The smoothed level transforms hint at a weak positive ridge around L5 but stay below significance. So the *significant* linear content is contemporaneous-to-one-month, agreeing with the published L=0 lead. This is the economically expected outcome for a zero-lag daily signal: commodity prices are live, so the freshest reading carries the most information."
- **interpretation:** "The lead-correlation view **agrees** with the published **L=0** lead — the only significant cells are at L0-L1 with the correct counter-cyclical sign. For a daily real-time signal this is expected, not surprising: unlike monthly macro indicators (where the data arrives weeks late and the optimal lead can be much longer than the publication delay), commodity prices are available instantly. Using a 10-month-old gold/copper ratio when today's is freely available makes no economic sense. The L0 dominance here is the structural default for zero-lag signals — compare with Building Permits (L8-9) or INDPRO (L4-12) where the lead puzzle is genuine."
- **key_message:** "The lead-correlation view **agrees** with the published L=0 lead (significant counter-cyclical cells at L0-L1) — the expected result for a zero-lag daily signal where today's price is always available. No lead-selection puzzle exists for this pair, unlike the monthly macro indicators."

#### LEAD_TOURNAMENT_BLOCK ("Lead Tournament")  **[FINAL — re-run reverted, old winner retained]**

- **method_theory:** "We swept the full tournament across L=0..12 (daily ratio resampled to the monthly grid). The chart plots best OOS Sharpe per lead over the full combo cloud (1,581 valid combinations across all leads), with XLI buy-and-hold dashed. The old published winner (zscore_126d L0 P1, Sharpe 1.273) is retained — a Track-B re-run pilot was attempted and reverted."
- **question:** "Does a lead beyond the published same-day (L=0) produce a better risk-adjusted winner — and what does the answer mean for a zero-lag daily signal?"
- **how_to_read:** "Bars: max OOS Sharpe per lead. Strip: all valid combos at that lead. Tall-thin = single combo; flat-wide = robust regime."
- **chart_name:** `lead_sharpe_distribution`
- **chart_caption:** "Best OOS Sharpe per lead. The maximum across the extended grid is **L=10 (1.370, `gold_copper_pctrank_504d` / Tfix07_hi / P1)**, with a secondary peak at **L=5 (1.315)** and L=1 (1.195). The published L=0 best (1.016) sits below the old published winner (zscore_126d L0, Sharpe 1.273 from the original tournament). 1,581 valid combos total; median OOS Sharpe 0.605; L=0 rank 1 among all leads by the old winner's Sharpe."
- **observation:** "Reading the bars: two distinct peaks — **L=5 (1.315)** and **L=10 (1.370)** — separated by a mid-grid dip (L7-L9 around 0.94-0.99). The published L=0 (1.016 on the extended grid) is near the bottom; however, the old published winner at L=0 (Sharpe 1.273) comes from the original tournament's richer signal grid and remains the retained configuration after the re-run revert. Reading the strip: the L=10 cloud is comparatively wide at the top (p75 = 1.005), and the overall median across all leads is 0.605."
- **interpretation:** "The extended grid surfaces higher Sharpes at longer leads (L=5, L=10), but this pair's re-run was attempted and **reverted** — the old winner (zscore_126d L0, Sharpe 1.273) is retained. The revert revealed a methodology insight: for a daily real-time signal with zero publication lag, the L0-12 monthly lead sweep is less meaningful than for monthly macro indicators. L0 is the structurally expected best lead for a zero-lag signal — that is not a finding, it is an economic given. You would not check last year's gold price to decide what to do today when today's price is on your screen. The far-lead peaks (L=5, L=10) ride smoothed percentile transforms that mechanically benefit from longer lookback alignment, not from a genuine lead-selection edge. Compare with Building Permits, where the optimal lead (L8-9) genuinely exceeds the publication lag — that is a real discovery; this is not. **Charts-only; old winner retained.**"
- **key_message:** "The extended grid's best Sharpe is L=10 (1.37), but the re-run was reverted and the old winner (zscore_126d L0, Sharpe 1.273) is retained. For a zero-lag daily signal, L0 dominance is the economic default — these charts serve cross-pair comparability, not lead discovery. **Charts-only, no re-run.**"
