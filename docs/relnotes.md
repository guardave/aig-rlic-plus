...
- **Phase 1** — downstream render for the 2 re-run winners. New rule **ECON-SR3**: the three winner-dependent charts (`rolling_sharpe_cp`, `signal_dist`, `tournament_scatter`) must consume canonical `results/` sidecars and never re-simulate the winner from signal/threshold heuristics (the bug class that had been silently shipping the *old* winner on a winner change). `tournament_scatter` plots the immutable coarse published grid as population with the extended-grid winner as a distinct labeled overlay. Narrative honesty: `indpro_spy` reframed as risk-adjusted (total return 2.22× is **below** B&H 2.92×; the edge is drawdown control, not return), and a false legacy "22 months in cash through COVID" claim was removed.
- **Phase 2 — Option D weekly sweep.** Built the weekly lead-lag code path and swept the 4 daily pairs (+ settled `petrol_inv`). **Zero native-confirmed upgrades** — every sweep flag was an isolated-single-lead Sharpe spike (lucky-window overfit), not a durable horizon edge. New rule **ECON-LT2**: a lead-sweep candidate is confirmed only if the native tournament beats published beyond the ±0.03 guardrail AND the edge is durable across ≥2 adjacent leads. Durable verdict record: `results/_cross_agent/lead_horizon_phase2_weekly_verdicts.md`.
- **Phase 3 — Lead Analysis / Lead Tournament evidence blocks** for all 12 pairs (only `permit_spy` had them before), with the two VIZ-LEAD1 charts on the universal monthly L0..12 axis. Per-pair honesty: the narrative follows each pair's actual data — 1 clean corroboration (`indpro_spy`), 2 reverse-causality nulls (`busloans`, `ism_services`), 1 fragile single-lead spike (`m2sl_yoy`), and several timescale/off-peak divergences — none dressed up beyond what the CSV supports. The 4 daily pairs carry the monthly-resample comparability caveat.
- **Tooling/QA** — fixed a `cloud_verify.py` `NameError` (dangling `exploratory_markers_hit` ref from the Sample-retirement edit) that crashed the cloud sweep on the first served page (latent on `main` since `82164fc`); added **HABIT-QA2** (run `pyflakes` after any harness edit).

**Deployment note (correction):** the preview app that tracks `fix260620_lead_horizon` is **`aig-rlic-plus-dawodev.streamlit.app`**, NOT `aig-rlic-plus-fix260620` (which tracks a different/older branch and served stale content during verification). Streamlit Cloud can **partial-sync** after a push — confirm content sync by enumerating Evidence tab labels, and **reboot** (not refresh) to force a clean full re-sync.
