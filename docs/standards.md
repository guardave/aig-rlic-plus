...
| ECON-FE1 | Final-Exam Confirmation Contract — tournament winners remain discovery-grade until a frozen-rule final exam passes sample separation, target-class thresholds, block-bootstrap uncertainty, multiple-testing adjustment, machine-readable artifact validation, and Quincy replay. | §2.5 Rule FE1 |
...
| ECON-SD | Pair Scope Discipline (Blocking) — every chart, table, and econometric claim on a pair's Story / Evidence / Strategy page must contain only the single named indicator column plus its mathematical derivatives (lags, z-scores, percentiles, momentum, volatility, regime states) AND the single named target column plus its mathematical derivatives (forward returns at any horizon, rolling vol, drawdowns). Non-derivative signals (other indicators like NFCI, Yield Curve, BBB-IG, CCC-BB, Bank ratio) and alternative targets are prohibited even when they correlate. Producer-side (Evan) validates every saved signal name against `results/{pair_id}/signal_scope.json`; QA (Quincy) blocks per GATE-31 on any off-scope signal. Save violations exit 1 with META-ELI5 error. Cross-ref META-AL, META-ELI5, META-CF, ECON-UD, ECON-AS. Added 2026-04-19 (Wave 7A) after HY-IG v2 stakeholder-caught heatmap scope leak. | §ECON-SD |
| ECON-UD | Universe Disclosure (Blocking on reference pairs) — every pair's Methodology page must include a "Signal Universe" section rendered from `results/{pair_id}/signal_scope.json` with two tables (indicator derivatives, target derivatives). Each row: `name | definition | formula/source | role | appears_in_charts`. Purpose: stakeholder can cross-reference any signal on Evidence back to the complete permitted universe — no hidden filtering. Blocking on reference pairs (per META-RPD); strongly recommended for non-reference pairs. Cross-ref ECON-SD, META-CF, META-ELI5, APP-CC1, APP-EX1, RES-17. Added 2026-04-19 (Wave 7A). | §ECON-UD |
| ECON-AS | Analyst Suggestions (Informational, cross-agent) — any agent (dana / evan / vera / ray / ace / quincy) who notices an off-scope signal that might be interesting during pair work may file an entry in `results/{pair_id}/analyst_suggestions.json`. Per-entry fields (all mandatory, NO lifecycle fields): `signal_name, proposed_by, source, observation, rationale, possible_use_case, caveats, date_filed`. Explicit non-rules: NO `status` field, NO automated trigger, NO workflow lifecycle. Rendered read-only on Methodology under "Analyst Suggestions for Future Work" with top caption "Suggestions are informational. If any warrant follow-up work, please request explicitly to the team." User workflow: read → request verbally/email → triggers regular wave. Cross-ref ECON-SD, META-BL (suggestion ≠ backlog), META-CF, META-ELI5. Added 2026-04-19 (Wave 7A). | §ECON-AS |
...
| QA-CL1 | Standard QA Checklist per Wave — 12-item mandatory list: every claim has verification command + result, all schemas validate, both smoke tests pass, Cloud pages render, zero `chart_pending` on reference pair, direction triangulation passes, stakeholder-spirit checks, META-XVC drift = 0, META-ELI5 compliance, GATE-30 deflection audit, every discrepancy evidenced. Extended Wave 8A (2026-04-20) to item 13 (QA-CL2 Semantic KPI Triangulation). | Standard QA Checklist per Wave |
| QA-CL2 | Semantic KPI Triangulation (Blocking) — for every reference-pair Strategy/Evidence page displaying numeric KPIs, Quincy verifies three plausibility invariants hold on the *displayed* values (not the schema instance): (1) Sharpe ↔ return ↔ vol — `Sharpe ≈ (ann_return − risk_free) / ann_vol` implies a vol in 5–40% range for equity; (2) MDD ↔ vol — `|MDD|` typically 2–4× ann_vol; ratio outside [1, 6] suspicious; (3) turnover ↔ trade count ↔ horizon — `n_trades / years ≈ annual_turnover × 2` within 2×. Action on FAIL: record the displayed values, the violated invariant, and the implied contradiction; treat as BLOCKING per GATE-31 (unit-drift, data bug, or rendering bug). Companion to META-UC — catches surviving drift the producer-side inventory missed. Added 2026-04-20 (Wave 8A) in response to Wave 4D-1 percent→ratio migration whose display-side drift shipped undetected until stakeholder caught "+0.1%" on the HY-IG v2 Strategy KPI card. Cross-ref META-UC, GATE-31, META-SRV, APP-DIR1. | QA-CL2 — Semantic KPI Triangulation |
| GATE-ES1 | Evidence-Status Promotion Verification — Quincy blocks any promotion above `found_in_search` unless evidence_status and final_exam_results artifacts validate, confirmation evidence is reproducible, anti-gaming checks pass, and landing/Strategy DOM copy matches the status. | GATE-ES1 |
...
| META-UC | Unit-Coherence After Schema Migration (Blocking) — when a META-CF schema revision changes the unit form, enum values, or semantic interpretation of any numeric or enum field (e.g., percent → ratio, short-code → registry-code, enum rename, sign-convention flip, direction-semantics shift), the migration's regression_note entry MUST include a `### Unit-Coherence propagation` section enumerating: (1) affected fields with before/after unit; (2) consumer inventory from grep across `app/`, `scripts/`, `output/`, `docs/`, `results/` including code reads, formatters/computations, fallback defaults, AND text labels/captions that name the old unit/enum in prose; (3) per-consumer status — `updated` (file:line diff cited), `no_change_needed` (rationale), or `deferred_with_reason` (backlog ID required). Acceptance blocked until every consumer is accounted for; silent-unaccounted consumers = BLOCK per GATE-31. Does NOT apply to purely additive schema bumps or documentation-only changes. Companion to META-SRV (producer self-report) and META-CF (migration trigger); enforced by QA-CL2 (semantic triangulation catches surviving drift). Added 2026-04-20 (Wave 8A) after Wave 4D-1 percent→ratio migration of `winner_summary.oos_ann_return` shipped without enumerating 4 Strategy-page format-strings, producing "+0.1%" instead of "+11.3%" KPI bug caught by stakeholder in Wave 8. Cross-ref META-SRV, META-CF, META-VNC, GATE-31, QA-CL2. | Unit-Coherence After Schema Migration (META-UC) |
| META-AM | Agent Memory Discipline (Blocking on Wave Closure) — at the end of every wave closure or major dispatch that meaningfully touches an agent's domain, the agent MUST append entries to all three of: (1) `~/.claude/agents/<role>-<name>/experience.md` (cross-project, timeless patterns), (2) `~/.claude/agents/<role>-<name>/memories.md` (specific incidents + dated lessons), (3) `_pws/<role>-<name>/session-notes.md` (project-specific session log). Required content per append: What I did (commit SHA / wave ID), What I learned (rules internalized, gotchas, patterns), What I'd tell my next instance (actionable wisdom). META-SRV evidence format applies — each update claim cites file path + `wc -l` / `git diff` verification. Enforcement: future QA-CL3 checklist item — first occurrence PASS-with-note, subsequent FAIL. Applies to all 6 agent roles (Dana, Evan, Vera, Ray, Ace, Quincy) and Lead. Added 2026-04-20 (Wave 9A) after audit revealed 5 of 6 agents had not updated experience.md or memories.md since before Wave 1 despite 8 waves of cumulative wisdom (SOPs absorbed the rules; agent profiles did not, making SOD hollow on any fresh dispatch). Cross-ref META-SRV (self-report format), META-VNC (memory as cross-session continuity), CLAUDE.md Agent Lifecycle § EOD (operationalizes at finer granularity). | Agent Memory Discipline (META-AM) |
...
- **META-UC, QA-CL2** — added 2026-04-20 (Wave 8A) in response to the Wave 4D-1 percent→ratio migration display bug caught by stakeholder. META-UC makes consumer-inventory propagation mandatory on every unit/enum/semantic schema migration (producer-side blocking). QA-CL2 extends the QA checklist to item 13 with three semantic plausibility triangulations (Sharpe-return-vol, MDD-vol, turnover-trade-count) that catch surviving unit drift at the display layer (QA-side blocking via GATE-31).
- **META-AM** — added 2026-04-20 (Wave 9A) after audit revealed 5 of 6 agents had not updated `~/.claude/agents/<role>/experience.md` or `memories.md` since before Wave 1 despite 8 waves of cumulative wisdom. Makes agent memory updates blocking at wave closure with META-SRV evidence format. Future QA-CL3 checklist item will mechanize enforcement (first occurrence PASS-with-note, subsequent FAIL). Applies to all 6 agent roles and Lead. Cross-ref META-SRV, META-VNC, CLAUDE.md Agent Lifecycle § EOD.
