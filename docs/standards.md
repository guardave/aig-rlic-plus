...
- **META-AM** — added 2026-04-20 (Wave 9A) after audit revealed 5 of 6 agents had not updated `~/.agents/profiles/<role>/experience.md` or `memories.md` since before Wave 1 despite 8 waves of cumulative wisdom. Makes agent memory updates blocking at wave closure with META-SRV evidence format. Future QA-CL3 checklist item will mechanize enforcement (first occurrence PASS-with-note, subsequent FAIL). Applies to all 6 agent roles and Lead. Cross-ref META-SRV, META-VNC, CLAUDE.md Agent Lifecycle § EOD.
