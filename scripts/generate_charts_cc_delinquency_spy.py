...
direction (procyclical, L4q) CONTRADICTS the countercyclical credit-stress
prior. Charts label numbers as search-phase / descriptive, never as validated
edge. Every number traces to results/cc_delinquency_spy/*.
"""
...
C_IND = "#D55E00"     # indicator (credit-card delinquency)
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_cc_delinquency_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="%{x|%Y-Q%q}<br>Delinquency: %{y:.2f}%<extra></extra>",
        ),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    nber_legend(fig)
    fig.update_layout(
        title="Credit-Card Delinquency Rate vs SPY",
    ...
    fig.update_yaxes(title_text="Delinquency rate (% of balances)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save(
        "hero",
        fig,
        "Credit-card delinquency rate and SPY with NBER recessions; delinquency "
        "climbed through the 2001 and 2008 downturns and again after 2022 — a "
        "lagging credit-stress gauge.",
...
def chart_regime_stats() -> None:
    ...
    labels = [
        "Q1<br>Lowest delinquency",
        "Q2",
        "Q3",
        "Q4<br>Highest delinquency",
    ]
    ...
    save(
        "regime_stats",
        fig,
        "Concurrent SPY performance across credit-card delinquency-level "
        "quartiles; the highest-delinquency quartile (Q4) has the weakest "
        "Sharpe (0.50) vs 1.23 in the lowest (Q1) — consistent with the "
        "countercyclical credit prior.",
...
def chart_correlation_heatmap() -> None:
    ...
    fig.update_layout(
        title="Correlation: Delinquency Signals vs Forward SPY Returns",
        template="plotly_white",
        height=520,
    )
    save(
        "correlation_heatmap",
        fig,
        "Pearson correlations between delinquency transforms and forward SPY "
        "returns at 1-, 2-, and 4-quarter horizons; associations are weak "
        "(|r| < 0.10) — level cells mildly negative, change cells mildly "
...
def chart_ccf() -> None:
    ...
    fig.update_layout(
        title="Cross-Correlation: Delinquency vs SPY Return",
    ...
    save(
        "ccf_prewhitened",
        fig,
        "Pre-whitened cross-correlation between credit-card delinquency and SPY "
        "returns; the only bars breaching the band are at negative offsets "
        "(-2, -3, -4), a mild inverse echo consistent with the countercyclical "
        "prior — not a clean predictive lead.",
...
def chart_granger() -> None:
    ...
    fig.update_layout(
        title="Granger Causality: Does Delinquency Help Forecast SPY?",
        xaxis_title="Lag (quarters)",
        yaxis_title="p-value",
        template="plotly_white",
        height=430,
    ...
    save(
        "granger_f_by_lag",
        fig,
        "Granger p-values by quarterly lag for credit-card delinquency "
        "predicting SPY returns; every lag is insignificant (min p 0.33) — "
        "delinquency does not Granger-cause SPY at the tested lags.",
...
def chart_local_projections() -> None:
    ...
    save(
        "local_projections",
        fig,
        "Estimated SPY response to a rise in the 4-quarter delinquency change; "
...
def chart_quantile() -> None:
    ...
    save(
        "quantile_coef",
        fig,
        "Delinquency coefficient across SPY forward-return quantiles; flat and "
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(
        title="Equity Curve: Delinquency Strategy vs SPY",
    ...
    save(
        ...
        "candidate; full-sample path, OOS from 2017-09).",
    ...
    save(
        "drawdown",
        fig2,
        "Drawdown comparison; the shallower drawdown (OOS -4.9% vs -23.9%) is "
        "the strategy's one defensible virtue — read the Sharpe as volatility "
        "avoidance, not forecasting (OOS return 13.4% is below B&H 14.3%).",
        [f"results/{PAIR}/strategy_returns_{DATE_TAG}.csv"],
    )
...
def chart_tournament() -> None:
    ...
    fig.add_hline(y=BH_SHARPE, line_dash="dash", line_color=C_BENCH, annotation_text="Buy-and-hold 0.87")
    fig.update_layout(
    ...
    save(
        ...
        "the median valid combo (0.63) UNDERPERFORMS buy-and-hold (0.87) — the "
    ...
    save(
        "tournament_scatter",
        fig,
        "Distribution of OOS Sharpe across valid tournament strategies by lead; "
        "the winner (1.37) is the grid maximum, median valid 0.63 below "
        "buy-and-hold 0.87.",
        [f"results/{PAIR}/tournament_results_{DATE_TAG}.csv"],
    )
...
def chart_rolling_correlation() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(
        title="Rolling Correlation: Delinquency vs SPY",
        yaxis_title="Correlation",
        template="plotly_white",
        height=430,
    )
    save(
        "rolling_correlation",
        fig,
        "Rolling correlation between credit-card delinquency and SPY returns; "
        "the sign swings through time, so the relationship needs ongoing "
        "monitoring.",
...
def chart_structural_break() -> None:
    ...
    save(
        "structural_break",
        fig,
        "Proxy check for large changes in the delinquency-SPY relationship "
        "(rolling-correlation z-score); max |z| = 2.95.",
...
def chart_subperiod() -> None:
    ...
    save(
        ...
        "episodes; several pre-OOS episodes precede the strategy window, so the "
        "durability read rests on a handful of cycles.",
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(
        go.Scatter(x=df.index, y=df["ccdelinq"], name="Delinquency rate (%)", line=dict(color=C_IND)),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    fig.update_yaxes(title_text="Delinquency rate (%)", secondary_y=False)
...
def main() -> None:
    ...
    chart_subperiod()
    chart_history_zoom("dotcom", "Dot-Com Recession: Delinquency Drifted Higher Through the Downturn", "1999-03-31", "2003-12-31")
    chart_history_zoom("gfc", "Global Financial Crisis: Delinquency Spiked to Sample Highs", "2006-03-31", "2010-12-31")
    chart_history_zoom("covid", "COVID Shock: Support Programs Kept Delinquency Subdued", "2018-03-31", "2021-12-31")
    chart_history_zoom("rate_hike_2022", "2022-24 Delinquency Upturn: Post-Pandemic Normalization", "2021-03-31", "2025-06-30")
