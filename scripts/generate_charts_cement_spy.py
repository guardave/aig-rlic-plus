#!/usr/bin/env python3
"""Chart generation for Portland Cement Shipments x SPY.
...
def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "cement_spy_monthly_latest.parquet")
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_cement_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index,
            y=df["cement"],
            name="Portland cement shipments",
            line=dict(color=C_IND, width=2),
            hovertemplate="%{x|%Y-%m}<br>Cement shipments: %{y:,.0f}<extra></extra>",
        ),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    nber_legend(fig)
    fig.update_layout(
        title="Portland Cement Shipments vs SPY",
    ...
    fig.update_yaxes(title_text="Cement shipments (nominal $, SA)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save("hero", fig, "Portland cement shipment level and SPY with NBER recession bands.", ["data/cement_spy_monthly_latest.parquet"])
...
def chart_regime_stats() -> None:
    ...
    fig = make_dual_panel_regime_chart(
        ...
        signal_label="Cement growth",
        x_axis_title="Cement YoY growth quartile",
        axis_noun="",
        sample_sizes=q["n"].astype(int).tolist(),
    )
    fig.update_layout(title="SPY Returns by Cement-Growth Regime")
    save("regime_stats", fig, "SPY performance across cement YoY growth quartiles.", ["results/cement_spy/regime_quartile_returns.csv"])
...
def chart_correlation_heatmap() -> None:
    ...
    fig.update_layout(title="Correlation: Cement Signals vs Forward SPY Returns", template="plotly_white", height=520)
    save("correlation_heatmap", fig, "Pearson correlations between cement transforms and forward SPY returns.", [str(CORE / "correlations.csv")])
...
def chart_ccf() -> None:
    ...
    fig.update_layout(title="Cross-Correlation: Cement Growth vs SPY Return", xaxis_title="Lag (months): +lead = cement leads, -lag = SPY leads", yaxis_title="Correlation", template="plotly_white", height=430)
    save("ccf_prewhitened", fig, "Pre-whitened cross-correlation between cement growth and SPY returns.", [str(CORE / "ccf_prewhitened.csv")])
...
def chart_granger() -> None:
    ...
    fig.update_layout(title="Granger Causality: Does Cement Growth Help Forecast SPY?", xaxis_title="Lag (months)", yaxis_title="p-value", template="plotly_white", height=430)
    save("granger_f_by_lag", fig, "Granger p-values by lag for cement growth predicting SPY returns.", [str(CORE / "granger_causality.csv")])
...
def chart_local_projections() -> None:
    ...
    fig.update_layout(title="Local Projection: SPY Response to Cement Growth", xaxis_title="Forward horizon", yaxis_title="Coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("local_projections", fig, "Estimated SPY response across horizons to a move in cement growth.", [str(CORE / "local_projections.csv")])
...
def chart_quantile() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Quantile Regression Coefficient", xaxis_title="SPY return quantile", yaxis_title="Cement coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("quantile_coef", fig, "Cement-growth coefficient across SPY forward-return quantiles.", [str(CORE / "quantile_regression.csv")])
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(title="Equity Curve: Cement Strategy vs SPY", yaxis_title="Growth of $1", template="plotly_white", height=460)
...
def chart_structural_break() -> None:
    ...
    save("structural_break", fig, "Proxy check for large changes in the cement/SPY relationship.", [str(RES / f"structural_break_{PAIR}.json")])
...
def chart_subperiod() -> None:
    ...
    save("subperiod_sharpe", fig, "Strategy and buy-and-hold SPY Sharpe during major stress episodes.", ["results/cement_spy/subperiod_sharpe.csv"])
    save("walk_forward", fig, "Alias for the dashboard's standard walk-forward chart slot.", ["results/cement_spy/subperiod_sharpe.csv"])
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["cement_yoy"], name="Cement YoY (%)", line=dict(color=C_IND)), secondary_y=False)
    ...
    fig.update_yaxes(title_text="Cement YoY (%)", secondary_y=False)
    fig.update_yaxes(title_text="SPY", secondary_y=True)
    save(f"history_zoom_{slug}", fig, f"{title}; gray bands are NBER recessions.", ["data/cement_spy_monthly_latest.parquet"])
