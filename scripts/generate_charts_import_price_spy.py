...
def chart_local_projections() -> None:
    ...
    fig.update_layout(title="Local Projection: SPY Response to Import-Price Growth", xaxis_title="Forward horizon", yaxis_title="Coefficient", template="plotly_white", height=430)
    save("local_projections", fig, "Estimated SPY response across horizons to a move in import-price growth.", [str(CORE / "local_projections.csv")])
...
def chart_quantile() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Quantile Regression Coefficient", xaxis_title="SPY return quantile", yaxis_title="Import-price coefficient", template="plotly_white", height=430)
