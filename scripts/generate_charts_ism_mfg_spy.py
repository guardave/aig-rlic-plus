#!/usr/bin/env python3
"""Chart generation for ISM Manufacturing PMI x SPY.
...
def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "ism_mfg_spy_monthly_latest.parquet")
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_ism_mfg_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_hline(
        ...
        line_dash="dot",
        line_color=C_LINE,
        annotation_text="PMI 50 (expansion / contraction)",
        annotation_position="top left",
        secondary_y=False,
    )
    nber_legend(fig)
    fig.update_layout(
        title="ISM Manufacturing PMI vs SPY",
    ...
    fig.update_yaxes(title_text="ISM Manufacturing PMI (diffusion index)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save("hero", fig, "ISM Manufacturing PMI level (with the 50 line) and SPY with NBER recession bands.", ["data/ism_mfg_spy_monthly_latest.parquet"])


def chart_regime_stats() -> None:
    q = pd.read_csv(RES / "regime_quartile_returns.csv")
    labels = ["Q1<br>Weak PMI", "Q2", "Q3", "Q4<br>Strong PMI"]
    fig = make_dual_panel_regime_chart(
        labels,
        q["sharpe"].astype(float).tolist(),
        (q["mean_return"].astype(float) * 12 * 100).tolist(),
        signal_label="PMI level",
        x_axis_title="ISM Manufacturing PMI quartile",
        axis_noun="",
        sample_sizes=q["n"].astype(int).tolist(),
    )
    fig.update_layout(title="SPY Returns by ISM Manufacturing PMI Regime")
    save("regime_stats", fig, "SPY performance across ISM Manufacturing PMI quartiles.", ["results/ism_mfg_spy/regime_quartile_returns.csv"])
...
def chart_correlation_heatmap() -> None:
    ...
    fig.update_layout(title="Correlation: ISM PMI Signals vs Forward SPY Returns", template="plotly_white", height=520)
    save("correlation_heatmap", fig, "Pearson correlations between ISM PMI transforms and forward SPY returns.", [str(CORE / "correlations.csv")])
...
def chart_ccf() -> None:
    ...
    fig.update_layout(title="Cross-Correlation: ISM PMI Change vs SPY Return", xaxis_title="Lag (months): +lead = PMI leads, -lag = SPY leads", yaxis_title="Correlation", template="plotly_white", height=430)
    save("ccf_prewhitened", fig, "Pre-whitened cross-correlation between the ISM PMI change and SPY returns.", [str(CORE / "ccf_prewhitened.csv")])
...
def chart_granger() -> None:
    ...
    fig.update_layout(title="Granger Causality: Does the ISM PMI Help Forecast SPY?", xaxis_title="Lag (months)", yaxis_title="p-value", template="plotly_white", height=430)
    save("granger_f_by_lag", fig, "Granger p-values by lag for the ISM PMI change predicting SPY returns.", [str(CORE / "granger_causality.csv")])
...
def chart_local_projections() -> None:
    ...
    fig.update_layout(title="Local Projection: SPY Response to the ISM PMI", xaxis_title="Forward horizon", yaxis_title="Coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("local_projections", fig, "Estimated SPY response across horizons to a move in the ISM PMI.", [str(CORE / "local_projections.csv")])
...
def chart_quantile() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Quantile Regression Coefficient", xaxis_title="SPY return quantile", yaxis_title="ISM PMI coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("quantile_coef", fig, "ISM PMI coefficient across SPY forward-return quantiles.", [str(CORE / "quantile_regression.csv")])
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(title="Equity Curve: ISM PMI Strategy vs SPY", yaxis_title="Growth of $1", template="plotly_white", height=460)
...
def chart_rolling_correlation() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Rolling Correlation: ISM PMI Change vs SPY", yaxis_title="Correlation", template="plotly_white", height=430)
    save("rolling_correlation", fig, "Rolling correlation between the ISM PMI change and SPY returns.", [str(RES / f"rolling_correlation_{PAIR}.csv")])
...
def chart_structural_break() -> None:
    ...
    save("structural_break", fig, "Proxy check for large changes in the ISM PMI/SPY relationship.", [str(RES / f"structural_break_{PAIR}.json")])
...
def chart_subperiod() -> None:
    ...
    save("subperiod_sharpe", fig, "Strategy and buy-and-hold SPY Sharpe during major stress episodes.", ["results/ism_mfg_spy/subperiod_sharpe.csv"])
    save("walk_forward", fig, "Alias for the dashboard's standard walk-forward chart slot.", ["results/ism_mfg_spy/subperiod_sharpe.csv"])
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["ism"], name="ISM Manufacturing PMI", line=dict(color=C_IND)), secondary_y=False)
    fig.add_trace(go.Scatter(x=df.index, y=df["spy"], name="SPY", line=dict(color=C_TGT)), secondary_y=True)
    fig.add_hline(y=50, line_dash="dot", line_color=C_LINE, secondary_y=False)
    nber_legend(fig)
    fig.update_layout(title=title, template="plotly_white", height=430, hovermode="x unified")
    fig.update_yaxes(title_text="ISM Manufacturing PMI", secondary_y=False)
    fig.update_yaxes(title_text="SPY", secondary_y=True)
    save(f"history_zoom_{slug}", fig, f"{title}; gray bands are NBER recessions, dotted line is PMI 50.", ["data/ism_mfg_spy_monthly_latest.parquet"])
...
def main() -> None:
    ...
    chart_subperiod()
    chart_history_zoom("dotcom", "Dot-Com Recession: PMI Fell Below 50 with the Capex Bust", "1999-01-31", "2003-12-31")
    chart_history_zoom("gfc", "Global Financial Crisis: PMI Collapsed into the Low 30s", "2006-01-31", "2010-12-31")
    chart_history_zoom("covid", "COVID Shock: PMI Plunge and Violent Rebound", "2018-01-31", "2021-12-31")
    chart_history_zoom("inflation_2022", "2022 Rate Shock: PMI Ground Lower toward 50", "2021-01-31", "2025-10-31")
