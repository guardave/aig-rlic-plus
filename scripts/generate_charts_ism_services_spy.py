...
Framing (binding, per Lead dispatch + Evan handoff): ISM Services PMI behaves
as a coincident/lagging survey in this sample. Charts serve the honest story:
reverse Granger dominance is visually primary, the 50 diffusion-index line is
prominent, and the searched contrarian winner is framed as fragile drawdown
management rather than validated alpha.

Charts produced (output/charts/ism_services_spy/plotly/, bare names per VIZ-A3):
...
Author: Viz Vera. Date: 2026-06-18.
"""
...
# ism_services_spy entry yet — gap flagged to Ace in the handoff note; these
# constants are the proposed canonical forms.
IND_LONG = "ISM Services PMI"
IND_SHORT = "ISM Services PMI"
TGT = "SPY"
...
GENERATED_BY = "Viz Vera — scripts/generate_charts_ism_services_spy.py (mode3_ism_services_spy)"


# ── Loaders ───────────────────────────────────────────────────────────────

def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "ism_services_spy_monthly_latest.parquet")
...
SRC_DATA = "data/ism_services_spy_monthly_latest.parquet"
...
def chart_hero():
    ...
    gap = df["ism_services_gap_50"]
    roll_mean = gap.rolling(120, min_periods=36).mean()
    roll_std = gap.rolling(120, min_periods=36).std()
    threshold = roll_mean - roll_std
    signal_dates = gap[gap < threshold].dropna().index

    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(
        x=pmi.index, y=pmi.values,
        name=f"{IND_SHORT} diffusion index",
        line=dict(color=C_IND, width=2)), secondary_y=False)
    fig.add_trace(go.Scatter(
        x=signal_dates, y=pmi.reindex(signal_dates),
        name="Contrarian buy signal: PMI gap z-score < -1",
        mode="markers", marker=dict(color=C_POS, size=7, symbol="triangle-up")),
        secondary_y=False)
    fig.add_trace(go.Scatter(
        x=df.index, y=df["spy"],
    ...
    nber_swatch(fig)
    fig.add_hline(y=50, line_dash="dash", line_color=C_EVENT, line_width=1.4,
                  annotation_text="50 = expansion/contraction line",
                  secondary_y=False)
    fig.add_annotation(x="2020-04-30", y=float(pmi.loc["2020-04-30"]),
                       text="COVID shock: PMI collapsed; searched rule buys weakness<br>after a 3-month lag, not because PMI leads SPY",
    ...
    fig.update_layout(
        title=(f"{IND_LONG} vs {TGT}: The Winner Buys Weak Services Sentiment, "
               f"but PMI Does Not Lead ({y0}–{y1})"),
        template="plotly_white", hovermode="x unified", height=500,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_yaxes(title_text=f"{IND_SHORT} diffusion index (50 = neutral)", secondary_y=False)
    fig.update_yaxes(title_text=f"{TGT} price (USD)", secondary_y=True)
    save_chart("hero", fig,
               caption=(f"{IND_LONG} diffusion index (vermillion, left axis) against "
                        f"{TGT} price (blue, right axis), {y0}–{y1}. The 50 line marks "
                        "services expansion versus contraction; green markers show "
                        "months when the winner's rolling z-score weakness trigger "
                        "fires. Shaded bands mark NBER recessions."),
               alignment=("Hero makes the countercyclical rule visually legible: buy "
                          "when services sentiment is unusually weak, while the "
                          "caption discloses that Granger evidence is reverse-only."),
...
def chart_regime_stats():
    ...
    labels = ["Q1<br>(Lowest PMI)", "Q2", "Q3", "Q4<br>(Highest PMI)"]
    ...
    fig.update_layout(
        title=dict(
            text=(f"The Descriptive Quartiles Are Procyclical: {TGT} Is Weakest "
                  f"When {IND_SHORT} Is Lowest"),
            y=0.99, yanchor="top"),
        margin=dict(t=150))
    best = q.loc[q["sharpe"].idxmax()]
    save_chart("regime_stats", fig,
               caption=(f"SPY performance by concurrent {IND_SHORT} level quartile: "
                        f"Sharpe (left) and annualized return (right). Q1 (lowest PMI) "
                        f"is weakest (Sharpe {q['sharpe'].iloc[0]:.2f}, max drawdown "
                        f"{q['max_drawdown'].iloc[0]*100:.1f}%); Q3/Q4 are stronger "
                        f"(Sharpe {q['sharpe'].iloc[2]:.2f}/{q['sharpe'].iloc[3]:.2f}). "
                        f"This supports the natural procyclical prior, not a stable "
                        f"leading signal."),
               ...
               nber_required=False)
    # Also emit the required returns-by-regime chart for 3-month PMI momentum.
    q3m = pd.read_csv(RES / "regime_quartile_returns_3m_change.csv")
    fig2 = make_dual_panel_regime_chart(
        quartile_labels=["Q1<br>(Weakest 3M change)", "Q2", "Q3", "Q4<br>(Strongest 3M change)"],
        sharpe=q3m["sharpe"].tolist(),
    ...
    fig2.update_layout(
        title=dict(
            text=(f"{TGT} Returns Improve When Services Momentum Is Stronger: "
                  f"{IND_SHORT} 3M Change Quartiles"),
            y=0.99, yanchor="top"),
        margin=dict(t=150))
    save_chart("returns_by_regime", fig2,
               caption=(f"SPY performance by concurrent {IND_SHORT} 3-month-change "
                        f"quartile. Q1 is nearly flat (Sharpe "
                        f"{q3m['sharpe'].iloc[0]:.2f}); Q3/Q4 are materially stronger "
                        f"({q3m['sharpe'].iloc[2]:.2f}/{q3m['sharpe'].iloc[3]:.2f}). "
                        f"Again, the descriptive evidence is procyclical."),
               alignment=("Required quartile/returns-by-regime chart; it distinguishes "
                          "the procyclical descriptive pattern from the contrarian "
                          "searched strategy."),
               rules=["VIZ-QR1", "VIZ-IC1", "VIZ-NS1", "VIZ-O1"],
               sources=[f"results/{PAIR}/regime_quartile_returns_3m_change.csv"],
               nber_required=False)
...
_SIGLBL = {
    "level": "PMI level",
    "gap_50": "PMI minus 50",
    "delta": "1-month PMI change",
    "change_3m": "3-month PMI change",
    "change_6m": "6-month PMI change",
    "zscore_60m": "PMI z-score (60-month)",
    "above_50": "Above-50 flag",
    "hmm_stress": "HMM low-PMI stress probability",
}
...
def chart_correlation_heatmap():
    ...
    save_chart("correlation_heatmap", fig,
               caption=(f"Pearson correlations between {IND_LONG} signals and {TGT} "
                        ...
                        f"({best['pair_name'].replace('__',' vs ')}) — weak evidence "
                        f"relative to the stronger reverse-causality diagnostics."),
               alignment=("Heatmap supports the 'weak forward channel' finding; best "
                          "cell is re-read from correlations.csv at generation time."),
...
def chart_granger():
    ...
    fig.add_trace(go.Bar(
        ...
        name=f"{TGT} → {IND_SHORT} (the market leads PMI) — significant at EVERY lag",
        marker_color=C_IND))
    fig.add_trace(go.Bar(
        x=fwd["lag"], y=fwd["f_statistic"],
        name=f"{IND_SHORT} → {TGT} (PMI leads the market) — significant at NO lag",
        marker_color="#aec7e8"))
    fig.add_trace(go.Scatter(
...
def chart_local_projections():
    ...
    for col, dirn, color in ((1, "fwd", C_IND), (2, "rev", C_TGT)):
        ...
        fig.add_trace(go.Scatter(
            ...
            name=("Coefficient: PMI → market" if dirn == "fwd"
                  else "Coefficient: market → PMI"),
    ...
    fwd_min = lp[lp["direction"] == "fwd"]["p_value"].min()
    rev_max = lp[lp["direction"] == "rev"]["p_value"].max()
    fig.update_layout(
        title=(f"Local Projections (HAC): Reverse Effects Dominate the Forward Effects "
               f"<br><sup>Forward min p = {fwd_min:.2f}; reverse max p = {rev_max:.2f} "
               f"across 1/3/6/12-month horizons</sup>"),
    ...
    save_chart("local_projections", fig,
               caption=(f"Local-projection impulse responses with HAC standard errors "
                        ...
                        f"from zero at every horizon (min p = {fwd_min:.2f}); the "
                        f"reverse panel is significant at every shown horizon "
                        f"(max p = {rev_max:.2f})."),
...
def chart_transfer_entropy():
    ...
    fig.update_layout(
        title=("Transfer Entropy Also Points Backward: Reverse Flow Is Significant, "
               "Forward Flow Is Only Marginal<br><sup>Tercile binning, 500 permutations; "
               "ISM→SPY p = 0.064, SPY→ISM p = 0.002</sup>"),
    ...
    save_chart("transfer_entropy", fig,
               caption=(f"Transfer entropy between {IND_SHORT} and {TGT} returns "
                        f"(tercile bins, 500 permutations). Forward flow is marginal "
                        f"({IND_SHORT} → {TGT} p = "
                        f"{te.iloc[0]['permutation_p_value']:.3f}); reverse flow is "
                        f"significant ({TGT} → {IND_SHORT} p = "
                        f"{te.iloc[1]['permutation_p_value']:.3f})."),
...
def chart_quantile():
    ...
    fig.update_layout(
        title=(f"Quantile Regression Is Mixed: Positive Left Tail, Negative Median/Right Tail "
               f"<br><sup>{IND_SHORT} on {TGT} forward returns; several quantiles are "
               f"significant but signs are not stable</sup>"),
    ...
    save_chart("quantile_coef", fig,
               caption=(f"Quantile-regression coefficient of {TGT} forward returns on "
                        ...
                        f"Lower-tail coefficients are positive around tau 0.10, while "
                        f"median/right-tail coefficients are negative and significant. "
                        f"The sign instability is another fragility warning."),
...
def chart_hmm():
    ...
    gfc = float(h.loc["2008-09-30":"2009-06-30", "prob_stress"].mean())
    assert gfc > 0.95, f"HMM sanity check failed: GFC prob_stress mean {gfc:.2f}"
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=h.index, y=h["prob_stress"], mode="lines",
        name="P(low-PMI stress regime)",
    ...
    fig.update_layout(
        title=(f"Hidden Markov Model (HMM): {IND_SHORT} Growth Spends Most of the "
               f"Sample in the Low-PMI Stress Regime<br><sup>2-state HMM on PMI level; "
               f"stress-state frequency {float(pd.read_csv(CORE / 'hmm_summary.csv').loc[0, 'frequency_pct']):.1f}%</sup>"),
        xaxis_title="Date",
        yaxis_title="Probability of low-PMI stress regime",
        template="plotly_white", height=420,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    save_chart("hmm_regime_probs", fig,
               caption=(f"Probability that {IND_SHORT} is in its low-PMI stress regime "
                        f"(2-state HMM). The stress state has mean PMI "
                        f"{float(pd.read_csv(CORE / 'hmm_summary.csv').loc[0, 'mean_pmi']):.1f} "
                        f"and covers "
                        f"{float(pd.read_csv(CORE / 'hmm_summary.csv').loc[0, 'frequency_pct']):.1f}% "
                        f"of months. Shaded bands mark NBER recessions."),
               alignment=("Labels follow hmm_summary.csv semantics: stress is the "
                          "lower-PMI regime, not proof of an equity-leading state."),
...
def chart_structural_break():
    sb = json.loads((RES / f"structural_break_{PAIR}.json").read_text())
    df = load_monthly()
    sig = df.loc[sb["sample_start"]:sb["sample_end"], "ism_services_pmi"].dropna()
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=sig.index, y=sig.values, mode="lines",
        name=f"{IND_SHORT} diffusion index", line=dict(color=C_IND, width=1.8)))
    fig.add_hline(y=50, line_dash="dash", line_color=C_NEUTRAL, line_width=1,
    ...
    fig.add_annotation(x=sb["break_date"], y=0.95, yref="paper", showarrow=False,
                       ...
                       text=(f"Candidate break {sb['break_date'][:7]}: sup-F "
                             f"{sb['f_stat']:.2f}, bootstrap p = {sb['p_value']:.3f} "
                             f"— flagged"),
    ...
    nber_swatch(fig)
    fig.update_layout(
        title=(f"Structural Break Flagged Around March 2009: {IND_SHORT} Level "
               f"(p = {sb['p_value']:.3f})"),
        xaxis_title="Date",
        yaxis_title=f"{IND_SHORT} diffusion index",
        template="plotly_white", height=420,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    save_chart("structural_break", fig,
               caption=(f"{IND_SHORT} over the test sample "
                        f"({sb['sample_start'][:7]}–{sb['sample_end'][:7]}, n = "
                        f"{sb['n_obs']}) with the Quandt-Andrews candidate break date "
                        f"({sb['break_date'][:7]}). The test flags a break "
                        f"(sup-F {sb['f_stat']:.2f}, residual-bootstrap p = "
                        f"{sb['p_value']:.3f}); cross-period results should be read "
                        f"as regime-dependent rather than stable law. "
                        f"Shaded bands mark NBER recessions."),
               alignment="Annotation-only structural-break chart per ECON-H4; values from JSON.",
...
def chart_history_zoom(slug: str):
    ...
    yoy = w["ism_services_pmi"].dropna()
    spy = w["spy"].dropna()

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06,
                        subplot_titles=[f"{IND_SHORT} diffusion index",
                                        f"{TGT} price (USD)"])
    ...
    fig.update_yaxes(title_text="PMI index (50 = neutral)", row=1, col=1)
    fig.update_yaxes(title_text="Price (USD)", row=2, col=1)
    save_chart(f"history_zoom_{slug}", fig,
               caption=(f"{IND_SHORT} level (top) and {TGT} price (bottom) "
    ...
    for c in ("rolling_sharpe_cp", "rolling_granger"):
        (OUT / f"chart_skip_{c}.json").write_text(json.dumps({
            ...
            "skipped_by": "VIZ-CP1-G",
                       ...
                       "regime_story=false in results/ism_services_spy/signal_scope.json. "
                       "No rolling Sharpe / rolling Granger upstream result file exists "
                       "to chart."),
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "generated_by": GENERATED_BY,
        }, indent=2) + "\n")
