#!/usr/bin/env python3
"""Chart generation for Manufacturers' New Orders x SPY.

Presentational layer only. Every series is read from the committed
results/mfg_new_orders_spy artifacts or the monthly panel; no numbers are
invented. New orders is a nominal-dollar level (non-stationary) so the traded
signals are growth transforms. New orders is a LEADING indicator (a Conference
Board LEI component) and the economic prior is PROCYCLICAL, but the searched
winner is COUNTERCYCLICAL at a 12-month lead -- captions flag that tension.
...
def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "mfg_new_orders_spy_monthly_latest.parquet")
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_mfg_new_orders_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index,
            y=df["mfg_new_orders"],
            name="Manufacturers' new orders ($M)",
            line=dict(color=C_IND, width=2),
            hovertemplate="%{x|%Y-%m}<br>New orders: $%{y:,.0f}M<extra></extra>",
        ),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    nber_legend(fig)
    fig.update_layout(
        title="Manufacturers' New Orders vs SPY",
    ...
    fig.update_yaxes(title_text="New orders ($ millions, SA)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save("hero", fig, "Manufacturers' new-orders level and SPY with NBER recession bands.", ["data/mfg_new_orders_spy_monthly_latest.parquet"])
...
def chart_regime_stats() -> None:
    ...
    fig = make_dual_panel_regime_chart(
        ...
        signal_label="New-orders growth",
        x_axis_title="New-orders YoY growth quartile",
        axis_noun="",
        sample_sizes=q["n"].astype(int).tolist(),
    )
    fig.update_layout(title="SPY Returns by New-Orders Growth Regime")
    save("regime_stats", fig, "SPY performance across new-orders YoY growth quartiles.", ["results/mfg_new_orders_spy/regime_quartile_returns.csv"])
...
def chart_correlation_heatmap() -> None:
    ...
    fig.update_layout(title="Correlation: New-Orders Signals vs Forward SPY Returns", template="plotly_white", height=520)
    save("correlation_heatmap", fig, "Pearson correlations between new-orders transforms and forward SPY returns.", [str(CORE / "correlations.csv")])
...
def chart_ccf() -> None:
    ...
    fig.update_layout(title="Cross-Correlation: New-Orders Growth vs SPY Return", xaxis_title="Lag (months): +lead = new orders leads, -lag = SPY leads", yaxis_title="Correlation", template="plotly_white", height=430)
    save("ccf_prewhitened", fig, "Pre-whitened cross-correlation between new-orders growth and SPY returns.", [str(CORE / "ccf_prewhitened.csv")])
...
def chart_granger() -> None:
    ...
    fig.update_layout(title="Granger Causality: Does New-Orders Growth Help Forecast SPY?", xaxis_title="Lag (months)", yaxis_title="p-value", template="plotly_white", height=430)
    save("granger_f_by_lag", fig, "Granger p-values by lag for new-orders growth predicting SPY returns.", [str(CORE / "granger_causality.csv")])
...
def chart_local_projections() -> None:
    ...
    fig.update_layout(title="Local Projection: SPY Response to New-Orders Growth", xaxis_title="Forward horizon", yaxis_title="Coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("local_projections", fig, "Estimated SPY response across horizons to a move in new-orders growth.", [str(CORE / "local_projections.csv")])
...
def chart_quantile() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Quantile Regression Coefficient", xaxis_title="SPY return quantile", yaxis_title="New-orders coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("quantile_coef", fig, "New-orders-growth coefficient across SPY forward-return quantiles.", [str(CORE / "quantile_regression.csv")])
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(title="Equity Curve: New-Orders Strategy vs SPY", yaxis_title="Growth of $1", template="plotly_white", height=460)
...
def chart_rolling_correlation() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Rolling Correlation: New-Orders Growth vs SPY", yaxis_title="Correlation", template="plotly_white", height=430)
    save("rolling_correlation", fig, "Rolling correlation between new-orders growth and SPY returns.", [str(RES / f"rolling_correlation_{PAIR}.csv")])
...
def chart_structural_break() -> None:
    ...
    save("structural_break", fig, "Proxy check for large changes in the new-orders/SPY relationship.", [str(RES / f"structural_break_{PAIR}.json")])
...
def chart_subperiod() -> None:
    ...
    save("subperiod_sharpe", fig, "Strategy and buy-and-hold SPY Sharpe during major stress episodes.", ["results/mfg_new_orders_spy/subperiod_sharpe.csv"])
    save("walk_forward", fig, "Alias for the dashboard's standard walk-forward chart slot.", ["results/mfg_new_orders_spy/subperiod_sharpe.csv"])
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["mfg_new_orders_yoy"], name="New orders YoY (%)", line=dict(color=C_IND)), secondary_y=False)
    ...
    fig.update_yaxes(title_text="New orders YoY (%)", secondary_y=False)
    fig.update_yaxes(title_text="SPY", secondary_y=True)
    save(f"history_zoom_{slug}", fig, f"{title}; gray bands are NBER recessions.", ["data/mfg_new_orders_spy_monthly_latest.parquet"])
...
def main() -> None:
    ...
    chart_subperiod()
    chart_history_zoom("dotcom", "Dot-Com Recession: New Orders Fell with the Capex Bust", "1999-01-31", "2003-12-31")
    chart_history_zoom("gfc", "Global Financial Crisis: New Orders Collapsed 2008-09", "2006-01-31", "2010-12-31")
    chart_history_zoom("covid", "COVID Shock: New-Orders Collapse and Violent Rebound", "2018-01-31", "2021-12-31")
    chart_history_zoom("inflation_2022", "2022 Rate Shock: Nominal Orders Stayed High on Inflation", "2021-01-31", "2025-08-31")
