#!/usr/bin/env python3
"""Chart generation for Building Permits YoY x SPY.

Presentational layer only. Every series is read from the committed
results/permit_yoy_spy artifacts or the monthly panel; no numbers are invented.
Building Permits YoY is a zero-centred GROWTH RATE (> 0 = permits higher than a
year ago, < 0 = contracting) -- NOT a diffusion index, so no ">50 = expansion"
language. Permits are a classic LEADING indicator (a Conference Board LEI
component) and the economic prior is PROCYCLICAL; the searched winner is
PROCYCLICAL at a 9-month lead, direction-consistent with that prior -- but the
formal lead-lag tests find no forecast, so captions frame it as found-in-search
drawdown control, not a predictor. Only NBER recession shading is used.
"""
...
def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "permit_yoy_spy_monthly_latest.parquet")
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_permit_yoy_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="%{x|%Y-%m}<br>Permits YoY: %{y:.1f}%<extra></extra>",
        ),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    nber_legend(fig)
    fig.update_layout(
        title="Building Permits YoY vs SPY",
    ...
    fig.update_yaxes(title_text="Building permits YoY (%, zero-centred)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save("hero", fig, "Building-permit YoY growth and SPY with NBER recession bands.", ["data/permit_yoy_spy_monthly_latest.parquet"])
...
def chart_correlation_heatmap() -> None:
    ...
    order = [f"{h}m" for h in (1, 3, 6) if f"{h}m" in piv.columns]
    piv = piv[order]
    fig = go.Figure(
        go.Heatmap(
            z=piv.values,
    ...
    fig.update_layout(title="Correlation: Permit-Growth Signals vs Forward SPY Returns", template="plotly_white", height=520)
    save("correlation_heatmap", fig, "Pearson correlations between permit-growth transforms and forward SPY returns.", [str(CORE / "correlations.csv")])
...
def chart_ccf() -> None:
    ...
    fig.update_layout(title="Cross-Correlation: Permit Growth vs SPY Return", xaxis_title="Lag (months): +lead = permits lead, -lag = SPY leads", yaxis_title="Correlation", template="plotly_white", height=430)
    save("ccf_prewhitened", fig, "Pre-whitened cross-correlation between permit growth and SPY returns.", [str(CORE / "ccf_prewhitened.csv")])
...
def chart_granger() -> None:
    ...
    fig.update_layout(title="Granger Causality: Does Permit Growth Help Forecast SPY?", xaxis_title="Lag (months)", yaxis_title="p-value", template="plotly_white", height=430)
    save("granger_f_by_lag", fig, "Granger p-values by lag for permit growth predicting SPY returns.", [str(CORE / "granger_causality.csv")])
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(title="Equity Curve: Permit-Growth Strategy vs SPY", yaxis_title="Growth of $1", template="plotly_white", height=460)
...
def chart_rolling_correlation() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Rolling Correlation: Permit Growth vs SPY", yaxis_title="Correlation", template="plotly_white", height=430)
    save("rolling_correlation", fig, "Rolling correlation between permit growth and SPY returns.", [str(RES / f"rolling_correlation_{PAIR}.csv")])
...
def chart_structural_break() -> None:
    ...
    save("structural_break", fig, "Proxy check for large changes in the permit/SPY relationship.", [str(RES / f"structural_break_{PAIR}.json")])
...
def chart_subperiod() -> None:
    ...
    save("subperiod_sharpe", fig, "Strategy and buy-and-hold SPY Sharpe during major stress episodes.", ["results/permit_yoy_spy/subperiod_sharpe.csv"])
    save("walk_forward", fig, "Alias for the dashboard's standard walk-forward chart slot.", ["results/permit_yoy_spy/subperiod_sharpe.csv"])
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["permit_yoy"], name="Permits YoY (%)", line=dict(color=C_IND)), secondary_y=False)
    ...
    fig.update_yaxes(title_text="Permits YoY (%)", secondary_y=False)
    fig.update_yaxes(title_text="SPY", secondary_y=True)
    save(f"history_zoom_{slug}", fig, f"{title}; gray bands are NBER recessions.", ["data/permit_yoy_spy_monthly_latest.parquet"])
...
def main() -> None:
    ...
    chart_subperiod()
    chart_history_zoom("dotcom", "Dot-Com Recession: Permit Growth Softened into the Downturn", "1999-01-31", "2003-12-31")
    chart_history_zoom("gfc", "Global Financial Crisis: Permit Growth Collapsed 2007-09", "2005-01-31", "2010-12-31")
    chart_history_zoom("covid", "COVID Shock: Permit Collapse and Rapid Rebound", "2018-01-31", "2021-12-31")
    chart_history_zoom("rate_hike_2022", "2022 Rate Shock: Higher Rates Cut Permit Growth", "2021-01-31", "2025-08-31")
