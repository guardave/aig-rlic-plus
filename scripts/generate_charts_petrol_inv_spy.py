#!/usr/bin/env python3
"""Chart generation: Total Petroleum Stocks (PETROL_INV) x SPY — Mode 3.

Vera-owned producer for the petrol_inv_spy standard chart set (20260617).
Consumes Evan's ECON-H4 handoff artifacts (results/petrol_inv_spy/) and Dana's
monthly dataset and Evan's strategy_returns_20260617.csv. Strategy charts use
Evan's saved return series; they do not re-run the strategy selection logic.

Framing (binding, per Lead dispatch + Evan handoff): Total petroleum stock
growth is a fragile procyclical signal. The naive prior is counter-cyclical
(inventories can build when demand weakens), but quartiles and the tournament
winner point procyclical. Lead timing is imprecise: Granger support clusters
at 6-8 months while the selected rule uses L12.

Charts produced (output/charts/petrol_inv_spy/plotly/, bare names per VIZ-A3):
...
Author: Viz Vera. Date: 2026-06-17.
"""
...
PAIR = "petrol_inv_spy"
DATE_TAG = "20260711"  # GH#13 full-grid re-selection run (was 20260617 coarse-grid)
...
# petrol_inv_spy entry yet — gap flagged to Ace in the handoff note; these
# constants are the proposed canonical forms.
IND_LONG = "Total Petroleum Stocks"
IND_SHORT = "Petroleum Stocks"
TGT = "SPY"
...
GENERATED_BY = "Viz Vera — scripts/generate_charts_petrol_inv_spy.py (mode3_petrol_inv_spy)"


# ── Loaders ───────────────────────────────────────────────────────────────

def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "petrol_inv_spy_monthly_latest.parquet")
...
SRC_DATA = "data/petrol_inv_spy_monthly_latest.parquet"
...
def chart_hero():
    df = load_monthly()
    df = df[df["spy"].notna()]
    sig = df["petrol_inv_3m_pct"].dropna()

    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(
        x=sig.index, y=sig.values,
        name=f"{IND_SHORT} 3-month change (%)",
    ...
    fig.add_hline(y=0, line_dash="dash", line_color=C_NEUTRAL, line_width=0.8,
                  secondary_y=False)
    fig.add_hline(y=0.323, line_dash="dot", line_color=C_EVENT, line_width=1,
                  annotation_text="Winner threshold: 0.323%")
    # Counter-cyclical prior example: COVID inventory build during demand shock.
    ann_y = float(sig.loc["2020-05-31"]) if "2020-05-31" in sig.index else float(sig.loc["2020-04-30"])
    fig.add_annotation(x="2020-05-31", y=ann_y,
                       text="COVID: inventories rose as fuel demand collapsed<br>but the selected rule is procyclical",
    ...
    fig.update_layout(
        title=(f"{IND_LONG} 3-Month Change vs {TGT}: Procyclical Winner, Fragile Evidence "
               f"({y0}–{y1})"),
        template="plotly_white", hovermode="x unified", height=500,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_yaxes(title_text=f"{IND_SHORT} 3-month change (%)", secondary_y=False)
    fig.update_yaxes(title_text=f"{TGT} price (USD)", secondary_y=True)
    save_chart("hero", fig,
               caption=(f"{IND_LONG} 3-month change (vermillion, left axis) "
                        f"against the {TGT} price (blue, right axis), {y0}–{y1}. "
                        "The selected Long/Cash rule buys when the lagged 3-month "
                        "change is above 0.323%, even though COVID shows why the "
                        "natural inventory prior can be counter-cyclical. Shaded "
                        "bands mark NBER recessions."),
               alignment=("Hero shows the actual winning signal and threshold; "
                          "COVID annotation flags the prior-vs-evidence tension."),
...
def chart_regime_stats():
    ...
    labels = ["Q1<br>(Lowest 3m change)", "Q2", "Q3", "Q4<br>(Highest 3m change)"]
    ...
    fig.update_layout(
        title=dict(
            text=(f"Quartiles Corroborate the Procyclical Finding: Higher Inventory "
                  f"Growth, Better {TGT} Performance"),
            y=0.99, yanchor="top"),
        margin=dict(t=150))
    best = q.loc[q["sharpe"].idxmax()]
    save_chart("regime_stats", fig,
               caption=(f"SPY performance by {IND_SHORT} 3-month-change quartile: "
                        f"Sharpe (left) and annualized return (right). The gradient is "
                        f"monotonic in Sharpe from Q1 {q['sharpe'].iloc[0]:.2f} / "
                        f"{q['ann_return'].iloc[0]*100:.1f}% annualized return to Q4 "
                        f"{q['sharpe'].iloc[3]:.2f} / "
                        f"{q['ann_return'].iloc[3]*100:.1f}%. This descriptive chart "
                        f"corroborates the procyclical winner but is not a separate "
                        f"tradable rule."),
...
def chart_correlation_heatmap():
    ...
    save_chart("correlation_heatmap", fig,
               caption=(f"Pearson correlations between {IND_LONG} signals and {TGT} "
                        ...
                        f"cell is r = {best['value']:.3f} "
                        f"({best['pair_name'].replace('__',' vs ')}). Pairwise "
                        f"correlations are weak-to-moderate and do not by themselves "
                        f"validate the trading rule."),
               alignment=("Heatmap supports weak-to-moderate linear evidence; best "
                          "cell re-read from correlations.csv."),
...
def chart_ccf():
    ...
    fig.update_layout(
        title=(f"Pre-Whitened Cross-Correlation Points to an Imprecise 6-Month Lead: {IND_SHORT} vs {TGT} "
               f"Returns<br><sup>AR(12) pre-whitening; {nsig} of {len(d)} lags "
               f"significant (lag +6)</sup>"),
    ...
    save_chart("ccf_prewhitened", fig,
               caption=(f"Cross-correlation between AR(12) pre-whitened {IND_SHORT} "
                        ...
                        f"{nsig} cell (lag +6, ccf "
                        f"{float(d.loc[d['significant'],'ccf'].iloc[0]) if nsig else 0:.3f}) "
                        f"crosses the 95% band. The timing is consistent with the "
                        f"6-8 month Granger cluster, not a precise 12-month horizon."),
               alignment="CCF chart mirrors the imprecise 6-12 month lead window.",
               rules=["VIZ-IC1", "VIZ-NS1", "VIZ-O1"],
               sources=[f"results/{PAIR}/core_models_{DATE_TAG}/ccf_prewhitened.csv"],
               nber_required=False)


# ── 5. Granger F by lag (both directions; forward PRIMARY) ────────────────
...
def chart_granger():
    ...
    fig.add_trace(go.Bar(
        ...
        name=f"{IND_SHORT} → {TGT} — significant at lags 6-8",
        marker_color=C_IND))
    fig.add_trace(go.Bar(
        x=rev["lag"], y=rev["f_statistic"],
        name=f"{TGT} → {IND_SHORT} — no significant reverse lags",
        marker_color="#aec7e8"))
    fig.add_trace(go.Scatter(
    ...
    fig.update_layout(
        title=(f"Granger Evidence Clusters at 6-8 Months, Not Exactly L12 "
               f"<br><sup>Toda-Yamamoto Granger F-statistics by lag; "
               f"{IND_SHORT} → {TGT} significant at lags "
               f"{list(fwd.loc[fwd['significant'], 'lag'])}; reverse none</sup>"),
    ...
    save_chart("granger_f_by_lag", fig,
               caption=(f"Toda-Yamamoto Granger F-statistics at lags 1–12, both "
                        f"directions. {IND_SHORT} → {TGT} (vermillion) clears the 5% "
                        f"critical line at lags 6, 7, and 8 (min p = "
                        f"{fwd['p_value'].min():.3f}); {TGT} → {IND_SHORT} (pale blue) "
                        f"clears it at none. This supports a 6-12 month lead band, "
                        f"not a pinpointed 12-month causal horizon."),
               alignment=("Forward direction rendered as the visually primary series; "
                          "caption reconciles Granger 6-8 month cluster with L12 winner."),
...
def chart_local_projections():
    ...
    for col, dirn, color in ((1, "fwd", C_IND), (2, "rev", C_TGT)):
        ...
        fig.add_trace(go.Scatter(
            ...
            name=("Coefficient: petroleum stocks → market" if dirn == "fwd"
                  else "Coefficient: market → petroleum stocks"),
    ...
    save_chart("local_projections", fig,
               caption=(f"Local-projection impulse responses with HAC standard errors "
                        ...
                        f"from zero at every horizon (min p = {pmin:.2f}); the reverse "
...
def chart_transfer_entropy():
    ...
    fig.update_layout(
        title=("Transfer Entropy: No Information Flow Survives the Permutation Test "
               "in Either Direction<br><sup>Tercile binning, 300 permutations; both "
...
def chart_quantile():
    ...
    fig.update_layout(
        title=(f"Quantile Regression: Signal Is Strongest in High-Return Quantiles "
               f"<br><sup>{IND_SHORT} growth on {TGT} forward returns; significant "
               f"tau values: {sig_tau}</sup>"),
    ...
    save_chart("quantile_coef", fig,
               caption=(f"Quantile-regression coefficient of {TGT} forward returns on "
                        ...
                        f"95% CI. Upper-return quantiles are positive and significant "
                        f"(tau 0.90 and 0.95), consistent with a procyclical upside "
                        f"state rather than a downside crash hedge."),
...
def chart_hmm():
    ...
    fig.add_trace(go.Scatter(
        ...
        name="P(high-variance petroleum-stock regime)",
    ...
    fig.update_layout(
        title=(f"Hidden Markov Model (HMM): {IND_SHORT} Growth Spends Most of the "
               ...
               f"{float((h['hmm_state']==0).mean())*100:.0f}% of months</sup>"),
        xaxis_title="Date",
    ...
    save_chart("hmm_regime_probs", fig,
               caption=(f"Probability that {IND_SHORT} YoY growth is in its "
                        ...
                        f"covers much of the 1993–2025 model sample and pins near 1.0 through "
                        f"the COVID inventory episode (verified against the manifest "
...
def chart_equity_curves():
    ...
    fig.add_trace(go.Scatter(
        ...
        name=f"Petroleum inventory Long/Cash rule (Sharpe {w['oos_sharpe']:.2f})",
    ...
    nber_swatch(fig)
    fig.update_layout(
        title=(f"Defensive Overlay Improves Sharpe but Gives Up Return: "
               f"{IND_SHORT} Rule vs Buy & Hold {TGT}<br><sup>OOS "
               f"{w['oos_period_start']} to {w['oos_period_end']}: "
               f"{w['oos_ann_return']*100:.1f}% vs {w['bh_ann_return']*100:.1f}% "
               f"annualized return</sup>"),
        xaxis_title="Date",
        yaxis_title="Growth of 1.00",
        template="plotly_white", height=480,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    save_chart("equity_curves", fig,
               caption=(f"Cumulative growth of 1.00 for Evan's saved strategy returns "
                        f"and buy-and-hold returns. The OOS rule raises Sharpe "
                        f"({w['oos_sharpe']:.2f} vs {w['bh_sharpe']:.2f}) but gives "
                        f"up annualized return ({w['oos_ann_return']*100:.1f}% vs "
                        f"{w['bh_ann_return']*100:.1f}%). Shaded bands mark NBER "
                        f"recessions."),
               alignment="Strategy series read from strategy_returns_20260617.csv; metrics from winner_summary.json.",
               rules=["VIZ-IC1", "VIZ-NBER1", "VIZ-NS1", "VIZ-O1"],
...
def chart_drawdown():
    ...
    nber_swatch(fig)
    fig.update_layout(
        title=(f"Drawdown Protection Is the Rule's Main Benefit: "
    ...
    save_chart("drawdown", fig,
               caption=(f"Drawdown path for the petroleum-inventory Long/Cash rule "
                        f"and buy-and-hold {TGT}. In OOS, the strategy's max drawdown "
                        f"is {w['oos_max_drawdown']*100:.1f}% versus "
                        f"{w['bh_max_drawdown']*100:.1f}% for buy-and-hold; this is "
                        f"the main tradeoff benefit, not higher return."),
...
def chart_tournament_dist():
    ...
    save_chart("tournament_sharpe_dist", fig,
               ...
               f"benchmark; bootstrap p = 0.099 (not significant at 5%)."),
...
def chart_rolling_correlation():
    ...
    nber_swatch(fig)
    fig.update_layout(
        title=(f"The Sign Flips Often: 24-Month Rolling Correlation, "
               f"{IND_SHORT} 3-Month Change vs {TGT} Returns<br><sup>Sign agreement only "
    ...
    save_chart("rolling_correlation", fig,
               caption=(f"24-month rolling correlation between {IND_SHORT} 3-month change "
...
def chart_structural_break():
    sb = json.loads((RES / f"structural_break_{PAIR}.json").read_text())
    df = load_monthly()
    sig = df.loc[sb["sample_start"]:sb["sample_end"], "petrol_inv_3m_pct"].dropna()
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=sig.index, y=sig.values, mode="lines",
        name=f"{IND_SHORT} 3-month change (%)", line=dict(color=C_IND, width=1.8)))
    ...
    fig.update_layout(
        title=(f"No Structural Break Detected: Quandt-Andrews sup-F Test on "
               f"{IND_SHORT} 3-Month Change (p = {sb['p_value']:.2f})"),
        xaxis_title="Date",
        yaxis_title=f"{IND_SHORT} 3-month change (%)",
        template="plotly_white", height=420,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    save_chart("structural_break", fig,
               caption=(f"{IND_SHORT} 3-month change over the test sample "
...
_EP_LABEL = {"dotcom": "Dot-Com 2000–02", "gfc": "GFC 2008–09",
             "covid": "COVID 2020", "rate_hike_2022": "2022 Rates Shock",
             "post_covid_expansion": "Post-COVID expansion"}
...
def chart_subperiod_sharpe():
    ...
    fig.update_layout(
        title=(f"Subperiod Sharpe Is Helpful but Narrow: Winner Strategy by "
               f"Historical Episode<br><sup>COVID "
               f"{float(rows.loc[rows['episode']=='covid','ann_sharpe'].iloc[0]):.2f}; "
               f"2022 rates shock spent in cash; GFC has insufficient daily OOS rows; "
               f"post-COVID expansion Sharpe "
               f"{float(rows.loc[rows['episode']=='post_covid_expansion','ann_sharpe'].iloc[0]):.2f}</sup>"),
    ...
    save_chart("subperiod_sharpe", fig,
               caption=(f"Winner-strategy annualized Sharpe within each canonical "
                        f"stress episode. COVID 2020 contributes "
                        f"(Sharpe "
                        f"{float(rows.loc[rows['episode']=='covid','ann_sharpe'].iloc[0]):.2f}); "
                        f"the 2022 rates shock was spent entirely in cash (0.00 = "
                        f"flat, not a loss), GFC is marked insufficient_data, and the "
                        f"post-COVID expansion is conditionally durable."),
...
def chart_history_zoom(slug: str):
    ...
    sig = w["petrol_inv_3m_pct"].dropna()
    spy = w["spy"].dropna()

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06,
                        subplot_titles=[f"{IND_SHORT} 3-month change (%)",
                                        f"{TGT} price (USD)"])
    ...
    fig.update_yaxes(title_text="3-month change (%)", row=1, col=1)
    fig.update_yaxes(title_text="Price (USD)", row=2, col=1)
    save_chart(f"history_zoom_{slug}", fig,
               caption=(f"{IND_SHORT} 3-month change (top) and {TGT} price (bottom) "
    ...
    for c in ("rolling_sharpe_cp", "rolling_granger"):
        (OUT / f"chart_skip_{c}.json").write_text(json.dumps({
            ...
            "reason": ("ECON-CP2 artifacts intentionally absent for this pair: "
                       "regime_story=false in results/petrol_inv_spy/signal_scope.json "
