#!/usr/bin/env python3
"""Chart generation for U. Michigan Expected Business Conditions x SPY.

Presentational layer only. Every series is read from the committed
results/umcsent_spy artifacts or the monthly panel; no numbers are invented.
Expected business conditions is a bounded, mean-reverting survey diffusion (the
LEVEL is stationary) so the traded signals are the level and its transforms
(1-month change, 12-month change, 60-month rolling z-score). It is a LEADING
sentiment indicator and the economic prior is PROCYCLICAL; the searched winner
is also PROCYCLICAL at a 6-month lead (direction consistent with the prior),
but the lead is fragile -- captions flag that and the absent forecasting
evidence. Only NBER recession shading is used (no second shading band).
"""
...
def load_monthly() -> pd.DataFrame:
    return pd.read_parquet(REPO / "data" / "umcsent_spy_monthly_latest.parquet")
...
def save(name: str, fig: go.Figure, caption: str, sources: list[str]) -> None:
    ...
    meta = {
        ...
        "generated_by": "scripts/generate_charts_umcsent_spy.py",
        "caption": caption,
...
def chart_hero() -> None:
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index,
            y=df["umcsent"],
            name="Expected business conditions (diffusion)",
            line=dict(color=C_IND, width=2),
            hovertemplate="%{x|%Y-%m}<br>Expected conditions: %{y:.1f}<extra></extra>",
        ),
        secondary_y=False,
    )
    fig.add_trace(
    ...
    nber_legend(fig)
    fig.update_layout(
        title="Expected Business Conditions vs SPY",
    ...
    fig.update_yaxes(title_text="Expected conditions (diffusion level)", secondary_y=False)
    fig.update_yaxes(title_text="SPY price", secondary_y=True)
    save("hero", fig, "Expected-conditions diffusion level and SPY with NBER recession bands.", ["data/umcsent_spy_monthly_latest.parquet"])


def chart_regime_stats() -> None:
    q = pd.read_csv(RES / "regime_quartile_returns.csv")
    labels = ["Q1<br>Weak optimism", "Q2", "Q3", "Q4<br>Strong optimism"]
    fig = make_dual_panel_regime_chart(
        labels,
        q["sharpe"].astype(float).tolist(),
        (q["mean_return"].astype(float) * 12 * 100).tolist(),
        signal_label="Expected conditions",
        x_axis_title="Expected-conditions level quartile",
        axis_noun="",
        sample_sizes=q["n"].astype(int).tolist(),
    )
    fig.update_layout(title="SPY Returns by Expected-Conditions Regime")
    save("regime_stats", fig, "SPY performance across expected-conditions level quartiles.", ["results/umcsent_spy/regime_quartile_returns.csv"])
...
def chart_correlation_heatmap() -> None:
    ...
    fig.update_layout(title="Correlation: Expected-Conditions Signals vs Forward SPY Returns", template="plotly_white", height=520)
    save("correlation_heatmap", fig, "Pearson correlations between expected-conditions transforms and forward SPY returns.", [str(CORE / "correlations.csv")])
...
def chart_ccf() -> None:
    ...
    fig.update_layout(title="Cross-Correlation: Expected Conditions vs SPY Return", xaxis_title="Lag (months): +lead = expected conditions leads, -lag = SPY leads", yaxis_title="Correlation", template="plotly_white", height=430)
    save("ccf_prewhitened", fig, "Pre-whitened cross-correlation between expected conditions and SPY returns.", [str(CORE / "ccf_prewhitened.csv")])
...
def chart_granger() -> None:
    ...
    fig.update_layout(title="Granger Causality: Does Expected Conditions Help Forecast SPY?", xaxis_title="Lag (months)", yaxis_title="p-value", template="plotly_white", height=430)
    save("granger_f_by_lag", fig, "Granger p-values by lag for expected conditions predicting SPY returns.", [str(CORE / "granger_causality.csv")])
...
def chart_local_projections() -> None:
    ...
    fig.update_layout(title="Local Projection: SPY Response to Expected Conditions", xaxis_title="Forward horizon", yaxis_title="Coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("local_projections", fig, "Estimated SPY response across horizons to a move in expected conditions.", [str(CORE / "local_projections.csv")])
...
def chart_quantile() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Quantile Regression Coefficient", xaxis_title="SPY return quantile", yaxis_title="Expected-conditions coefficient (95% CI whiskers)", template="plotly_white", height=430)
    save("quantile_coef", fig, "Expected-conditions coefficient across SPY forward-return quantiles.", [str(CORE / "quantile_regression.csv")])
...
def chart_equity_drawdown() -> None:
    ...
    fig.update_layout(title="Equity Curve: Expected-Conditions Strategy vs SPY", yaxis_title="Growth of $1", template="plotly_white", height=460)
...
def chart_rolling_correlation() -> None:
    ...
    fig.add_hline(y=0, line_color=C_LINE)
    fig.update_layout(title="Rolling Correlation: Expected Conditions vs SPY", yaxis_title="Correlation", template="plotly_white", height=430)
    save("rolling_correlation", fig, "Rolling correlation between expected conditions and SPY returns.", [str(RES / f"rolling_correlation_{PAIR}.csv")])
...
def chart_structural_break() -> None:
    ...
    save("structural_break", fig, "Proxy check for large changes in the expected-conditions/SPY relationship.", [str(RES / f"structural_break_{PAIR}.json")])
...
def chart_subperiod() -> None:
    ...
    save("subperiod_sharpe", fig, "Strategy and buy-and-hold SPY Sharpe during major stress episodes.", ["results/umcsent_spy/subperiod_sharpe.csv"])
    save("walk_forward", fig, "Alias for the dashboard's standard walk-forward chart slot.", ["results/umcsent_spy/subperiod_sharpe.csv"])
...
def chart_history_zoom(slug: str, title: str, start: str, end: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["umcsent_chg_12m"], name="Expected conditions 12m change", line=dict(color=C_IND)), secondary_y=False)
    ...
    fig.update_yaxes(title_text="Expected conditions 12m change", secondary_y=False)
    fig.update_yaxes(title_text="SPY", secondary_y=True)
    save(f"history_zoom_{slug}", fig, f"{title}; gray bands are NBER recessions.", ["data/umcsent_spy_monthly_latest.parquet"])
...
def main() -> None:
    ...
    chart_subperiod()
    chart_history_zoom("dotcom", "Dot-Com Recession: Optimism Sagged into the 2001 Downturn", "1999-01-31", "2003-12-31")
    chart_history_zoom("gfc", "Global Financial Crisis: Expected Conditions Collapsed 2008-09", "2006-01-31", "2010-12-31")
    chart_history_zoom("covid", "COVID Shock: Expected-Conditions Plunge and Rebound", "2018-01-31", "2021-12-31")
    chart_history_zoom("inflation_2022", "2022 Rate Shock: Optimism Hit Lows but the Rule Lagged", "2021-01-31", "2025-09-30")
