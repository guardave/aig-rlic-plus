#!/usr/bin/env python3
"""
Full Econometrics Pipeline: C&I Loans (BUSLOANS) -> SPY
========================================================
Pair ID: busloans_spy (Pair #19, Mode 1)
Branch: fix260612_busloans_spy

Economic hypothesis (stated up front, per SOP):
  H0: BUSLOANS growth does NOT Granger-cause / predict SPY forward returns.
  H1a (procyclical prior): rising loan growth -> expansion -> higher SPY returns.
  H1b (lagging prior, taken seriously): C&I loans are a Conference Board
      LAGGING index component. Firms draw credit lines INTO downturns
      (COVID 2020: +30% YoY while equities crashed) and loan growth troughs
      AFTER recessions end (2009-10: -20% YoY during the equity recovery).
      Under H1b the indicator primarily LAGS equities (reverse Granger
      direction dominates) and any tradable forward signal must come from
      rate-of-change / acceleration transforms, possibly inverted.

  expected_direction (Dana, provisional): mixed. Direction is determined
  empirically; we test BOTH orientations in the tournament and run Granger
  in BOTH directions.

Category (Rule C1): credit-equity -> full correlation battery (incl.
distance correlation), pre-whitened CCF, Toda-Yamamoto Granger (both
directions), transfer entropy, local projections (fwd + reverse),
quantile regression, HMM 2-state regime detection.

Data: Dana's parquet data/busloans_spy_monthly_19470131_20260531.parquet
(953 x 21, monthly; SPY from 1993-01). Stationarity tests were produced by
Dana (results/busloans_spy/stationarity_tests_20260612.csv) — reviewed and
confirmed here, NOT re-run (SOP: confirm, don't duplicate).

Lag convention (binding, per Dana's handoff): BUSLOANS monthly aggregate is
available ~2-3 weeks after month-end -> L1 is the real-time floor. Tournament
lead grid starts at L1. Lead semantics: position at month t is a function of
the signal observed at month t-L; strategy_return_t = position_t * spy_ret_t.
With L>=1 there is no lookahead (the L1 case assumes execution at the END of
the month in which the prior month's value was published — feasible).
...
DATA_PATH = os.path.join(BASE_DIR, "data", "busloans_spy_monthly_19470131_20260531.parquet")
...
SIGNAL_COLS = {
    ...
    "yoy": "busloans_pct_yoy",
    "mom": "busloans_pct_mom",
    "mom3m": "busloans_3m_pct",
    "mom6m": "busloans_6m_pct",
    "dev_trend": "busloans_dev_trend_pct",
    "level_zscore_60m": "busloans_zscore_60m",
    "yoy_zscore_60m": "busloans_yoy_zscore_60m",
    "accel": "busloans_accel_pct",
    "contraction": "busloans_contraction",
...
def write_manifest(path, columns, assertions, extra=None):
    m = {
        ...
        "pair_id": PAIR_ID,
        "produced_by": "scripts/pair_pipeline_busloans_spy.py",
...
def stage_load():
    ...
    assert df.shape == (953, 21), f"unexpected shape {df.shape}"
    # Known-episode checks (Dana's handoff)
    assert 25.0 < df.loc["2020-04-30", "busloans_pct_yoy"] < 26.0
    assert 29.5 < df.loc["2020-05-31", "busloans_pct_yoy"] < 30.5
    assert -21.0 < df.loc["2009":"2010", "busloans_pct_yoy"].min() < -19.0
    ...
    print("  Defense-2 episode checks: COVID spike, 2009-10 trough, return magnitudes — PASS")
    return df
...
def stage_signals(df):
    """Fit 2-state HMM and Markov-switching on YoY loan growth; persist ALL
    ...
    # label: higher-variance state = stress (META-D2 variance ranking)
    ...
    # sanity: stress regime should capture the big-swing eras (COVID spike & GFC trough)
    cov_p = prob_stress.loc["2020-04-30":"2020-12-31"].mean()
    print(f"  HMM stress prob mean 2020-04..12: {cov_p:.2f} (high-variance episodes)")

    write_manifest(os.path.join(MODELS_DIR, "hmm_states.parquet"),
                   {"hmm_state": "integer state; stress = high-variance YoY-growth regime",
                    "prob_stress": "P(stress regime); higher = loan growth in high-variance regime",
                    "prob_calm": "1 - prob_stress"},
                   [{"description": "probabilities in [0,1]", "check": "prob_stress between 0 and 1"},
                    {"description": "COVID drawdown era is high-variance", "filter": "2020-04 to 2020-12",
    ...
    try:
        ms = MarkovRegression(ms_data["spy_ret"], k_regimes=2,
                              exog=sm.add_constant(ms_data["busloans_pct_yoy"]),
    ...
    write_manifest(sig_path,
                   {c: f"tournament-eligible signal derived solely from BUSLOANS ({c})" for c in sig_cols},
                   [{"description": "COVID YoY spike present", "filter": "2020-05-31",
                     "column": "busloans_pct_yoy", "check": "value > 29"},
                    {"description": "GFC-aftermath trough", "filter": "2009-01..2010-12",
                     "column": "busloans_pct_yoy", "check": "min < -19"},
...
# ===================================================================
# STAGE 3: EXPLORATORY + CORRELATION BATTERY (Rule C1/C2, credit-equity)
# ===================================================================
...
def stage_core_models(df):
    ...
    d_max = 1  # YoY growth treated I(0)/borderline (KPSS marginal) -> TY augmentation lag
    ...
    print(f"        BUSLOANS->SPY significant at lags: {list(fwd_sig.lag)}")
    print(f"        SPY->BUSLOANS significant at lags: {list(rev_sig.lag)}")
    ...
    for direction in ["fwd", "rev"]:
        for h in [1, 3, 6, 12]:
            if direction == "fwd":
                ...
                # reverse: SPY return predicting BUSLOANS YoY h months ahead
...
def stage_quartiles(df):
    v = df[["busloans_pct_yoy", "spy_ret"]].dropna()
    q = pd.qcut(v["busloans_pct_yoy"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
    rows = []
    ...
    write_manifest(path,
                   {"quartile": "BUSLOANS YoY growth quartile (Q1 lowest growth)", "n_months": "obs",
...
def stage_tournament(df):
    ...
    split = {
        ...
        "justification": (
            ...
            f"({n_months} months, 1993-02 onward; BUSLOANS history extends to 1947 but the "
            f"tournament requires target returns). min(max(36, round({n_months}*0.25)), 120) = {oos_n} "
            f"months. No structural-break exclusion applied; H.8 definitional breaks pre-1973 are "
            f"outside the SPY-bound sample entirely."),
    }
...
def stage_winner(df, tdf, split):
    ...
    sr_meta = {
        ...
        "produced_by": "scripts/pair_pipeline_busloans_spy.py", "rule": "ECON-SR1",
        ...
        "generated_at": NOW_ISO, "generated_by": "Econ Evan (fix260612_busloans_spy)",
    }
...
def stage_cross_period(df, winner, split, position, strat_ret, sig_lagged):
    ...
    # CP1-A: subperiod Sharpe from episode registry (credit category)
    with open(os.path.join(SCHEMA_DIR, "episode_registry.json")) as f:
        reg = json.load(f)
    episodes = reg.get("credit", reg["_fallback"])
...
def main():
    ...
    # economic direction: does the winner go long when loan growth is high (procyclical) or low?
    ...
    winner_summary = {
        ...
        "signal_code": f"busloans_{winner['signal']}" if not winner["signal"].startswith(("hmm", "markov")) else winner["signal"],
        "signal_display_name": f"BUSLOANS {winner['signal']}",
        ...
        "lead_unit": "months",
        "lead_description": f"Signal lead = {int(winner['lead_months'])} month(s); L1 is the real-time floor given the ~2-3 week H.8 publication lag",
        ...
        "notes": (f"Mode 1, fix260612_busloans_spy. Tournament: {len(tdf)-1} strategy combos "
                  ...
                  f"BUSLOANS is a LAGGING indicator (Conference Board lagging-index component); "
                  f"Toda-Yamamoto Granger finds NO forward causality (BUSLOANS->SPY n.s. at all lags 1-12) "
                  f"and STRONG reverse causality (SPY->BUSLOANS significant at every lag) — this pair is "
                  f"primarily confirmatory, and the strategy edge should be read with corresponding caution. "
    ...
    sd = ("Long SPY when the lagged BUSLOANS signal is {} its threshold; otherwise {}."
    ...
    # Dana's provisional expected_direction is 'mixed'; a countercyclical winner is consistent with 'mixed'
    interp["direction_consistent"] = interp.get("expected_direction") in ("mixed", direction)
    interp["key_finding"] = (
        f"Lead-lag verdict: BUSLOANS LAGS SPY. Toda-Yamamoto Granger BUSLOANS->SPY is not significant at "
        f"any lag 1-12, while SPY->BUSLOANS is significant at every lag — confirming the Conference Board "
        f"lagging-indicator classification. The tournament winner ({winner['signal']}/{winner['threshold']}/"
        f"{strat_family} {orientation}/L{winner['lead_months']}/{winner['lookback']}) is countercyclical "
        f"(long SPY only when MoM loan growth is in the bottom quartile of its trailing 36m range): OOS "
        ...
        f"bootstrap p={winner_boot_p:.3f} (n.s. at 5%) and durability is episode-concentrated — treat as a "
        f"defensive/confirmatory overlay, not a standalone alpha signal.")
    ...
    ind_der = [
        ...
        for c, d, s, r in [
            ("busloans_usd", "C&I loans outstanding, all commercial banks, $bn SA", "FRED BUSLOANS", "raw"),
            ("busloans_pct_yoy", "12-month % change in loans (cycle signal)", "100*(L_t/L_{t-12}-1)", "derivative"),
            ("busloans_pct_mom", "1-month % change in loans", "100*(L_t/L_{t-1}-1)", "derivative"),
            ("busloans_3m_pct", "3-month % change (momentum)", "100*(L_t/L_{t-3}-1)", "derivative"),
            ("busloans_6m_pct", "6-month % change (momentum)", "100*(L_t/L_{t-6}-1)", "derivative"),
            ("busloans_ma12_usd", "12-month moving average of the level", "rolling mean(12)", "derivative"),
            ("busloans_dev_trend_pct", "% deviation of the level from its 12m MA", "100*(L_t/MA12-1)", "derivative"),
            ("busloans_zscore_60m", "rolling 60m z-score of the LEVEL (trends positive — see Dana note)", "(L_t-mean60)/sd60", "derivative"),
            ("busloans_yoy_zscore_60m", "rolling 60m z-score of YoY growth (preferred cycle signal)", "(yoy_t-mean60)/sd60", "derivative"),
            ("busloans_accel_pct", "month-on-month change in MoM growth (acceleration, pp)", "mom_t - mom_{t-1}", "derivative"),
            ("busloans_contraction", "1 when YoY loan growth < 0", "indicator(yoy<0)", "regime_state"),
    ...
    tgt_der = [
        ...
        for c, d, s, r in [
            ...
            ("spy_ret", "SPY monthly return (decimal; registry name retained — see Dana quirk note)", "P_t/P_{t-1}-1", "derivative"),
    ...
    scope = {
        ...
        "indicator_axis": {
            "canonical_column": "busloans_usd",
            "display_name": "C&I Loans Outstanding (BUSLOANS)",
            "derivatives": ind_der,
        },
        "target_axis": {
        ...
        "notes": ("ECON-SD: only BUSLOANS derivatives and SPY derivatives are in scope. Controls in the "
                  "parquet (UNRATE, DGS10, DFF, VIX) are context columns and are NOT used as signals or "
                  "rendered on the pair page. regime_story: false (CP2 skipped)."),
    }
...
- BUSLOANS→SPY TY-Granger significant lags: {fwd_sig_lags or 'NONE'}
- SPY→BUSLOANS TY-Granger significant lags: {rev_sig_lags or 'NONE'}
...
"""
    ...
    # analyst_suggestions.json (ECON-AS) — honest, minimal
    sugg = {"schema_version": "1.0.0", "pair_id": PAIR_ID, "rule": "ECON-AS",
            "suggestions": [
                {"signal_name": "SLOOS C&I lending standards (tightening %)", "proposed_by": "evan",
                 "source": "FRED/Data Master (ci_loan — distinct series, see LEAD-DV1)",
                 "observation": "BUSLOANS quantity lags the cycle; SLOOS standards are the LEADING twin of the same credit channel.",
                 "rationale": "Literature (Lown-Morgan 2006) finds standards lead loan quantities and output by 2-4 quarters.",
                 "possible_use_case": "new pair",
                 "caveats": "Quarterly frequency; survey-based; shorter history (1990+).",
