#!/usr/bin/env python3
"""
Full Econometrics Pipeline: Housing Starts (SAAR, HOUST) -> SPY
===============================================================
Pair ID: housing_starts_spy
Mode: 2 (single maker, multiple checkers)

Economic hypothesis (stated up front, per SOP):
  H0: Housing Starts growth does NOT predict / Granger-cause forward SPY returns.
  H1 (procyclical / leading): rising starts -> stronger, rate-sensitive
      construction activity, an early-cycle signal that leads employment /
      residential investment / consumption -> higher forward SPY. Collapsing
      starts (2006-08) preceded the GFC bear.
  H1b (peak-cycle mean-reversion): at cycle peaks a far-above-trend reading can
      flip countercyclical (INDPRO precedent). Direction is empirical.

  expected_direction (provisional): procyclical. Both orientations tested in the
  tournament; Granger run in BOTH directions (starts and equities are jointly
  driven by rates/the cycle; reverse causality is plausible).

SA handling (contrast with nhs_spy):
  HOUST is Seasonally Adjusted (SAAR). No deseasonalisation is required: MoM% is
  a valid momentum input. Signals: YoY (primary), MoM, 3M-change, 3M-avg YoY,
  YoY acceleration, z-score on YoY. The raw SAAR level (hst_level) is
  trend-dominated / non-stationary and is NON-signal (provenance only).

Category (Rule C1): housing/activity -> full correlation battery (incl. distance
correlation), pre-whitened CCF, Toda-Yamamoto Granger (both directions), transfer
entropy, local projections (fwd + reverse), quantile regression, HMM 2-state
regime detection, regime quartile returns.

Data: Dana's parquet data/housing_starts_spy_monthly_latest.parquet (monthly;
1990-01 -> 2026-06; SPY-bound from 1993). Source = FRED HOUST live API.
Stationarity tests produced by Dana
(results/housing_starts_spy/stationarity_tests_20260814.csv) — reviewed and
CONFIRMED here, NOT re-run. hst_level is NON-stationary and NOT used as a signal;
all signals are stationary growth/transform series.

Lag convention (binding): Census/HUD New Residential Construction publishes ~17th
for the prior month -> L1 is the real-time floor. Tournament lead grid centers L6
and sweeps L0..12. Lead semantics: position at month t is a function of the signal
observed at month t-L; strategy_return_t = position_t * spy_ret_t.

Author: Econ Evan (Econometrics Agent) — Lead wearing role hat (Mode 2)
Date: 2026-08-14
"""
...
INDICATOR_NAME = "Housing Starts (SAAR)"
TARGET_NAME = "SPY"
TARGET_SYMBOL = "SPY"
DATE_TAG = "20260814"
...
DATA_PATH = os.path.join(BASE_DIR, "data", "housing_starts_spy_monthly_latest.parquet")
...
# HOUST LEVEL excluded (non-stationary). Only stationary transforms.
SIGNAL_COLS = {
    "yoy": "hst_pct_yoy",
    "mom": "hst_pct_mom",
    "yoy_3m": "hst_3m_pct_yoy",
    "chg_3m": "hst_3m_pct",
    "yoy_accel": "hst_yoy_accel_pct",
    "yoy_zscore_120m": "hst_yoy_zscore_120m",
    "contraction": "hst_yoy_contraction_flag",
...
def write_manifest(path, columns, assertions, extra=None):
    m = {
        ...
        "pair_id": PAIR_ID,
        "produced_by": "scripts/pair_pipeline_housing_starts_spy.py",
...
def stage_load():
    ...
    # Known-episode checks (Dana's handoff / interpretation_metadata)
    assert df.loc["2009", "hst_pct_yoy"].min() < -20.0, "GFC housing collapse (<-20% YoY) expected"
    assert df.loc["2023", "hst_pct_yoy"].min() < 0.0, "2022-23 rate-shock contraction expected"
    assert int(df["hst_yoy_contraction_flag"].sum()) > 0, "contraction flag should fire"
    spy_ret = df["spy_ret"].dropna()
    assert spy_ret.abs().max() < 0.40, "monthly SPY return magnitude implausible"
    print(f"  Loaded {df.shape}, {df.index.min().date()} -> {df.index.max().date()}")
    print("  Defense-2 episode checks: GFC housing collapse, 2022-23 contraction, return magnitudes — PASS")
    return df


# ===================================================================
# STAGE 2: DERIVED REGIME SIGNALS (HMM + Markov-switching)
# ===================================================================
@log_stage("2_signals")
def stage_signals(df):
    ...
    gfc_p = prob_stress.loc["2008-01-31":"2009-06-30"].mean()
    print(f"  HMM stress prob mean 2008-01..2009-06 (GFC): {gfc_p:.2f} (housing collapse is high-variance)")

    write_manifest(os.path.join(MODELS_DIR, "hmm_states.parquet"),
                   {"hmm_state": "integer state; stress = high-variance Housing-Starts-YoY-growth regime",
                    "prob_stress": "P(stress regime); higher = Housing Starts growth in high-variance regime",
                    "prob_calm": "1 - prob_stress"},
                   [{"description": "probabilities in [0,1]", "check": "prob_stress between 0 and 1"},
                    {"description": "GFC housing collapse era is high-variance", "filter": "2008-01 to 2009-06",
    ...
    try:
        ms = MarkovRegression(ms_data["spy_ret"], k_regimes=2,
                              exog=sm.add_constant(ms_data["hst_pct_yoy"]),
    ...
    write_manifest(sig_path,
                   {c: f"tournament-eligible signal derived solely from HOUST ({c})" for c in sig_cols},
                   [{"description": "GFC housing collapse present", "filter": "2008-01..2009-12",
                     "column": "hst_pct_yoy", "check": "min < -20"},
                    {"description": "2022-23 rate-shock contraction", "filter": "2022-01..2023-12",
                     "column": "hst_pct_yoy", "check": "min < 0"},
...
def stage_correlations(df):
    ...
    for outdir in (MODELS_DIR, EXPLORE_DIR):
...
def stage_core_models(df):
    ...
    print(f"        Housing Starts->SPY significant at lags: {list(fwd_sig.lag)}")
    print(f"        SPY->Housing Starts significant at lags: {list(rev_sig.lag)}")
...
def stage_quartiles(df):
    v = df[["hst_pct_yoy", "spy_ret"]].dropna()
    q = pd.qcut(v["hst_pct_yoy"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
    rows = []
    ...
    write_manifest(path,
                   {"quartile": "Housing Starts YoY growth quartile (Q1 lowest growth)", "n_months": "obs",
...
def stage_tournament(df):
    ...
    split = {
        ...
        "justification": (
            ...
            f"({n_months} months, 1993 onward; HOUST history extends to 1959 but the "
            f"tournament requires target returns). min(max(36, round({n_months}*0.25)), 120) = {oos_n} "
            f"months. No structural-break exclusion applied."),
    }
    ...
    leads = [0, 1, 2, 3, 6, 9, 12]      # L1 = real-time floor (Census/HUD ~17th publication lag); L6 centered; ECON-LL1 monthly grid
    ...
    for code, col in SIGNAL_COLS.items():
        ...
        for lead in leads:
            ...
            if code in ["yoy", "mom", "yoy_3m", "chg_3m", "yoy_accel"]:
            ...
            for (thr_name, lb_name), thr in {**thr_static, **thr_roll}.items():
                ...
                for strat in strategies:
                    for orientation in ["pro", "counter"]:
                        ...
                        strat_ret = position * spy_ret  # lead >= 0; shift already applied to signal
    ...
    manifest = {
        ...
        "execution_lag": "position_t = rule(signal_{t-lead}); L1 real-time floor (Census/HUD housing-starts ~17th publication lag); L6 centered",
...
# ===================================================================
# STAGE 7 helpers: winner selection + series derivation
# ===================================================================
...
def _thr_series(sig, thr_name, lb_name, is_mask):
    lookbacks = {"LB36": 36, "LB60": 60, "LB120": 120}
    if thr_name.startswith("T1_fixed_p"):
        pct = int(thr_name.split("p")[-1])
        return sig[is_mask].dropna().quantile(pct / 100)
    if thr_name == "T4_zero":
        return 0.0
    if thr_name.startswith("T2_roll_p"):
        lb = lookbacks[lb_name]
        return sig.rolling(lb, min_periods=max(int(lb * 0.6), 24)).quantile(int(thr_name.split("p")[-1]) / 100)
    if thr_name.startswith("T3_zscore"):
        lb = lookbacks[lb_name]
        roll = sig.rolling(lb, min_periods=max(int(lb * 0.6), 24))
        k = float(thr_name.split("_")[-1])
        return roll.mean() - k * roll.std() if "neg_" in thr_name else roll.mean() + k * roll.std()
    raise ValueError(thr_name)


def _position_from(sig, thr, thr_name, strat, orientation, lb_name):
    lookbacks = {"LB36": 36, "LB60": 60, "LB120": 120}
    above = sig < thr if "neg_" in thr_name else sig > thr
    pos_bool = ~above if orientation == "counter" else above
    if strat == "P1_long_cash":
...
def derive_winner_series(df, winner, split):
    ...
    thr = _thr_series(sig, winner["threshold"], winner["lookback"], is_mask)
    strat, orientation = winner["strategy"].rsplit("_", 1)
    position = _position_from(sig, thr, winner["threshold"], strat, orientation, winner["lookback"])
    return position, work["spy_ret"], sig, thr


# ===================================================================
# STAGE 7: WINNER SELECTION + artifacts (trade logs, strategy returns)
# ===================================================================
...
def stage_winner(df, tdf, split):
    ...
    if resolved_at > 1:
        lines = [f"# Tournament Tie Note — {PAIR_ID} ({DATE_TAG})", "",
                 ...
                 "the selected winner is preferred on the documented tie-break dimension."]
    ...
    sr_meta = {
        ...
        "produced_by": "scripts/pair_pipeline_housing_starts_spy.py", "rule": "ECON-SR1",
        ...
        "generated_at": NOW_ISO, "generated_by": "Econ Evan (Mode 2, housing_starts_spy)",
    }
    ...
    # --- winner_trade_log.csv (internal monthly position log) ---
    log = pd.DataFrame({
        "date": spy_ret.index.strftime("%Y-%m-%d"),
    ...
    pos_series = position.reindex(spy_ret.index).fillna(0.0)
    for dt in spy_ret.index:
        p = float(pos_series.loc[dt])
        if abs(p - prev) > 1e-9:
            side = "BUY" if p > prev else "SELL"
            qty = abs(p) * 100
            notional = abs(p - prev) * capital
            sv = sig_lagged.loc[dt] if dt in sig_lagged.index else np.nan
...
def stage_cross_period(df, winner, split, position, strat_ret, sig_lagged):
    ...
    with open(os.path.join(SCHEMA_DIR, "episode_registry.json")) as f:
        reg = json.load(f)
    episodes = reg.get("activity", reg["_fallback"])  # housing indicator -> activity episode set
...
# ===================================================================
def _recompute_oos_from_winner_summary(df, ws, split):
    """Independently recompute OOS Sharpe FROM the winner_summary encoded rule.
    umcsent-precedent guardrail: must reproduce headline within +/-0.03."""
    work = df.dropna(subset=["spy_ret"]).copy()
    is_mask = work.index <= split["in_sample_end"]
    col = ws["signal_column"]
    sig = work[col].shift(int(ws["lead_value"]))
    thr = _thr_series(sig, ws["threshold_code"], ws["lookback"], is_mask)
    long_when_high = ws["direction"] == "procyclical"
    rule_is_gt = ws["threshold_rule"] == "gt"
    orientation = "pro" if (long_when_high == rule_is_gt) else "counter"
    strat = ws["strategy_family"]
    position = _position_from(sig, thr, ws["threshold_code"], strat, orientation, ws["lookback"])
    strat_ret = position * work["spy_ret"]
    oos_mask = (strat_ret.index >= split["oos_start"]) & (strat_ret.index <= split["oos_end"])
    m = ann_metrics(strat_ret[oos_mask])
    return {"oos_sharpe": m["sharpe"], "oos_ann_return": m["ann_return"], "oos_max_drawdown": m["max_dd"]}


def _ensure_signal_code_registered(sig_code, winner, parquet_col):
    reg_path = os.path.join(SCHEMA_DIR, "signal_code_registry.json")
    with open(reg_path) as f:
        reg = json.load(f)
    existing = {e["signal_code"] for e in reg["signals"]}
    if sig_code in existing:
        print(f"  signal_code registry assertion: PASS ({sig_code} already registered)")
        return
    descr_map = {
        "yoy": "12-month percent change in Housing Starts (FRED HOUST, SAAR); primary growth signal.",
        "mom": "Month-over-month percent change in Housing Starts (valid because HOUST is seasonally adjusted).",
        "yoy_accel": "Month-on-month change in Housing Starts YoY growth (acceleration, pp).",
        "yoy_3m": "YoY percent change of the 3-month moving average of Housing Starts (smooths noise).",
        "chg_3m": "Three-month percent change of the Housing Starts level.",
        "yoy_zscore_120m": "Rolling 120-month z-score of Housing Starts YoY growth.",
        "contraction": "Binary flag: 1 when Housing Starts YoY growth < 0 (construction-activity contraction).",
    }
    reg["signals"].append({
        "signal_code": sig_code,
        "display_name": f"Housing Starts {winner['signal']} ({INDICATOR_NAME})",
        "parquet_column_pattern": parquet_col,
        "description": descr_map.get(winner["signal"], f"HOUST transform {winner['signal']}."),
        "source_method": "roc" if winner["signal"] in ("yoy", "mom", "yoy_3m", "chg_3m") else (
            "zscore" if "zscore" in winner["signal"] else "transform"),
        "added_at": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:],
        "first_pair": PAIR_ID,
        "notes": "Housing Starts (SAAR) growth transform; tested both procyclical and countercyclical in the tournament.",
    })
    with open(reg_path, "w") as f:
        json.dump(reg, f, indent=2)
    print(f"  signal_code registry: APPENDED {sig_code} (ECON-DS3 append-only)")


def _write_evidence_status(winner, ws, split, n_valid, median_sharpe, boot_p, verdict, stab,
                           fwd_sig_lags, rev_sig_lags, oos_pos_mean):
    fwd_found = bool(fwd_sig_lags)
    plain = (
        f"What you see is the best rule we found by searching {n_valid} valid strategy combinations on "
        f"data the models had not seen; this one came out on top (Sharpe {ws['oos_sharpe']:.2f} vs "
        f"{ws['bh_sharpe']:.2f} for buy-and-hold SPY). Honesty checks: (1) the typical strategy in our "
        f"search scored {median_sharpe:.2f}, so the winner may be among the luckier of many tries; "
        ...
        + ("Housing Starts growth DID lead SPY in the formal causality test, consistent with housing being "
           "an early-cycle leading indicator, " if fwd_found else
           "Housing Starts growth did NOT lead SPY in the formal causality test, ")
        + "so read this as a search result, not an independently validated forecasting signal. "
          "No fresh confirmation test (final exam) on untouched data has been run yet.")
    tech = (
        f"Tournament-OOS only ({split['oos_start']}..{split['oos_end']}, {ws['oos_n']} months; "
        f"ECON-OOS2 v1_max36_25pct_cap120). Winner {ws['signal_code']}/{ws['threshold_code']}/"
        f"{ws['strategy_family']}/L{ws['lead_value']}/{ws['lookback']}: OOS Sharpe {ws['oos_sharpe']}, "
        f"bootstrap p={boot_p} vs resampled B&H, CP1-A durability {verdict}, rolling-corr {stab}, "
        f"mean OOS exposure {oos_pos_mean:.2f}. Spec-curve: median OOS Sharpe across {n_valid} valid "
        f"combos = {median_sharpe:.3f} vs B&H {ws['bh_sharpe']}. Lead-lag: TY Granger forward "
        f"significant lags {fwd_sig_lags or 'NONE'}, reverse significant lags {rev_sig_lags or 'NONE'}. "
        f"SA series (SAAR): signals are stationary growth transforms (raw level excluded). "
        f"No holdout/final exam run; selection and evaluation share the same OOS window.")
    es = {
        "pair_id": PAIR_ID, "schema_version": "1.2.0", "status": "found_in_search",
        "updated_at": NOW_ISO, "owner": "evan",
        "plain_english": plain, "technical_note": tech,
        "next_step": ("Run ECON-FE1 final exam: freeze the winning rule and test once on a confirmation "
                      "window the search never touched. Calibrate expectation by the bootstrap p and the "
                      "Granger lead-lag verdict above."),
    }
...
def main():
    ...
    if winner["signal"].startswith(("hmm", "markov")):
        sig_code = {"hmm_stress": "hmm_stress", "markov_regime": "markov_regime"}[winner["signal"]]
    else:
        sig_code = f"hst_{winner['signal']}"

    _SIGNAL_DISPLAY = {
        "yoy": "Housing Starts YoY growth",
        "mom": "Housing Starts month-over-month growth",
        "yoy_accel": "Housing Starts YoY acceleration",
        "yoy_3m": "Housing Starts 3-month-average YoY growth",
        "chg_3m": "Housing Starts 3-month change",
        "yoy_zscore_120m": "Housing Starts YoY z-score",
        "contraction": "Housing Starts YoY contraction flag",
        "hmm_stress": "Housing Starts housing-regime probability",
        "markov_regime": "Housing Starts Markov-regime probability",
    }
    signal_display_name = _SIGNAL_DISPLAY.get(winner["signal"], f"Housing Starts {winner['signal']}")
    ...
    winner_summary = {
        ...
        "lead_unit": "months",
        "lead_description": f"Signal lead = {int(winner['lead_months'])} month(s); L1 is the real-time floor given the ~17th-of-month Census/HUD publication lag for the prior month",
    ...
    # Long condition in terms of the raw signal reflects long_when_high (which
    # already folds in both the threshold comparator and the pro/counter
    # orientation), NOT thr_rule alone — otherwise a counter+gt winner reads as
    # a self-contradictory "long when above ... weak = risk-on".
    sd = ("Long SPY when the lagged Housing Starts signal is {} its threshold; otherwise {}."
          .format("above" if long_when_high else "below",
    ...
    if not long_when_high:
        sd += " (Countercyclical orientation: weak/decelerating housing starts = risk-on.)"
    else:
        sd += " (Procyclical orientation: strong housing-starts growth = risk-on, consistent with housing as a leading indicator.)"
    winner_summary["strategy_description"] = sd

    # ECON-T5 selection provenance block
    cand_all = tdf[(tdf.signal != "BENCHMARK") & tdf.valid].copy()
    cand_sorted = cand_all.sort_values(["oos_sharpe", "oos_ann_return"], ascending=False)
    runner = cand_sorted.iloc[1] if len(cand_sorted) > 1 else cand_sorted.iloc[0]
    # locate the winner's row index in the on-disk tournament CSV
    tourn_file = f"tournament_results_{DATE_TAG}.csv"
    win_mask = ((tdf["signal"] == winner["signal"]) & (tdf["threshold"] == winner["threshold"]) &
                (tdf["strategy"] == winner["strategy"]) & (tdf["lead_months"] == winner["lead_months"]) &
                (tdf["lookback"] == winner["lookback"]))
    win_row_idx = int(tdf.index[win_mask][0])
    n_signals = int(tdf.loc[tdf.signal != "BENCHMARK", "signal"].nunique())
    n_thresholds = int(tdf.loc[tdf.signal != "BENCHMARK", "threshold"].nunique())
    n_strategies = int(tdf.loc[tdf.signal != "BENCHMARK", "strategy"].nunique())
    winner_summary["selection"] = {
        "objective": "max_oos_sharpe",
        "objective_formula": "argmax OOS Sharpe over valid combos, ECON-T3 cascade tie-break (oos_sharpe -> oos_ann_return -> min|DD| -> n_trades -> signal)",
        "grid_scanned": {
            "leads": sorted({int(x) for x in tdf["lead_months"].unique()}),
            "n_signals": n_signals,
            "n_thresholds": n_thresholds,
            "n_strategies": n_strategies,
            "n_valid_combos": int(n_valid),
            "median_valid_objective": round(float(cand_all["oos_sharpe"].median()), 4),
        },
        "tie_break_step": int(resolved_at),
        "raw_winner_row": {
            ...
            "lead_value": int(winner["lead_months"]),
            "source_tournament_file": f"results/{PAIR_ID}/{tourn_file}",
            "source_row_index": win_row_idx,
            "display_alias": (f"{winner['signal']}/{winner['threshold']}/{winner['strategy']}/"
                              f"L{int(winner['lead_months'])}/{winner['lookback']}"),
        },
        "runner_up": {
        ...
        "rationale": (f"Winner is the max-OOS-Sharpe valid combo ({n_valid} valid of {len(tdf)-1}); "
                      f"cascade resolved at step {resolved_at} with {n_tied} tied at step 1."),
    ...
    oos_pos_mean = float(position[(position.index >= split["oos_start"])].mean())
    granger_verdict = (
        f"Toda-Yamamoto Granger Housing Starts->SPY significant at lags {fwd_sig_lags or 'NONE'}; "
        f"SPY->Housing Starts significant at lags {rev_sig_lags or 'NONE'}.")
    winner_summary["notes"] = (
        f"Mode 2, housing_starts_spy. Tournament: {len(tdf)-1} strategy combos "
        ...
        f"Housing Starts is SA (FRED HOUST, SAAR); signals are stationary growth transforms, raw level excluded. "
        f"Lead-lag: {granger_verdict} Robustness caveats: winner bootstrap p = {winner_boot_p:.3f} "
    ...
    rc = subprocess.run([PYEXE, os.path.join(BASE_DIR, "scripts", "validate_schema.py"),
    ...
    # --- ECON-SR1 winner_summary RE-RECONCILIATION (umcsent guardrail) ---
    recompute = _recompute_oos_from_winner_summary(df, winner_summary, split)
    headline = winner_summary["oos_sharpe"]
    diff = recompute["oos_sharpe"] - headline
    print(f"  ECON-SR1 winner_summary recompute: encoded-rule OOS Sharpe {recompute['oos_sharpe']:.4f} "
          f"vs headline {headline:.4f} (diff {diff:+.4f})")
    if abs(diff) > 0.03:
        raise SystemExit(
            f"winner_summary ENCODING MISMATCH: recomputed {recompute['oos_sharpe']:.4f} "
            f"!= headline {headline:.4f} (|diff|={abs(diff):.4f} > 0.03).")
    print("  winner_summary encoding faithfully reproduces headline OOS Sharpe (±0.03): PASS")

    _ensure_signal_code_registered(sig_code, winner, SIGNAL_COLS[winner["signal"]])
    ...
    fwd_found = bool(fwd_sig_lags)
    interp["key_finding"] = (
        f"Lead-lag: Housing Starts YoY {'Granger-causes SPY at lags ' + str(fwd_sig_lags) if fwd_found else 'does NOT Granger-cause SPY at any lag 1-12'}; "
        f"reverse SPY->Housing Starts significant at lags {rev_sig_lags or 'NONE'}. "
        f"The tournament winner (Housing Starts {winner['signal']}, {strat_family} {orientation}, "
        f"lead {winner['lead_months']}m) is {direction}: OOS Sharpe "
        f"{winner_summary['oos_sharpe']:.2f} vs buy-and-hold {winner_summary['bh_sharpe']:.2f}, max drawdown "
        f"{winner_summary['oos_max_drawdown']*100:.1f}% vs {winner_summary['bh_max_drawdown']*100:.1f}%. "
        f"Bootstrap p={winner_boot_p:.3f}; durability '{verdict}'; rolling-correlation '{stab}'.")
    if fwd_found and verdict in ("durable",) and winner_boot_p < 0.05:
    ...
    tw = {
        ...
        "suggested_strategy_objective": None,
        "generated_at": NOW_ISO,
    }
    ...
    ind_der = [
        ...
        for c, d, s, r in [
            ("hst_level", "Housing Starts, 000s SAAR (FRED HOUST). NON-STATIONARY, trend-dominated — not used as a signal", "FRED HOUST", "raw"),
            ("hst_pct_yoy", "12-month % change in Housing Starts (headline growth signal)", "100*(N_t/N_{t-12}-1)", "derivative"),
            ("hst_pct_mom", "1-month % change in Housing Starts (valid: SA series)", "100*(N_t/N_{t-1}-1)", "derivative"),
            ("hst_3m_pct", "3-month % change of the Housing Starts level", "100*(N_t/N_{t-3}-1)", "derivative"),
            ("hst_3m_pct_yoy", "YoY change of 3-month moving average", "100*(MA3_t/MA3_{t-12}-1)", "derivative"),
            ("hst_yoy_accel_pct", "month-on-month change in YoY growth (acceleration, pp)", "yoy_t - yoy_{t-1}", "derivative"),
            ("hst_yoy_zscore_120m", "rolling 120m z-score of YoY growth", "(yoy_t-mean120)/sd120", "derivative"),
            ("hst_yoy_contraction_flag", "1 when Housing Starts YoY growth < 0", "indicator(yoy<0)", "regime_state"),
    ...
    scope = {
        ...
        "notes": ("ECON-SD: only Housing Starts derivatives and SPY derivatives are in scope. The "
                  "Housing Starts LEVEL (hst_level) is SAAR and NON-STATIONARY (trend-dominated) — "
                  "excluded from the signal set. HOUST is SA at source, so MoM is a valid signal and no "
                  "deseasonalisation is applied. Only stationary growth/z-score transforms are used. "
    ...
    _write_evidence_status(winner, winner_summary, split, n_valid, median_sharpe,
                           winner_boot_p, verdict, stab, fwd_sig_lags, rev_sig_lags, oos_pos_mean)
...
## Lead-lag verdict
- Housing Starts->SPY TY-Granger significant lags: {fwd_sig_lags or 'NONE'}
- SPY->Housing Starts TY-Granger significant lags: {rev_sig_lags or 'NONE'}
...
"""
    ...
    sugg = {"schema_version": "1.0.0", "pair_id": PAIR_ID, "rule": "ECON-AS",
            "suggestions": [
                {"signal_name": "Housing Starts / Building Permits ratio", "proposed_by": "evan",
                 "source": "FRED HOUST / PERMIT",
                 "observation": "Starts-to-permits ratio captures execution vs pipeline; a falling ratio "
                                "(starts lagging permits) can flag construction softening before either level turns.",
                 "rationale": "Combines two housing leading indicators; the ratio may lead SPY more cleanly "
                              "than either series alone by netting out common rate-driven trends.",
                 "possible_use_case": "new pair / variant",
                 "caveats": "Both series SA; ratio is well-defined but permits/starts co-move strongly.",
                 "date_filed": "2026-08-14"},
                {"signal_name": "Mortgage-rate-conditioned Housing Starts signal", "proposed_by": "evan",
                 "source": "FRED HOUST / MORTGAGE30US",
                 "observation": "Housing Starts response to the cycle is rate-state dependent (2022-24 shock).",
                 "rationale": "Gating the starts signal by the mortgage-rate regime may sharpen it.",
                 "possible_use_case": "signal enhancement",
                 "caveats": "Adds a second series; risk of overfitting the small OOS window.",
                 "date_filed": "2026-08-14"}],
            "last_updated_by": "evan", "last_updated_at": NOW_ISO}
    ...
    # Evan handoff note
    handoff = f"""Handoff: Econ Evan -> Viz Vera / Research Ray / AppDev Ace

Pair: {PAIR_ID} (Housing Starts SAAR -> SPY), Mode 2.

Winner: {tw['winner_label']}
  OOS Sharpe {winner_summary['oos_sharpe']} vs B&H {winner_summary['bh_sharpe']} | DD {winner_summary['oos_max_drawdown']} vs {winner_summary['bh_max_drawdown']} | ann ret {winner_summary['oos_ann_return']} vs {winner_summary['bh_ann_return']}
  Direction: {direction} | valid combos {n_valid}/{len(tdf)-1} | cascade step {resolved_at} | ties@1 {n_tied}
  Bootstrap p={winner_boot_p:.3f} | durability '{verdict}' | rolling-corr '{stab}' | break flagged {sb['flagged']} ({sb['break_date']})

Lead-lag: {granger_verdict} Reverse-LP flag: {rev_flag}.

SA note for Ray/Vera: HOUST is seasonally adjusted (SAAR); MoM is a valid signal and no deseasonalisation
is applied. Charts/narrative must NOT plot or describe the raw SAAR level as a signal (non-stationary,
provenance only). The headline signal is '{sig_code}' ({SIGNAL_COLS[winner['signal']]}).

Key artifacts under results/{PAIR_ID}/: winner_summary.json, strategy_returns_{DATE_TAG}.csv,
winner_trade_log.csv, tournament_results_{DATE_TAG}.csv, core_models_{DATE_TAG}/, regime_quartile_returns.csv,
subperiod_sharpe.csv, rolling_correlation_{PAIR_ID}.csv, kpis.json, signal_scope.json, evidence_status.json.

evidence_status = found_in_search (no final exam yet). Ray to set strategy_objective (suggested: {tw['suggested_strategy_objective']}).
"""
    pws = os.path.join(BASE_DIR, "_pws", "lead-lesandro", "housing_starts_spy")
    os.makedirs(pws, exist_ok=True)
    with open(os.path.join(pws, "evan_handoff.md"), "w") as f:
        f.write(handoff)
    ...
    print(f"  Granger fwd lags: {fwd_sig_lags or 'NONE'} | rev lags: {rev_sig_lags or 'NONE'}")
    print(f"  Winner bootstrap p={winner_boot_p:.3f} | confidence={interp['confidence']}")
