#!/usr/bin/env python3
"""Pair pipeline: ISM Manufacturing PMI x SPY.

MONTHLY pipeline. The indicator is the ISM Manufacturing PMI, a diffusion index
(>50 = expansion) sourced from Data Master.xlsx. Bounded, mean-reverting — the LEVEL is
stationary and used directly. Economic prior: high/rising PMI marks expansion → PROCYCLICAL
(hold SPY when the PMI signals growth). Modelled on the SLOOS level-stationary template on a
monthly grid (sqrt(12) annualisation, leads in months).
"""
...
MASTER_SHEET = "ISM Manufacturing PMI"
MASTER_COL = "ISM Manufacturing PMI"
PERIODS_PER_YEAR = 12                 # monthly


def read_master() -> pd.Series:
    """ISM Manufacturing PMI, monthly, from Data Master."""
    import openpyxl
    ...
    s = pd.Series({pd.Timestamp(d): float(v) for d, v in rows}, name="ism").sort_index()
    s = s.resample("ME").last()
    print(f"  [MASTER] ISM Manufacturing PMI: {len(s.dropna())} mo, "
...
def source_data() -> pd.DataFrame:
    print("Sourcing ISM Manufacturing PMI (Data Master) and SPY month-end prices")
    raw = read_master()
    spy, spy_source = _read_spy_monthly()
    df = pd.concat([raw, spy], axis=1).dropna(subset=["ism", "spy"])
    ...
    df["ism_level"] = lvl
    df["ism_diff_1m"] = lvl.diff(1)             # month-on-month change
    df["ism_chg_12m"] = lvl.diff(12)             # 12-month change
    roll_mean = lvl.rolling(60, min_periods=24).mean()
    roll_std = lvl.rolling(60, min_periods=24).std()
    df["ism_zscore_60m"] = (lvl - roll_mean) / roll_std

    df = df.loc["1993-01-01":].copy()
    df.to_parquet(DATA_DIR / "ism_mfg_spy_monthly_latest.parquet")
    df.describe().T.to_csv(DATA_DIR / f"summary_stats_ism_mfg_spy_{DATE_TAG}.csv")
    missing = df.isna().sum().rename("missing_count").to_frame()
    missing["missing_pct"] = missing["missing_count"] / len(df)
    missing.to_markdown(DATA_DIR / f"missing_value_report_ism_mfg_spy_{DATE_TAG}.md")

    data_dict = pd.DataFrame([
        {"Column Name": "ism", "Display Name": "ISM Manufacturing PMI",
         "Description": "ISM Manufacturing PMI diffusion index (>50 = expansion)",
         "Source": "Data Master.xlsx", "Series ID": "NAPM", "Unit": "Index (diffusion)",
         "Transformation": "Monthly level",
         "Direction Convention": "Higher = manufacturing expansion (bullish equities)",
         "Effective Start": str(df["ism"].first_valid_index().date()),
         "Known Quirks": "Diffusion index bounded ~[0,100], pivots near 50, mean-reverting; classic leading indicator",
         "Display Note": "ISM PMI above ~50 marks expansion and risk-on conditions.",
    ...
    data_dict.to_csv(DATA_DIR / f"data_dictionary_ism_mfg_spy_{DATE_TAG}.csv", index=False)
...
def stationarity_report(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for col in ["ism_level", "ism_diff_1m", "ism_chg_12m", "ism_zscore_60m", "spy_ret"]:
        s = df[col].dropna()
        if len(s) < 20:
            continue
        try:
...
SIGNALS = {
    ...
    "diff_1m": "ism_diff_1m",
    "chg_12m": "ism_chg_12m",
    "zscore_60m": "ism_zscore_60m",
}
SIGNAL_DISPLAY = {
    "level": "ISM Manufacturing PMI (level)",
...
def run_tournament(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    winner = {
        ...
        "strategy_description": "Hold SPY when the lagged ISM Manufacturing PMI signal is favorable (expansion/optimism); else cash.",
...
def write_metadata(df: pd.DataFrame, winner: dict, elapsed: float) -> None:
    interp = {
        "pair_id": PAIR_ID, "schema_version": "1.1.0", "indicator_nature": "leading", "indicator_type": "production",
        ...
        "last_updated_by": "ray", "last_updated_at": NOW_ISO, "indicator": "ism", "target": "spy",
        "expected_direction": "procyclical", "observed_direction": winner["direction"],
        "direction_consistent": winner["direction"] == "procyclical",
        "mechanism": "The ISM Manufacturing PMI is a diffusion index (>50 = expansion). High or rising PMI marks manufacturing expansion and risk-on conditions; low/falling PMI marks contraction. The strategy holds SPY when the PMI signals expansion. PMI is a classic leading indicator of the business cycle.",
        "confidence": "low",
        ...
        "narrative_summary": "ISM Manufacturing PMI is tested as a procyclical growth overlay for SPY — hold equity when the PMI signals expansion.",
        ...
        "data_provenance": {"source": "Data Master.xlsx (ISM)", "series_id": "NAPM", "accessed_at": NOW_ISO}}
    ...
    signal_scope = {
        ...
        "indicator_axis": {"canonical_column": "ism", "display_name": "ISM Manufacturing PMI",
            "derivatives": [{"name": c, "definition": d, "formula": f, "role": r, "appears_in_charts": ch} for c, d, f, r, ch in [
                ("ism", "ISM Manufacturing PMI", "Data Master (ISM)", "raw", ["hero"]),
                ("ism_level", "PMI level (diffusion index)", "identity", "threshold_input", ["hero", "regime_stats"]),
                ("ism_diff_1m", "Month-on-month change", "x_t - x_{t-1}", "derivative", []),
                ("ism_chg_12m", "12-month change", "x_t - x_{t-12}", "threshold_input", ["local_projections"]),
                ("ism_zscore_60m", "Level vs 5-year history", "(x-mean60m)/std60m", "threshold_input", [])]]},
        ...
        "notes": "Scope limited to ISM Manufacturing PMI signals and SPY returns."}
    ...
    (RES / "analyst_suggestions.json").write_text(json.dumps({"pair_id": PAIR_ID, "schema_version": "1.0.0", "last_updated_at": NOW_ISO,
        "suggestions": [{"signal_name": "Lead robustness", "proposed_by": "evan", "source": "Data Master",
            ...
            "date_filed": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:]}], "notes": "Generated with the ISM Manufacturing PMI x SPY pair pipeline."}, indent=2) + "\n")
