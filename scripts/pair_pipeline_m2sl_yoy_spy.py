#!/usr/bin/env python3
"""
Full Econometrics Pipeline: M2 Money Supply YoY (M2SL) -> SPY
=============================================================
Pair ID: m2sl_yoy_spy
Branch: pair260619_m2sl_yoy_spy

Economic hypothesis (stated up front, per SOP):
  H0: M2 YoY growth does NOT predict / Granger-cause forward SPY returns.
  H1 (procyclical / liquidity): rising M2 YoY -> liquidity tailwind -> higher
      forward SPY; M2 *contraction* (the first-ever YoY decline, 2022-23) ->
      risk-off (coincided with the 2022 bear). Tradable signal likely long when
      YoY growth is strong / contraction_flag off.
  H1b (inflation counter-channel): excessive M2 growth -> inflation -> Fed
      tightening -> equity HEADWIND at a lag. The level-vs-acceleration
      distinction may matter (accelerating money vs already-high money).

  expected_direction (Ray/Dana, provisional): procyclical. Direction is
  determined empirically; both orientations are tested in the tournament and
  Granger is run in BOTH directions (monetary aggregates and equities are
  jointly driven by Fed policy / the cycle; reverse causality is plausible).

Category (Rule C1): macro/production -> full correlation battery (incl.
distance correlation), pre-whitened CCF, Toda-Yamamoto Granger (both
directions), transfer entropy, local projections (fwd + reverse), quantile
regression, HMM 2-state regime detection, regime quartile returns.

Data: Dana's parquet data/m2sl_yoy_spy_monthly_latest.parquet (400 x 14,
monthly; 1993-01 -> 2026-04; SPY-bound). Source = FRED M2SL live API (current
vintage; the Data Master M2SL snapshot is a stale vintage ~0.5% above current
FRED at recent dates due to SA revisions — FRED is ground truth; M2 is a
revised series). Stationarity tests were produced by Dana
(results/m2sl_yoy_spy/stationarity_tests_20260619.csv) — reviewed and confirmed
here, NOT re-run (SOP: confirm, don't duplicate). M2SL LEVEL is NON-stationary
and is NOT used as a signal; all signals are stationary growth/transform series.

Lag convention (binding, per brief): M2 (H.6) publishes ~4th Tuesday for the
prior month -> L1 is the real-time floor. Tournament lead grid centers L6 and
sweeps the standard grid starting at L1. Lead semantics: position at month t is
a function of the signal observed at month t-L; strategy_return_t =
position_t * spy_ret_t. With L>=1 there is no lookahead.
...
DATA_PATH = os.path.join(BASE_DIR, "data", "m2sl_yoy_spy_monthly_latest.parquet")
...
# M2SL LEVEL excluded (non-stationary, brief). Only stationary transforms used.
SIGNAL_COLS = {
    "yoy": "m2sl_pct_yoy",
    "mom": "m2sl_pct_mom",
    "mom3m": "m2sl_3m_pct",
    "mom6m": "m2sl_6m_pct",
    "accel": "m2sl_yoy_accel_pct",
    "yoy_zscore_120m": "m2sl_yoy_zscore_120m",
    "contraction": "m2sl_contraction_flag",
...
def write_manifest(path, columns, assertions, extra=None):
    m = {
        ...
        "pair_id": PAIR_ID,
        "produced_by": "scripts/pair_pipeline_m2sl_yoy_spy.py",
...
def stage_load():
    ...
    assert df.shape == (400, 14), f"unexpected shape {df.shape}"
    # Known-episode checks (Dana's handoff / interpretation_metadata)
    assert 26.0 < df.loc["2020":"2021", "m2sl_pct_yoy"].max() < 28.0, "COVID money surge ~27% YoY expected"
    assert df.loc["2022":"2023", "m2sl_pct_yoy"].min() < 0.0, "first modern M2 YoY contraction (<0) expected"
    assert int(df["m2sl_contraction_flag"].sum()) > 0, "contraction flag should fire (2022-23)"
    ...
    print("  Defense-2 episode checks: COVID surge ~27%, 2022-23 contraction, return magnitudes — PASS")
    return df
...
def stage_signals(df):
    ...
    cov_p = prob_stress.loc["2020-04-30":"2021-06-30"].mean()
    print(f"  HMM stress prob mean 2020-04..2021-06: {cov_p:.2f} (COVID money-surge era is high-variance)")

    write_manifest(os.path.join(MODELS_DIR, "hmm_states.parquet"),
                   {"hmm_state": "integer state; stress = high-variance M2-YoY-growth regime",
                    "prob_stress": "P(stress regime); higher = M2 growth in high-variance regime",
                    "prob_calm": "1 - prob_stress"},
                   [{"description": "probabilities in [0,1]", "check": "prob_stress between 0 and 1"},
                    {"description": "COVID money surge era is high-variance", "filter": "2020-04 to 2021-06",
    ...
    try:
        ms = MarkovRegression(ms_data["spy_ret"], k_regimes=2,
                              exog=sm.add_constant(ms_data["m2sl_pct_yoy"]),
    ...
    write_manifest(sig_path,
                   {c: f"tournament-eligible signal derived solely from M2SL ({c})" for c in sig_cols},
                   [{"description": "COVID YoY surge present", "filter": "2020-01..2021-12",
                     "column": "m2sl_pct_yoy", "check": "max > 26"},
                    {"description": "2022-23 first modern contraction", "filter": "2022-01..2023-12",
                     "column": "m2sl_pct_yoy", "check": "min < 0"},
...
def stage_core_models(df):
    ...
    print(f"        M2SL->SPY significant at lags: {list(fwd_sig.lag)}")
    print(f"        SPY->M2SL significant at lags: {list(rev_sig.lag)}")
...
def stage_quartiles(df):
    v = df[["m2sl_pct_yoy", "spy_ret"]].dropna()
    q = pd.qcut(v["m2sl_pct_yoy"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
    rows = []
    ...
    write_manifest(path,
                   {"quartile": "M2SL YoY growth quartile (Q1 lowest growth)", "n_months": "obs",
...
def stage_tournament(df):
    ...
    split = {
        ...
        "justification": (
            ...
            f"({n_months} months, 1993-02 onward; M2SL history extends to 1959 but the "
    ...
    manifest = {
        ...
        "execution_lag": "position_t = rule(signal_{t-lead}), lead >= 1 (L1 real-time floor per M2 H.6 publication lag; L6 centered)",
...
def stage_winner(df, tdf, split):
    ...
    sr_meta = {
        ...
        "produced_by": "scripts/pair_pipeline_m2sl_yoy_spy.py", "rule": "ECON-SR1",
        ...
        "generated_at": NOW_ISO, "generated_by": "Econ Evan (pair260619_m2sl_yoy_spy)",
    }
...
def stage_cross_period(df, winner, split, position, strat_ret, sig_lagged):
    ...
    with open(os.path.join(SCHEMA_DIR, "episode_registry.json")) as f:
        reg = json.load(f)
    episodes = reg.get("activity", reg["_fallback"])  # macro indicator -> activity episode set
...
def main():
    ...
    # signal_code for winner: registry-mapped. growth transforms -> m2sl_<code>; regimes -> generic
    if winner["signal"].startswith(("hmm", "markov")):
        sig_code = {"hmm_stress": "hmm_stress", "markov_regime": "markov_regime"}[winner["signal"]]
    else:
        sig_code = f"m2sl_{winner['signal']}"
    ...
    winner_summary = {
        ...
        "signal_display_name": f"M2SL {winner['signal']}",
        ...
        "lead_unit": "months",
        "lead_description": f"Signal lead = {int(winner['lead_months'])} month(s); L1 is the real-time floor given the ~4th-Tuesday H.6 publication lag for the prior month",
    ...
    if not long_when_high:
        sd += " (Countercyclical orientation: low/decelerating money growth = risk-on.)"
    else:
        sd += " (Procyclical orientation: strong money growth = risk-on, consistent with the liquidity channel.)"
    ...
    granger_verdict = (
        f"Toda-Yamamoto Granger M2SL->SPY significant at lags {fwd_sig_lags or 'NONE'}; "
        f"SPY->M2SL significant at lags {rev_sig_lags or 'NONE'}.")
    winner_summary["notes"] = (
        f"Mode 3, pair260619_m2sl_yoy_spy. Tournament: {len(tdf)-1} strategy combos "
        ...
        f"M2 is a revised series (FRED M2SL current vintage; Data Master snapshot is ~0.5% stale). "
    ...
    # --- ECON-SR1 winner_summary RE-RECONCILIATION (umcsent precedent guardrail) ---
    # Recompute OOS Sharpe FROM the encoded winner_summary rule (independent of tournament row).
    ...
    if abs(diff) > 0.03:
        raise SystemExit(
            f"winner_summary ENCODING MISMATCH: recomputed OOS Sharpe {recompute['oos_sharpe']:.4f} "
            f"!= headline {headline:.4f} (|diff|={abs(diff):.4f} > 0.03). The threshold/lead/rule "
            f"encoding in winner_summary does not reproduce the headline — fix the encoding (umcsent precedent).")
    print("  winner_summary encoding faithfully reproduces headline OOS Sharpe (±0.03): PASS")

    # --- ECON-DS3 registry assertion / append-if-missing ---
    ...
    interp["key_finding"] = (
        f"Lead-lag: M2SL YoY {'Granger-causes SPY at lags ' + str(fwd_sig_lags) if fwd_found else 'does NOT Granger-cause SPY at any lag 1-12'}; "
        f"reverse SPY->M2SL significant at lags {rev_sig_lags or 'NONE'}. "
        f"The tournament winner ({winner['signal']}/{winner['threshold']}/{strat_family} {orientation}/"
        f"L{winner['lead_months']}/{winner['lookback']}) is {direction}: OOS Sharpe "
        f"{winner_summary['oos_sharpe']:.2f} vs B&H {winner_summary['bh_sharpe']:.2f}, max DD "
        f"{winner_summary['oos_max_drawdown']*100:.1f}% vs {winner_summary['bh_max_drawdown']*100:.1f}%. "
        f"Bootstrap p={winner_boot_p:.3f}; durability '{verdict}'; rolling-corr '{stab}'.")
    # confidence: high only with established forward causality + durable; else low/medium
    ...
    ind_der = [
        ...
        for c, d, s, r in [
            ("m2sl_usd", "M2 money stock, $bn SA (FRED M2SL). NON-STATIONARY — not used as a signal", "FRED M2SL", "raw"),
            ("m2sl_pct_yoy", "12-month % change in M2 (headline cycle signal)", "100*(M_t/M_{t-12}-1)", "derivative"),
            ("m2sl_pct_mom", "1-month % change in M2", "100*(M_t/M_{t-1}-1)", "derivative"),
            ("m2sl_3m_pct", "3-month % change (money momentum)", "100*(M_t/M_{t-3}-1)", "derivative"),
            ("m2sl_6m_pct", "6-month % change (money momentum)", "100*(M_t/M_{t-6}-1)", "derivative"),
            ("m2sl_yoy_accel_pct", "month-on-month change in YoY growth (acceleration, pp)", "yoy_t - yoy_{t-1}", "derivative"),
            ("m2sl_yoy_zscore_120m", "rolling 120m z-score of YoY growth", "(yoy_t-mean120)/sd120", "derivative"),
            ("m2sl_contraction_flag", "1 when M2 YoY growth < 0 (first fired 2022-23)", "indicator(yoy<0)", "regime_state"),
    ...
    scope = {
        ...
        "indicator_axis": {
            "canonical_column": "m2sl_usd",
            "display_name": "M2 Money Supply (M2SL)",
            "derivatives": ind_der,
        },
        "target_axis": {
        ...
        "notes": ("ECON-SD: only M2SL derivatives and SPY derivatives are in scope. The M2SL LEVEL "
                  "(m2sl_usd) is NON-STATIONARY and is excluded from the signal set — only stationary "
                  "growth/transform series are used. regime_story: false (CP2 skipped)."),
    }
...
## Lead-lag verdict
- M2SL->SPY TY-Granger significant lags: {fwd_sig_lags or 'NONE'}
- SPY->M2SL TY-Granger significant lags: {rev_sig_lags or 'NONE'}
...
"""
    ...
    sugg = {"schema_version": "1.0.0", "pair_id": PAIR_ID, "rule": "ECON-AS",
            "suggestions": [
                {"signal_name": "Real M2 YoY (M2 deflated by CPI)", "proposed_by": "evan",
                 "source": "FRED M2SL / CPIAUCSL",
                 "observation": "Nominal M2 YoY conflates the inflation and liquidity channels (H1 vs H1b). "
                                "Real M2 growth isolates the liquidity impulse net of the inflation that "
                                "drives the Fed-tightening headwind.",
                 "rationale": "Monetarist literature treats real balances as the transmission variable; "
                              "deflating may sharpen the procyclical signal and resolve the level-vs-accel ambiguity.",
                 "possible_use_case": "new pair / variant",
                 "caveats": "Adds a second revised series (CPI); both are revised, compounding vintage risk.",
...
def _recompute_oos_from_winner_summary(df, ws, split):
    ...
    (signal_column + lead_value + threshold_code/value/rule + strategy_family +
    direction + lookback). This is the umcsent-precedent guardrail: it must
    reproduce the headline within +/-0.03 or the encoding is wrong."""
    work = df.dropna(subset=["spy_ret"]).copy()
    is_mask = work.index <= split["in_sample_end"]
    lookbacks = {"LB36": 36, "LB60": 60, "LB120": 120}
    col = ws["signal_column"]
    sig = work[col].shift(int(ws["lead_value"]))
    thr_name, lb_name = ws["threshold_code"], ws["lookback"]
    if thr_name.startswith("T1_fixed_p"):
        pct = int(thr_name.split("p")[-1]); thr = sig[is_mask].dropna().quantile(pct / 100)
    elif thr_name == "T4_zero":
        thr = 0.0
    elif thr_name.startswith("T2_roll_p"):
        lb = lookbacks[lb_name]
        ...
        lb = lookbacks[lb_name]; roll = sig.rolling(lb, min_periods=max(int(lb * 0.6), 24))
        k = float(thr_name.split("_")[-1])
        thr = roll.mean() - k * roll.std() if "neg_" in thr_name else roll.mean() + k * roll.std()
    else:
        raise ValueError(thr_name)
    # threshold_rule lt means the "neg_" / below-threshold long-condition
    above = sig < thr if ws["threshold_rule"] == "lt" else sig > thr
    # reconstruct orientation from direction: long_when_high == (rule==gt) for pro
    ...
    pos_bool = ~above if orientation == "counter" else above
    fam = ws["strategy_family"]
    if fam == "P1_long_cash":
        position = pos_bool.astype(float)
    elif fam == "P2_signal_strength":
        lb = lookbacks[lb_name]; roll = sig.rolling(lb, min_periods=max(int(lb * 0.6), 24))
        rng = (roll.max() - roll.min()).replace(0, np.nan)
        raw = ((sig - roll.min()) / rng).clip(0, 1)
        position = 1 - raw if orientation == "counter" else raw
...
def _ensure_signal_code_registered(sig_code, winner, parquet_col):
    ...
    if sig_code in existing:
        ...
        return
    descr_map = {
        "yoy": "12-month percent change in M2 money stock (FRED M2SL, $bn SA).",
        "mom": "One-month percent change in M2 money stock.",
        "mom3m": "Three-month percent change in M2 (money momentum).",
        "mom6m": "Six-month percent change in M2 (money momentum).",
        "accel": "Month-on-month change in M2 YoY growth (acceleration, pp).",
        "yoy_zscore_120m": "Rolling 120-month z-score of M2 YoY growth.",
        "contraction": "Binary flag: 1 when M2 YoY growth < 0 (first fired 2022-23).",
    }
    reg["signals"].append({
        "signal_code": sig_code,
        "display_name": f"M2 {winner['signal']} ({INDICATOR_NAME})",
        "parquet_column_pattern": parquet_col,
        "description": descr_map.get(winner["signal"], f"M2SL transform {winner['signal']}."),
        "source_method": "roc" if winner["signal"] in ("yoy", "mom", "mom3m", "mom6m") else (
            "zscore" if "zscore" in winner["signal"] else "transform"),
        "added_at": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:],
        "first_pair": PAIR_ID,
        "notes": "Money-supply growth transform; tested both procyclical and countercyclical in the tournament.",
    })
...
def _write_evidence_status(winner, ws, split, n_valid, median_sharpe, boot_p, verdict, stab,
    ...
    plain = (
        ...
        + ("Money-supply growth DID lead SPY in the formal causality test, consistent with the liquidity "
           "channel, " if fwd_found else
           "Money-supply growth did NOT lead SPY in the formal causality test, ")
