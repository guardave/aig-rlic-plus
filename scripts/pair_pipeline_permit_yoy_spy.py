#!/usr/bin/env python3
"""Pair pipeline: Building Permits YoY x SPY.

MONTHLY pipeline. The indicator is the YEAR-OVER-YEAR percent change of US building
permits (computed from the H Permit level in Data Master.xlsx). It is a growth rate,
zero-centred and roughly stationary: > 0 means permits are higher than a year ago
(housing/construction expanding), < 0 means contracting. Building permits are a classic
LEADING indicator of the business cycle (a Conference Board LEI component). Economic prior:
permits growing (YoY > 0) marks a cyclical upswing -> PROCYCLICAL (hold SPY when permit YoY
is positive/rising). Signals: the YoY level itself, its 1-month change, 12-month change, and
60-month z-score; natural zero threshold. Monthly grid, sqrt(12) annualisation.
"""
...
MASTER_SHEET = "H Permit"  # col "RE - H Permit" = building-permits level (thousands, SAAR)
MASTER_COL = "Building Permits YoY"
PERIODS_PER_YEAR = 12                 # monthly


def read_master() -> pd.Series:
    """Building Permits YoY, monthly, from Data Master."""
    import openpyxl
    ...
    lvl = pd.Series({pd.Timestamp(d): float(v) for d, v in rows}, name="permit_level").sort_index()
    lvl = lvl.resample("ME").last()
    # Indicator IS the year-over-year % change of building permits (a growth rate,
    # zero-centred and roughly stationary). Computed from the level (reproducible).
    s = (100.0 * (lvl / lvl.shift(12) - 1.0)).rename("permit_yoy")
    print(f"  [MASTER] Building Permits YoY (from H Permit level): {len(s.dropna())} mo, "
...
def source_data() -> pd.DataFrame:
    print("Sourcing Building Permits YoY (Data Master) and SPY month-end prices")
    raw = read_master()
    spy, spy_source = _read_spy_monthly()
    df = pd.concat([raw, spy], axis=1).dropna(subset=["permit_yoy", "spy"])
    ...
    df["permit_yoy_level"] = lvl
    df["permit_yoy_diff_1m"] = lvl.diff(1)             # month-on-month change
    df["permit_yoy_chg_12m"] = lvl.diff(12)             # 12-month change
    roll_mean = lvl.rolling(60, min_periods=24).mean()
    roll_std = lvl.rolling(60, min_periods=24).std()
    df["permit_yoy_zscore_60m"] = (lvl - roll_mean) / roll_std

    df = df.loc["1993-01-01":].copy()
    df.to_parquet(DATA_DIR / "permit_yoy_spy_monthly_latest.parquet")
    df.describe().T.to_csv(DATA_DIR / f"summary_stats_permit_yoy_spy_{DATE_TAG}.csv")
    missing = df.isna().sum().rename("missing_count").to_frame()
    missing["missing_pct"] = missing["missing_count"] / len(df)
    missing.to_markdown(DATA_DIR / f"missing_value_report_permit_yoy_spy_{DATE_TAG}.md")

    data_dict = pd.DataFrame([
        {"Column Name": "ism", "Display Name": "Building Permits YoY",
         "Description": "Year-over-year % change of US building permits (from H Permit level)",
         "Source": "Data Master.xlsx", "Series ID": "PERMIT (YoY of level)", "Unit": "Percent (YoY)",
         "Transformation": "Monthly level",
         "Direction Convention": "Higher (permits growing YoY) = housing/construction expansion (bullish equities)",
         "Effective Start": str(df["permit_yoy"].first_valid_index().date()),
         "Known Quirks": "Growth rate centred near 0; permits are a classic leading indicator (LEI component); SAAR level, revised",
         "Display Note": "Positive permit YoY (permits above a year ago) marks housing-led expansion and risk-on conditions.",
    ...
    data_dict.to_csv(DATA_DIR / f"data_dictionary_permit_yoy_spy_{DATE_TAG}.csv", index=False)
...
def stationarity_report(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for col in ["permit_yoy_level", "permit_yoy_diff_1m", "permit_yoy_chg_12m", "permit_yoy_zscore_60m", "spy_ret"]:
        s = df[col].dropna()
        if len(s) < 20:
            continue
        try:
...
SIGNALS = {
    "level": "permit_yoy_level",
    "diff_1m": "permit_yoy_diff_1m",
    "chg_12m": "permit_yoy_chg_12m",
    "zscore_60m": "permit_yoy_zscore_60m",
}
SIGNAL_DISPLAY = {
    "level": "Building Permits YoY (level)",
...
def threshold_codes_for(signal_code: str) -> list[str]:
    ...
    return ["T0_zero", "T_roll_p50", "T_roll_p75"]  # YoY level is zero-centred (growing vs shrinking)
...
def run_tournament(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    winner = {
        ...
        "strategy_description": "Hold SPY when the lagged Building Permits YoY signal is favorable (expansion/optimism); else cash.",
        ...
        "notes": "Building Permits YoY is a zero-centred growth rate (>0 = permits growing vs a year ago). Permits are a classic leading indicator (LEI component). Signals and thresholds are built on the YoY series; winner is search-selected — read as found-in-search.",
...
def write_metadata(df: pd.DataFrame, winner: dict, elapsed: float) -> None:
    interp = {
        "pair_id": PAIR_ID, "schema_version": "1.1.0", "indicator_nature": "leading", "indicator_type": "housing",
        ...
        "last_updated_by": "ray", "last_updated_at": NOW_ISO, "indicator": "permit_yoy", "target": "spy",
        "expected_direction": "procyclical", "observed_direction": winner["direction"],
        "direction_consistent": winner["direction"] == "procyclical",
        "mechanism": "Building permits lead construction and housing activity, one of the earliest cyclical movers. When permit growth (YoY) is positive and rising, housing and its supply chain are expanding, which is risk-on for equities; negative/falling permit growth flags a cyclical downturn. The strategy holds SPY when permit YoY is favorable (growing).",
        "confidence": "low",
        ...
        "narrative_summary": "Building Permits YoY is tested as a procyclical, leading housing-cycle overlay for SPY — hold equity when permit growth is positive/rising.",
        ...
        "data_provenance": {"source": "Data Master.xlsx (Building Permits, H Permit sheet)", "series_id": "PERMIT (YoY of level)", "accessed_at": NOW_ISO}}
    ...
    signal_scope = {
        ...
        "indicator_axis": {"canonical_column": "permit_yoy", "display_name": "Building Permits YoY",
            "derivatives": [{"name": c, "definition": d, "formula": f, "role": r, "appears_in_charts": ch} for c, d, f, r, ch in [
                ("permit_yoy", "Building Permits YoY", "Data Master (ISM)", "raw", ["hero"]),
                ("permit_yoy_level", "Permit YoY level (% change)", "identity", "threshold_input", ["hero", "regime_stats"]),
                ("permit_yoy_diff_1m", "Month-on-month change", "x_t - x_{t-1}", "derivative", []),
                ("permit_yoy_chg_12m", "12-month change", "x_t - x_{t-12}", "threshold_input", ["local_projections"]),
                ("permit_yoy_zscore_60m", "Level vs 5-year history", "(x-mean60m)/std60m", "threshold_input", [])]]},
        ...
        "notes": "Scope limited to Building Permits YoY signals and SPY returns."}
    ...
    (RES / "analyst_suggestions.json").write_text(json.dumps({"pair_id": PAIR_ID, "schema_version": "1.0.0", "last_updated_at": NOW_ISO,
        "suggestions": [{"signal_name": "Lead robustness", "proposed_by": "evan", "source": "Data Master",
            ...
            "date_filed": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:]}], "notes": "Generated with the Building Permits YoY x SPY pair pipeline."}, indent=2) + "\n")
