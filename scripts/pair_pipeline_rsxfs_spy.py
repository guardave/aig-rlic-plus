#!/usr/bin/env python3
"""Pair pipeline: Advance Retail Sales (RSXFS) x SPY.

Compact monthly pipeline for the portal's standard pair contract, modelled on
pair_pipeline_unrate_spy.py. RSXFS is a TRENDING nominal-dollar level (Advance
Retail Sales: Retail Trade, SA, FRED), so — unlike a bounded diffusion index —
the tradable signals are GROWTH transforms (YoY / MoM / 3-6m % change,
z-scored / accelerated), never the level itself. Economic prior: retail-sales
growth is procyclical with equities (strong consumer → risk-on).

Stage 1 (this file, so far): source RSXFS from FRED + SPY month-end, build the
...
def source_data() -> pd.DataFrame:
    print("Sourcing RSXFS from FRED and SPY month-end prices")
    rsxfs = _read_fred_csv("RSXFS").resample("ME").last()
    spy, spy_source = _read_spy_monthly()

    df = pd.concat([rsxfs.rename("rsxfs"), spy], axis=1).dropna(subset=["rsxfs", "spy"])
    ...
    lvl = df["rsxfs"]
    df["rsxfs_yoy"] = 100.0 * (lvl / lvl.shift(12) - 1.0)   # headline 12m growth
    df["rsxfs_mom"] = 100.0 * (lvl / lvl.shift(1) - 1.0)    # 1m growth
    df["rsxfs_3m"] = 100.0 * (lvl / lvl.shift(3) - 1.0)     # 3m growth
    df["rsxfs_6m"] = 100.0 * (lvl / lvl.shift(6) - 1.0)     # 6m growth
    roll_mean = df["rsxfs_yoy"].rolling(60, min_periods=36).mean()
    roll_std = df["rsxfs_yoy"].rolling(60, min_periods=36).std()
    df["rsxfs_yoy_zscore_60m"] = (df["rsxfs_yoy"] - roll_mean) / roll_std
    df["rsxfs_accel"] = df["rsxfs_yoy"].diff()             # change in YoY growth

    df = df.loc["1993-01-31":].copy()
    df.to_parquet(DATA_DIR / "rsxfs_spy_monthly_latest.parquet")
    df.describe().T.to_csv(DATA_DIR / f"summary_stats_rsxfs_spy_{DATE_TAG}.csv")
    missing = df.isna().sum().rename("missing_count").to_frame()
    missing["missing_pct"] = missing["missing_count"] / len(df)
    missing.to_markdown(DATA_DIR / f"missing_value_report_rsxfs_spy_{DATE_TAG}.md")

    data_dict = pd.DataFrame(
        [
            {
                "Column Name": "rsxfs",
                "Display Name": "Advance Retail Sales: Retail Trade",
                "Description": "Advance monthly retail trade sales (excl. food services)",
                "Source": "FRED",
                "Series ID": "RSXFS",
                "Unit": "Millions of USD",
                "Transformation": "Monthly level (SA)",
                "Direction Convention": "Higher growth = stronger consumer / risk-on",
                "Effective Start": str(df["rsxfs"].first_valid_index().date()),
                "Known Quirks": "Trending nominal level — model growth, not level; extreme 2020-21 COVID swings",
                "Display Note": "Advance estimate; revised in subsequent Monthly Retail Trade releases.",
    ...
    data_dict.to_csv(DATA_DIR / f"data_dictionary_rsxfs_spy_{DATE_TAG}.csv", index=False)
...
def stationarity_report(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for col in ["rsxfs", "rsxfs_yoy", "rsxfs_mom", "rsxfs_3m", "rsxfs_yoy_zscore_60m", "spy_ret"]:
        s = df[col].dropna()
        if len(s) < 24:
            continue
        try:
...
# Retail-sales GROWTH signals only (the level is non-stationary — never traded).
SIGNALS = {
    "yoy": "rsxfs_yoy",
    "mom": "rsxfs_mom",
    "chg_3m": "rsxfs_3m",
    "chg_6m": "rsxfs_6m",
    "yoy_zscore_60m": "rsxfs_yoy_zscore_60m",
    "accel": "rsxfs_accel",
}

SIGNAL_DISPLAY = {
    "yoy": "Retail sales YoY growth",
    "mom": "Retail sales MoM growth",
    "chg_3m": "Retail sales 3-month growth",
    "chg_6m": "Retail sales 6-month growth",
    "yoy_zscore_60m": "Retail sales YoY growth 60-month z-score",
    "accel": "Retail sales growth acceleration (ΔYoY)",
}
...
def make_position(signal: pd.Series, threshold: pd.Series | float, direction: str) -> pd.Series:
    ...
    else:  # procyclical: long when retail growth is strong (prior)
        pos = (signal >= thresh).astype(float)
...
def run_tournament(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    winner = {
        ...
        "strategy_description": "Hold SPY when the lagged retail-sales growth signal is favorable; otherwise hold cash.",
        ...
        "notes": "RSXFS is a coincident-to-slightly-leading consumer indicator; the nominal-dollar level is "
...
REP_SIGNAL = "rsxfs_yoy"  # representative growth signal for evidence econometrics
...
def write_evidence(df: pd.DataFrame, tourn: pd.DataFrame, winner: dict) -> None:
    ...
    if len(gdata) > 80:
        ...
        for lag, res in tests.items():
            ...
            granger_rows.append({"lag": lag, "f_stat": f_stat, "p_value": p_val, "direction": "RSXFS_to_SPY"})
...
def write_metadata(df: pd.DataFrame, winner: dict, elapsed: float) -> None:
    interp = {
        ...
        "indicator_nature": "coincident", "indicator_type": "macro",
        ...
        "indicator": "rsxfs", "target": "spy",
        "expected_direction": "procyclical", "observed_direction": winner["direction"],
        "direction_consistent": winner["direction"] == "procyclical",
        "mechanism": "Retail-sales growth tracks household consumption, the largest component of GDP; strong "
                     "consumer spending is risk-on for equities and slowing spending is an early demand-softening "
                     "signal. Exposure is scaled toward SPY when retail-sales growth is firm.",
        "confidence": "low",
        ...
        "caveats": [
            "Retail sales is broadly coincident with the cycle, so a long apparent lead is likely a search artifact.",
            ...
            "Advance retail-sales figures are revised in subsequent Monthly Retail Trade releases.",
        ],
        "narrative_summary": "RSXFS growth is tested as a procyclical consumer-demand overlay for SPY, not as a "
                             "clean leading indicator.",
        "known_stress_episodes": [
            {"label": "Dot-Com recession", "start": "2001-03-01", "end": "2001-11-30", "note": "Consumer spending softened."},
            {"label": "Global Financial Crisis", "start": "2007-12-01", "end": "2009-06-30", "note": "Retail sales fell sharply."},
            {"label": "COVID shock", "start": "2020-02-01", "end": "2020-04-30", "note": "Retail collapsed then rebounded violently."},
            {"label": "2022 rate shock", "start": "2022-01-01", "end": "2022-12-31", "note": "Nominal sales stayed high on inflation."},
        ],
        "data_provenance": {"source": "FRED", "series_id": "RSXFS", "accessed_at": NOW_ISO},
    }
    ...
    signal_scope = {
        ...
        "indicator_axis": {
            "canonical_column": "rsxfs",
            "display_name": "Advance Retail Sales: Retail Trade",
            "derivatives": [
                {"name": col, "definition": desc, "formula": formula, "role": role, "appears_in_charts": charts}
                for col, desc, formula, role, charts in [
                    ("rsxfs", "Advance retail trade sales level (nominal $, SA)", "FRED RSXFS", "raw", ["hero"]),
                    ("rsxfs_yoy", "12-month retail-sales growth", "100*(x_t/x_{t-12}-1)", "threshold_input", ["hero", "local_projections", "regime_stats"]),
                    ("rsxfs_mom", "1-month retail-sales growth", "100*(x_t/x_{t-1}-1)", "derivative", []),
                    ("rsxfs_3m", "3-month retail-sales growth", "100*(x_t/x_{t-3}-1)", "derivative", []),
                    ("rsxfs_6m", "6-month retail-sales growth", "100*(x_t/x_{t-6}-1)", "derivative", []),
                    ("rsxfs_yoy_zscore_60m", "YoY growth vs its five-year history", "(yoy - mean60)/std60", "threshold_input", []),
                    ("rsxfs_accel", "Change in YoY growth (acceleration)", "yoy_t - yoy_{t-1}", "derivative", []),
                ]
            ],
        },
        "target_axis": {
        ...
        "notes": "Scope limited to RSXFS-derived retail-sales growth signals and SPY returns.",
    }
    ...
    evidence_status = {
        ...
        "technical_note": f"Winner selected by OOS Sharpe from the {DATE_TAG} tournament grid; RSXFS is a coincident "
                          "consumer indicator and the long apparent lead is likely a search artifact.",
    ...
    analyst_suggestions = {
        ...
        "suggestions": [{
            "signal_name": "RSXFS lead-durability & revision robustness",
            "proposed_by": "evan", "source": "FRED",
            "observation": f"The selected rule uses an L{winner['lead_value']} lead on {winner['signal_code']}; retail "
                           "sales is coincident, so adjacent-lead durability and the COVID-window dependence should be checked.",
            "rationale": "Advance retail sales is revised and broadly coincident, so a long lead needs adjacent-lead "
        ...
        "notes": "Generated with the RSXFS x SPY pair pipeline.",
    }
