#!/usr/bin/env python3
"""Pair pipeline: SLOOS C&I Lending Standards — Net % Tightening (Small Firms) x SPY.

QUARTERLY pipeline (Fed Senior Loan Officer Opinion Survey). The indicator is the NET
PERCENT of domestic banks tightening C&I loan standards to SMALL firms — a bounded,
mean-reverting diffusion-style series (not a trending level), sourced from the project's
Data Master.xlsx 'C&I Loan' sheet (LEAD-DV1: that column is the SLOOS tightening survey,
NOT loan volumes — the C&I loan STOCK is FRED:BUSLOANS, a separate pair). Quarterly, NSA.

Economic prior: rising net-tightening is a CREDIT CRUNCH signal that LEADS equity weakness
by ~1-4 quarters → COUNTERCYCLICAL (reduce SPY when tightening is high/rising). Modelled on
the rsxfs monthly pipeline but on a quarterly grid (√4 annualisation, leads in quarters).

Stage 1 (this file, so far): source SLOOS + SPY quarter-end, build signals, stationarity,
write data artifacts. Tournament / evidence / metadata appended next.
"""
...
MASTER_SHEET = "C&I Loan"           # LEAD-DV1: this column IS the SLOOS tightening survey
MASTER_COL = "C&I Loan"
PERIODS_PER_YEAR = 4                 # quarterly


def read_sloos_master() -> pd.Series:
    """SLOOS net-% tightening (small firms), quarterly, from Data Master."""
    import openpyxl
    ...
    s = pd.Series({pd.Timestamp(d): float(v) for d, v in rows}, name="sloos_ci_small").sort_index()
    s = s.resample("QE").last()
    print(f"  [MASTER] SLOOS C&I small-firm tightening: {len(s.dropna())} q, "
...
def source_data() -> pd.DataFrame:
    print("Sourcing SLOOS (Data Master) and SPY quarter-end prices")
    sloos = read_sloos_master()
    spy, spy_source = _read_spy_quarterly()
    df = pd.concat([sloos, spy], axis=1).dropna(subset=["sloos_ci_small", "spy"])
    df["spy_ret"] = df["spy"].pct_change()
    for h in (1, 2, 4):
        df[f"spy_fwd_{h}q"] = df["spy"].pct_change(h).shift(-h)

    lvl = df["sloos_ci_small"]
    # Net-% tightening is bounded & mean-reverting; the LEVEL is a valid signal.
    df["sloos_level"] = lvl
    df["sloos_diff_1q"] = lvl.diff(1)             # quarter-on-quarter change in tightening
    df["sloos_chg_4q"] = lvl.diff(4)              # year-on-year change
    roll_mean = lvl.rolling(20, min_periods=12).mean()
    roll_std = lvl.rolling(20, min_periods=12).std()
    df["sloos_zscore_20q"] = (lvl - roll_mean) / roll_std

    df = df.loc["1993-01-01":].copy()
    df.to_parquet(DATA_DIR / "sloos_ci_small_spy_quarterly_latest.parquet")
    df.describe().T.to_csv(DATA_DIR / f"summary_stats_sloos_ci_small_spy_{DATE_TAG}.csv")
    missing = df.isna().sum().rename("missing_count").to_frame()
    missing["missing_pct"] = missing["missing_count"] / len(df)
    missing.to_markdown(DATA_DIR / f"missing_value_report_sloos_ci_small_spy_{DATE_TAG}.md")

    data_dict = pd.DataFrame([
        {"Column Name": "sloos_ci_small", "Display Name": "SLOOS C&I Tightening — Small Firms",
         "Description": "Net % of domestic banks tightening C&I loan standards to small firms",
         "Source": "Data Master.xlsx (Fed SLOOS)", "Series ID": "DRTSCIS (SLOOS)", "Unit": "Net percent",
         "Transformation": "Quarterly level (NSA)",
         "Direction Convention": "Higher = tighter credit / higher macro stress (bearish equities)",
         "Effective Start": str(df["sloos_ci_small"].first_valid_index().date()),
         "Known Quirks": "Survey diffusion index, bounded ~[-100,100], mean-reverting; leads ~1-4 quarters",
         "Display Note": "Rising tightening precedes credit crunches and equity weakness.",
         "Refresh Freq.": "Quarterly", "Refresh Source": "Data Master / Fed SLOOS"},
    ...
    data_dict.to_csv(DATA_DIR / f"data_dictionary_sloos_ci_small_spy_{DATE_TAG}.csv", index=False)
...
def stationarity_report(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for col in ["sloos_level", "sloos_diff_1q", "sloos_chg_4q", "sloos_zscore_20q", "spy_ret"]:
        s = df[col].dropna()
        if len(s) < 20:
            continue
        try:
...
SIGNALS = {
    "level": "sloos_level",
    "diff_1q": "sloos_diff_1q",
    "chg_4q": "sloos_chg_4q",
    "zscore_20q": "sloos_zscore_20q",
}
SIGNAL_DISPLAY = {
    "level": "SLOOS net-% tightening (level)",
    "diff_1q": "Quarterly change in tightening",
    "chg_4q": "4-quarter change in tightening",
    "zscore_20q": "Tightening vs 5-year history (z-score)",
}
...
def make_position(signal: pd.Series, threshold: pd.Series | float, direction: str) -> pd.Series:
    ...
    if direction == "countercyclical":
        # long SPY when tightening is LOW/easing (below threshold) — the credit-easy regime
...
def run_tournament(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    leads = [0, 1, 2, 3, 4]  # quarters (SLOOS leads ~1-4q)
    rows = []
    ...
    winner = {
        ...
        "strategy_description": "Hold SPY when the lagged SLOOS-tightening signal is favorable (easy credit); else cash.",
        ...
        "notes": "SLOOS C&I small-firm net-% tightening is a QUARTERLY credit survey that historically leads equity "
                 "weakness by 1-4 quarters. Net-% is bounded/mean-reverting; the level is stationary and used directly. "
                 "Quarterly OOS is small — read any winner as found-in-search.",
...
def write_evidence(df: pd.DataFrame, tourn: pd.DataFrame, winner: dict) -> None:
    ...
    if len(g) > 40:
        ...
        for lag, res in tests.items():
            ...
            granger_rows.append({"lag": lag, "f_stat": f, "p_value": p, "direction": "SLOOS_to_SPY"})
...
def write_metadata(df: pd.DataFrame, winner: dict, elapsed: float) -> None:
    interp = {
        "pair_id": PAIR_ID, "schema_version": "1.1.0", "indicator_nature": "leading", "indicator_type": "credit",
        ...
        "last_updated_by": "ray", "last_updated_at": NOW_ISO, "indicator": "sloos_ci_small", "target": "spy",
        ...
        "mechanism": "When banks tighten C&I lending standards to small firms, credit-dependent activity slows and "
                     "recession/earnings risk rises; equity exposure should be reduced when net-tightening is high or rising. "
                     "SLOOS historically leads the cycle by ~1-4 quarters.",
        "confidence": "low",
        ...
        "caveats": ["Quarterly survey with a small OOS window — winner is search-selected, needs holdout validation.",
                    "SLOOS is NSA and revised; the survey is qualitative (net-% of banks), not a hard activity measure.",
                    "Credit-tightening episodes are few, so the lead estimate rests on a handful of cycles."],
        "narrative_summary": "SLOOS C&I small-firm tightening is tested as a leading credit-stress overlay for SPY — reduce "
                             "exposure when banks tighten.",
        "known_stress_episodes": [
            {"label": "Dot-Com recession", "start": "2001-03-01", "end": "2001-11-30", "note": "Tightening rose ahead of the downturn."},
            {"label": "Global Financial Crisis", "start": "2007-12-01", "end": "2009-06-30", "note": "Net-tightening spiked to record highs."},
            {"label": "COVID shock", "start": "2020-02-01", "end": "2020-06-30", "note": "Banks tightened sharply in 2020-Q2."},
            {"label": "2022-23 tightening", "start": "2022-06-01", "end": "2023-12-31", "note": "Post-SVB credit tightening."}],
        "data_provenance": {"source": "Data Master.xlsx (Fed SLOOS)", "series_id": "DRTSCIS", "accessed_at": NOW_ISO}}
    ...
    signal_scope = {
        ...
        "indicator_axis": {"canonical_column": "sloos_ci_small", "display_name": "SLOOS C&I Tightening — Small Firms",
            "derivatives": [{"name": c, "definition": d, "formula": f, "role": r, "appears_in_charts": ch} for c, d, f, r, ch in [
                ("sloos_ci_small", "Net-% of banks tightening C&I standards to small firms", "Fed SLOOS (Data Master)", "raw", ["hero"]),
                ("sloos_level", "Tightening level (net %)", "identity", "threshold_input", ["hero", "regime_stats"]),
                ("sloos_diff_1q", "Quarter-on-quarter change", "x_t - x_{t-1}", "derivative", []),
                ("sloos_chg_4q", "4-quarter change", "x_t - x_{t-4}", "threshold_input", ["local_projections"]),
                ("sloos_zscore_20q", "Tightening vs 5-year history", "(x-mean20q)/std20q", "threshold_input", [])]]},
        ...
        "notes": "Scope limited to SLOOS C&I small-firm tightening signals and SPY returns."}
    ...
    (RES / "evidence_status.json").write_text(json.dumps({"pair_id": PAIR_ID, "schema_version": "1.2.0", "status": "found_in_search",
        ...
        "technical_note": f"Winner selected by OOS Sharpe from the {DATE_TAG} quarterly tournament; SLOOS leads but the OOS window is small.",
    ...
    (RES / "analyst_suggestions.json").write_text(json.dumps({"pair_id": PAIR_ID, "schema_version": "1.0.0", "last_updated_at": NOW_ISO,
        "suggestions": [{"signal_name": "SLOOS lead robustness", "proposed_by": "evan", "source": "Fed SLOOS",
            "observation": f"The rule uses an L{winner['lead_value']}q lead on {winner['signal_code']}; SLOOS is a leading credit survey.",
            "rationale": "Credit-tightening cycles are few, so the lead needs adjacent-lead durability and a cycle-count caveat.",
            "possible_use_case": "robustness check", "caveats": "Search-selected on a small quarterly OOS; needs final-exam validation.",
            "date_filed": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:]}], "notes": "Generated with the SLOOS x SPY pair pipeline."}, indent=2) + "\n")
