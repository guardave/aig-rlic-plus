#!/usr/bin/env python3
"""Pair pipeline: U. Michigan Expected Business Conditions x SPY.

MONTHLY pipeline. The indicator is the University of Michigan survey diffusion of expected
change in business conditions, sourced from Data Master.xlsx. Bounded, mean-reverting — the
LEVEL is stationary and used directly. Economic prior: rising optimism precedes stronger
spending → PROCYCLICAL (hold SPY when optimism is elevated). Monthly grid, sqrt(12).
"""
...
MASTER_SHEET = "UoM_ExptdChgBinesCyle"
MASTER_COL = "U. Michigan Expected Business Conditions"
PERIODS_PER_YEAR = 12                 # monthly


def read_master() -> pd.Series:
    """U. Michigan Expected Business Conditions, monthly, from Data Master."""
    import openpyxl
    ...
    s = pd.Series({pd.Timestamp(d): float(v) for d, v in rows}, name="umcsent").sort_index()
    s = s.resample("ME").last()
    print(f"  [MASTER] U. Michigan Expected Business Conditions: {len(s.dropna())} mo, "
...
def source_data() -> pd.DataFrame:
    print("Sourcing U. Michigan Expected Business Conditions (Data Master) and SPY month-end prices")
    raw = read_master()
    ...
    df.to_parquet(DATA_DIR / "umcsent_spy_monthly_latest.parquet")
    df.describe().T.to_csv(DATA_DIR / f"summary_stats_umcsent_spy_{DATE_TAG}.csv")
    missing = df.isna().sum().rename("missing_count").to_frame()
    missing["missing_pct"] = missing["missing_count"] / len(df)
    missing.to_markdown(DATA_DIR / f"missing_value_report_umcsent_spy_{DATE_TAG}.md")

    data_dict = pd.DataFrame([
        {"Column Name": "umcsent", "Display Name": "U. Michigan Expected Business Conditions",
         "Description": "U. Michigan expected-change-in-business-conditions diffusion",
         "Source": "Data Master.xlsx", "Series ID": "UMCSENT", "Unit": "Index (diffusion)",
         ...
         "Known Quirks": "Survey diffusion, bounded/mean-reverting; sentiment leads spending",
         "Display Note": "Rising expected business conditions precede stronger spending and risk-on equity conditions.",
    ...
    data_dict.to_csv(DATA_DIR / f"data_dictionary_umcsent_spy_{DATE_TAG}.csv", index=False)
...
SIGNAL_DISPLAY = {
    "level": "U. Michigan Expected Business Conditions (level)",
...
def run_tournament(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    winner = {
        ...
        "strategy_description": "Hold SPY when the lagged U. Michigan Expected Business Conditions signal is favorable (expansion/optimism); else cash.",
...
def write_metadata(df: pd.DataFrame, winner: dict, elapsed: float) -> None:
    interp = {
        ...
        "mechanism": "The University of Michigan expected-change-in-business-conditions series is a consumer diffusion measure of forward economic optimism. Rising optimism precedes stronger spending and risk-on equity conditions; falling optimism precedes retrenchment. The strategy holds SPY when optimism is elevated.",
        "confidence": "low",
        ...
        "narrative_summary": "U. Michigan expected business conditions is tested as a procyclical sentiment overlay for SPY.",
        ...
        "data_provenance": {"source": "Data Master.xlsx (University of Michigan)", "series_id": "UMCSENT", "accessed_at": NOW_ISO}}
    ...
    signal_scope = {
        ...
        "indicator_axis": {"canonical_column": "umcsent", "display_name": "U. Michigan Expected Business Conditions",
            "derivatives": [{"name": c, "definition": d, "formula": f, "role": r, "appears_in_charts": ch} for c, d, f, r, ch in [
                ("umcsent", "U. Michigan Expected Business Conditions", "Data Master (University of Michigan)", "raw", ["hero"]),
                ("umcsent_level", "Expected-conditions level (diffusion)", "identity", "threshold_input", ["hero", "regime_stats"]),
        ...
        "notes": "Scope limited to U. Michigan Expected Business Conditions signals and SPY returns."}
    ...
    (RES / "analyst_suggestions.json").write_text(json.dumps({"pair_id": PAIR_ID, "schema_version": "1.0.0", "last_updated_at": NOW_ISO,
        "suggestions": [{"signal_name": "Lead robustness", "proposed_by": "evan", "source": "Data Master",
            ...
            "date_filed": DATE_TAG[:4] + "-" + DATE_TAG[4:6] + "-" + DATE_TAG[6:]}], "notes": "Generated with the U. Michigan Expected Business Conditions x SPY pair pipeline."}, indent=2) + "\n")
